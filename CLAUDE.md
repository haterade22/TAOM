...
> **Target: Bannerlord 1.5.3** (installed, Steam beta; pinned in `.claude/pinned-game-version.txt`; on the hook's drift warning run `/engine-bump`). The `E:\Decompiled_Bannerlord\` dump matches (v1.5.3; older baselines preserved) but `ilspycmd`/`taom-src` on the installed DLLs is authoritative for signatures. Impact + history: [`docs/migration/v1.5.2-impact.md`](docs/migration/v1.5.2-impact.md) · [`v1.4.8-impact.md`](docs/migration/v1.4.8-impact.md) · [`TRACKING.md`](docs/migration/TRACKING.md) · [`v1.4.x-overview.md`](docs/migration/v1.4.x-overview.md).
...
| Session-start prints "ARMORY ART DRIFT", an Armory sync or art drop landed, "the artists changed meshes" | **`/armory-audit`** | None: always, BEFORE any battle or tournament smoke. A missing collision body is the #352 infinite load (#599). |
...
12 languages (BR, CNs, CNt, DE, FR, IT, JP, KO, PL, RU, SP, TR) × 3 modules ≈ 15K strings/lang.
...
| **Agents on a moving platform need a NAVMESH, not a teleport** | `SiegeTower` and `MissionShip` are the only engine classes doing this and NEITHER moves an agent: both attach a navmesh prefab (`NavMeshPrefabName`, group 1) whose faces ride the entity. Teleport-only works at 3.2 m, fails at 9 m. Physics is what they stand ON, navmesh makes it VALID: both, or they fall (#627). `docs/features/mumakil.md` |
| **A crew frame near a deck edge, or facing the mount, stops the bow** | An archer whose 0.74 m body overhangs its navmesh re-nocks forever at 0.85 draw, pinned at a CONSTANT height no teleport fixes; so does one whose sightline crosses the mount's head and rider. Frames go a body radius inside the deck and behind the shoulders; islands >0.74 m; pin locomotion once, not on a clock. A seat still holding an agent HANGS mission end (#627). `docs/features/mumakil.md` |
...
| **Game menus stop time** | A standard `GameMenu` stops time with dead controls, and `MapState` saves the open menu id, so a menu left open after a timed process starts reads as a frozen game that survives load. Land on the map after anything timed. A WAIT menu's condition delegate hides EVERY option (Leave too) when false: pass `true` (#567). `taom.time_status` / `taom.rescue_time` recover. |
| **A "frozen game" may be a stuck kingdom vote** | The popup's exit is a 5s timer needing an `IsKingsDecisionOver` false->true edge, not a click. No edge, no close: navigation locked, Escape dead, no crash. TWO causes, both `Patch80`: a cancelled election (#547) and a non-leader player (#550: takeover promotes, old saves repair at launch, seam D closes). `docs/features/diplomacy.md` |
...
| **Animation must be authored on the ENGINE skeleton** | Kit facts: FBX bone locals stored VERBATIM, root turned 180, root motion zeroed at frame 0, bone tracks in FBX NODE order while the engine reads the skeleton's LIST order. So: engine frames, a hierarchy whose depth-first walk is that list, REST at frame 0, take = master name. `docs/reference/bannerlord-skeleton-authoring.md` |
...
| **Player start kits** | Culture-default rosters hand out `starter_<donor>` twins; `player_career_*` and the vanilla-six override hand out the culture's lowest troop items (#629). `StarterKitCoverageTests` pins both layers. The override puts `_replaceWhileMerging="true"` on the ROSTER, never a set: `EquipmentSet` has no schema key. `docs/features/starting-equipment-tuning.md` |
| **Troop bows are generated `ladder_*` items** | Reach, damage, accuracy and Bow/Crossbow skill come from the tier cell. 123 `ladder_<line>_<bow|xbow>_t<tier>` items in the unversioned Armory, from `tools/ranged_ladders.json`; a reinstall drops them, archers spawn bowless, no error. `generate_ranged_ladder_items.py --verify`; gate `RANGED_LADDER_INVERSION`. `docs/features/ranged-ladders.md` |
...
| `session-start.sh` ("ARMORY ART DRIFT") | Run `/armory-audit` and act on any `MISSING_BODY` row (it names the troops) BEFORE any battle or tournament smoke. A `*_geo.tpac` newer than the committed catalogue means every ref audit is stale; the last time nobody ran it the elf start hung two days (#599). |
...
- Target: Bannerlord v1.5.3 (installed, Steam beta, since 2026-09-15; `ilspycmd` on the installed DLLs is authoritative)
- `E:\Decompiled_Bannerlord\`: v1.5.3 dump (category tree = shipping-client, strips editor code; dual-build `{_shipping_build,_editor_build}` for editor types; `_*_v1.4.8`, `_*_v1.5.0` and `_*_v1.5.2` baselines preserved; the Modding Kit moved with the client this time, so the map can be re-baked). Details: [bannerlord-engine-and-toolchain.md](docs/reference/bannerlord-engine-and-toolchain.md); installed DLLs stay authoritative for signatures.
