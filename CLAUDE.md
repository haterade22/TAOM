...
| **A tpac the Kit never saved is invisible, with no log line** | The client renders only packages with a `RuntimeDataCache/<package GUID>.rdc`, which ONLY the editor writes. A hand-built or artist-dropped tpac without one is skipped whole: items unregistered, blank icons, no `Unable to find`, no `Overriding item`. Open the module in the Kit, save, check the entry exists (#616). |
