...
| Vendored Main-module DLLs | `Main/_Module/bin/Win64_Shipping_Client/` — `MinHook.x64.dll`, `TAOM.NativeSkinFixes.dll`. Allowlisted in `.gitignore`. `TAOM.dll` + `TAOM.pdb` stay ignored (build outputs, regenerated each build). When `TAOM.NativeSkinFixes.dll` is rebuilt from C++ source (currently lives outside this repo), drop the new DLL here and `git add` it — otherwise teammates run the stale version. MCMv5 is provided by TAOM.Dependencies (`Bannerlord.MBOptionScreen*.dll` + `MCM.UI.Adapter.MCMv5.dll`) + the `Bannerlord.MCM` NuGet — do NOT vendor `MCMv5.dll` here. **As of 2026-05-24, the `BehaviorTrees.dll` and `BehaviorTreeWrapper.dll` vendored binaries are gone — their source was decompiled and inlined to `Main/BehaviorTrees/` + `Main/BehaviorTreeWrapper/` and compiles into `TAOM.dll`. RCA: `docs/reviews/rca-looter-battle-nre-2026-05-24.md`. Do NOT re-vendor; edit the inlined source instead.** |
| TAOM.Dependencies stub modules | `Stubs/Bannerlord.{Harmony,UIExtenderEx,ButterLib,MBOptionScreen}/_Module/SubModule.xml` — four passive stubs that declare the standard BUTR module IDs so third-party mods depending on those IDs are toggleable in the vanilla launcher. Each has `<SubModules />` empty (no DLL loads); real DLLs come from TAOM.Dependencies. Deployed via `DeployTAOMDependenciesStubs` MSBuild target in `Dependencies/TAOM.Dependencies.csproj`. When a BUTR `PackageReference` version bumps, bump the matching stub's `<Version>` too. See `docs/migration/dr3-maintenance.md` "Stub modules" section. |
