...
| Vendored Main-module DLLs | `Main/_Module/bin/Win64_Shipping_Client/` — `BehaviorTrees.dll`, `BehaviorTreeWrapper.dll`, `MinHook.x64.dll`, `TAOM.NativeSkinFixes.dll`. Allowlisted in `.gitignore` (same pattern as `Dependencies/_Module/bin/`). `TAOM.dll` + `TAOM.pdb` stay ignored (build outputs, regenerated each build). When `TAOM.NativeSkinFixes.dll` is rebuilt from C++ source (currently lives outside this repo), drop the new DLL here and `git add` it — otherwise teammates run the stale version. MCMv5 is provided by TAOM.Dependencies (`Bannerlord.MBOptionScreen*.dll` + `MCM.UI.Adapter.MCMv5.dll`) + the `Bannerlord.MCM` NuGet — do NOT vendor `MCMv5.dll` here. |
