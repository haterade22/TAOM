...
| Understand the quest/issue system, or convert vanilla quests to LOTR | [issue-and-quest-system.md](./docs/reference/engine/issue-and-quest-system.md) (engine A-to-Z: `IssueBase`/`QuestBase`/`IssueManager`/`QuestManager`, the 43-issue sandbox set, `RemoveBehaviors<T>` suppression, `SpecialQuestType` auto-cancel rule) + [lotr-issues.md](./docs/features/lotr-issues.md) (**PLAN** — TAOM conversion strategy + per-issue disposition matrix; replace + disable vanilla) |
