...
| **Armory asset trees** | No cooked tree: 0 `AssetPackages/*.tpac` vs 4,490 loose `Assets/**`. The engine loads loose, and loose WINS where both exist (TAOM/TAOM_Map ship both; `rgl_log` names `Assets` for each). The old stale-pack trap is unreachable here. `validate_mesh_refs.py` falls back and warns. `docs/reference/armory-guide.md` |
