...
| **Animation must be authored on the ENGINE skeleton** | Skinning is roll-independent, rotations are not: a clip authored on a mesh rig folds in the Kit. The Kit stores FBX bone locals VERBATIM and turns the root 180: rig from the engine frames (`retarget_mannequin_to_human.py --engine-skeleton`), export Y/X, turn the pose, REST at frame 0. `docs/reference/bannerlord-skeleton-authoring.md` |
