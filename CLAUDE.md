...
| **Never write a RECONSTRUCTED shared file to disk** | Rebuilding CHANGELOG.md from HEAD plus your own entry silently drops every other session's uncommitted entries, and the result looks well-formed (tell: near-equal inserts and deletes). 2026-08-28: two lost. Reconstruct into the INDEX (`git hash-object -w` + `git update-index`, which never touch the worktree); on disk, append in place |
