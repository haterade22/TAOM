...
| **Troop bows are generated `ladder_*` items** | Reach, damage and accuracy are the bow's, set per tier and kingdom rank; so is the troop's Bow skill. 123 `ladder_<line>_<bow|xbow>_t<tier>` items live in the unversioned Armory from `tools/ranged_ladders.json`; a reinstall drops them and archers spawn bowless, no error. `generate_ranged_ladder_items.py --verify`; gate `RANGED_LADDER_INVERSION`. `docs/features/ranged-ladders.md` |
