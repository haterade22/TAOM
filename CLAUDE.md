...
| EditorCacheRebuild | `Main/Features/EditorCacheRebuild/` (parallel + incremental + resumable settlement distance cache rebuild for the Bannerlord Editor — `Patch37_CacheBuildOverride` patches `NavigationCache<SettlementRecord>.GenerateCacheData()` via dynamically-built closed generic since `SettlementRecord` is `private sealed nested` in `SandBox.View.Map.SettlementPositionScript`; `NavigationCacheAdapter` is reflection-driven; `ParallelPhase1Builder` + `ParallelPhase2Builder` use `Parallel.For` + `ConcurrentBag` with locked dict writes; `SmokeTestGate` validates serial-vs-parallel pathfind equivalence at build start; `CheckpointSerializer` saves state between phases for crash recovery; `SettlementDiffer` + `ChangedSettlementsFilter` enable incremental Phase 1 when ≤30 settlements changed; `ValidationReportWriter` emits per-build JSON; ~108hr full rebuild → ~30min target) |
...
| `Patch37_EditorCacheRebuild` | Editor-only cache rebuild override. Patches `NavigationCache<SettlementRecord>.GenerateCacheData()` (Prefix returns false) on the editor's private-nested generic instantiation — runtime `NavigationCache<Settlement>` is untouched. Target resolved via `Type.GetType("SandBox.View.Map.SettlementPositionScript+SettlementRecord, SandBox.View")` + `MakeGenericType`. **Only fires in editor mode** because runtime cache loading uses a different closed generic. | `NavigationCache<SettlementRecord>.GenerateCacheData()` (Prefix) |
