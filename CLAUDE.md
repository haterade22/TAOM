...
| **Carrying an agent by teleport stops it shooting** | A seated agent's 0.10 m settle, corrected every frame, reads as 30 m/s and no draw completes: use a deadband, and space crew frames 0.74 m or they shove each other into it. A seat still holding an agent HANGS mission end (`IsDeactivated` spins on `HasAIMovingTo`); detaching kills the Ranged curve (#627). `docs/features/elephant.md` |
