...
| **Player start kits** | Culture-default rosters hand out `starter_<donor>` twins; `player_career_*` and the vanilla-six override hand out the culture's lowest troop items (#629). `StarterKitCoverageTests` pins both. The override puts `_replaceWhileMerging="true"` on the ROSTER, never a set: `EquipmentSet` has no schema key. `docs/features/starting-equipment-tuning.md` |
