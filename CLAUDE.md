...
| **Elephant howdah / bone-tracking** | Crew ON for the howdah harness since 2026-09-19 (#627): archers spawned from `OnMissionTick`, one `HowdahCrewAgentOrigin` each (a shared mahout origin booked crew deaths as the rider's). Bone tracking still DEFERRED (slide source). Log: `docs/features/elephant.md` "Reading the howdah log". |
