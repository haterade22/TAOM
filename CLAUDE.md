...
| **Enlisted service** | Parked hidden+inactive on a lord is legitimate — so is ACTIVE+VISIBLE inside the commander's settlement, AND during the commander-unavailable grace. Only `DischargeService` ends service. Duties never detach, spawn or destroy anything (#428). The enlisted player is alone on `PlayerTeam`; his commander is on `PlayerAllyTeam` (#443). `docs/features/enlistment.md`. |
...
73 categories mapping a stack trace to its owning feature -> exact target -> status.
