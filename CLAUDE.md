...
| **Enlisted service** | Parked hidden+inactive on a lord is legitimate, so is ACTIVE+VISIBLE in the commander's settlement and during the grace. Only `DischargeService` ends service. Duties never detach/spawn/destroy (#428). For ONE battle the player joins the commander's army, so both share `PlayerTeam`; outside it `Army` is null (#443). `docs/features/enlistment.md`. |
