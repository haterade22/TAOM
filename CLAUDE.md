...
| Consult / append the master lessons-learned record (every `/deep-review`, `/review-codex`, and RCA lesson, by subsystem) | [docs/reviews/LESSONS-LEARNED.md](./docs/reviews/LESSONS-LEARNED.md) (index) → per-category files at `docs/reviews/lessons/<category>.md` — **read the relevant category file before touching a subsystem; append a `### rule / Why missed / Prevent / Source` entry there after every RCA.** Indexed from the harness memory (`~/.claude/projects/.../memory/MEMORY.md`) |
...
| SkipCampaignIntro | `Main/Features/SkipCampaignIntro/` (`Patch58` Prefix on `SandBoxGameManager.OnLoadFinished` skips the vanilla SandBox `campaign_intro.ivf` on a NEW game → straight into character creation. Mirrors the engine's own `IsDevelopmentMode` no-video branch: for `!LoadingSavedGame` it calls the private `LaunchSandboxCharacterCreation()` + sets the trailing `MBGameManager.IsLoaded=true` (both `AccessTools`-cached) and returns false so the original never builds the `VideoPlaybackState`. Save-loads run vanilla untouched; any binding-null/exception falls back to the vanilla intro. **Hardcoded always-skip, no MCM toggle.** Applied in `OnSubModuleLoad` — NOT the late batch — because the target fires during the new-game load (after campaign init, before CC). Two drift-guard tests pin the internal bindings; `HarmonyPatchBindingTests` covers the target. See `docs/features/skip-campaign-intro.md`.) |
| NativeSkinFixes | `Main/Features/NativeSkinFixes/` (managed wrapper for `TAOM.NativeSkinFixes.dll` — covers_head morph fix + hair/beard cloth simulation. 3 P/Invoke interop classes + installer. Editor-mode skip. **PARKED 2026-07-08 — OFF by default, DISABLED at the wiring level:** the install call in `OnBeforeInitialModuleScreenSetAsRoot` is commented out so the native hooks never load regardless of any persisted MCM value (`EnableNativeSkinFixes` default flipped to `false`); engine rendering is vanilla. RE-ENABLE = uncomment the install branch in `SubModule.cs` + flip the default back to `true`. When active: loaded from `OnBeforeInitialModuleScreenSetAsRoot`, uninstalled from `OnSubModuleUnloaded`. C++ source at `Dependencies/NativeSkinFixes.NativeHooks/`. See `docs/features/native-skin-fixes.md`.) |
...
| CultureConversion | `Main/Features/CultureConversion/` (conquered town/castle + bound villages GRADUALLY adopt the new owner's culture — `OnSettlementOwnerChanged` starts a hold-timer toward the owner culture (gated on `IVolunteerRecruitmentService.HasCulturePool` + optional player-owned/loyalty gates); `DailyTickEvent` completes it after `RequiredHoldDays`, setting `Settlement.Culture` on town+villages, **replacing foreign-culture notables** with same-occupation notables from the new culture's `NotableTemplates` (#325, 2026-07-03 — workshops/alleys/caravans transfer to the replacements, relations reset, active issues cancelled; order-critical adapter sequence: template pre-check [CreateNotable NREs on a missing occupation template] → CreateNotable → property transfer → `CompleteIssueWithCancel` → `AddPower(-Power)` [a notable dying at power ≥ 100 spawns an OLD-culture relative heir] → `ApplyByRemove`; toggleable `replaceNotablesOnConversion`, default on; one-shot at conversion, never on load re-apply), and clearing notable `VolunteerTypes` so recruits repopulate from the new pool. `Settlement.Culture` is NOT engine-saved → completed overrides persist in `CultureConversionStore` (composite-string SyncData, R-format+NaN reject like `PendingMessenger`) and re-apply on `OnGameLoadedEvent`. `SettlementConversionRecord` captures the ORIGINAL culture once so reconquest-back-to-original removes the override (restores vanilla same-culture loyalty). Recruitment integration: TAOM recruitment ignores `settlement.Culture` (keys on ~81 hard-coded settlement pools), so `VolunteerContext` gained `IsConvertedSettlement`+`SettlementCultureId` (set by `VolunteerContextAdapter` via `ICultureConversionStore.IsConverted` on settlement OR bound parent) and `GetVolunteerTroopId` resolves `CultureMap[SettlementCultureId]` before settlement/clan pools for converted fiefs. Thin behavior → pure `CultureConversionService` → `ICultureConversionAdapter` (Settlement.Find/Culture/BoundVillages/Town.Loyalty/VolunteerTypes) per ADR-002/007. Config `culture_conversion/culture_conversion_config.json` + MCM "Culture Conversion" group. See `docs/features/culture-conversion.md`.) |
| CaravanTrade | `Main/Features/CaravanTrade/` (AI/player caravans range past the local town cluster instead of shuttling between very-close towns, trade across the endless Free-vs-Evil war, and carry fuller baskets. Four Harmony postfixes on vanilla `CaravansCampaignBehavior` private methods (`Patch59_CaravanTrade`) + two `TaomCaravanModel` overrides → pure `ICaravanTradeService`. Levers: **war gate** (`CanTradeWith` — lift the war veto per `WarTradePolicy` {None/IgnoreWar/SameAlignmentAndNeutral, default same-side+neutral}, resolving sides via `IAlignmentService.GetKingdomSide` with `GetCultureSide` fallback for player-founded kingdoms; only flips a war-caused `false→true`; honors the player's prohibited-kingdom list during war); **range re-weight** (`GetTradeScoreForTown` — strips vanilla's `1/days` distance spike, re-applies `1/(nearFieldFlatten+days)^decay` clamped, cuts the just-left town's score; selection-only); **range envelope** (`GetDistanceLimitVeryFarAsDaysForNavigationType` scale-on-read postfix — widens the "very far" ceiling; reads the master toggle live so master-off reverts instantly; engine-global, not player-scopable); **basket diversity** (`CalculateBudgetFactor` floor + `TaomCaravanModel.GetInitialTradeGold`/`GetMaxGoldToSpendOnOneItemCategory` — poor caravans clear more of the per-category buy gate). **"Further = more money" is emergent** (vanilla already prices undersupplied far towns up to 10× — the levers just let caravans reach them; the existing ClanFinance drip pays the owner more), no injected gold, no SyncData — save-clean; master-off = exact vanilla. Applies to player caravans too (MCM-toggleable; the range + war-gate levers scope at faction/global level, the re-weight + diversity at clan level — see the feature-doc Known Limitations). Validating config `caravan_trade/caravan_trade_config.json` + MCM "Caravan Trade". Deep-review HIGH (`AreEnemyAlignments` Neutral inversion) + 4 Codex MED all fixed. Issue #329. See `docs/features/caravan-trade.md`.) |
| WarOfTheRingMomentum | `Main/Features/WarOfTheRingMomentum/` (+ `UI/`) (Evil-vs-Good war-progress meter ported from LOTRAOM's "Momentum" — #327. Battles/sieges/raids/army-gatherings/daily-strength feed a per-side ×100 momentum with 504/168/12h decay; player participation ×1.5 + a ≥5-player-event victory gate (both MCM-tunable). Dynamic enrollment: every `IAlignmentService` Free/Evil kingdom sweeps in at `WarPhase.FullWar` (culture-fallback covers player-founded kingdoms; the sweep also reconciles alignment changes + eliminations — e.g. Khand/`battania`→Neutral drops it live). Victory at ±`victoryThreshold` (or a side eliminated, player-gated) sets the terminal `WarPhase.WarEnded` — which lifts all three Diplomacy peace-block layers at once (they key off `IsWarOfTheRingActive`/`ShouldBlockPeace`) — then peace-out via `IAllianceAdapter.MakePeace` (EndWar-before-MakePeace ordering pinned). Thin `WarOfTheRingMomentumBehavior` routes CampaignEvents → `IWarEventSnapshotAdapter` (flat DTOs, `TroopCasualties`) → pure services; kingdoms resolve via `Kingdom.All.FirstOrDefault` (NOT `MBObjectManager.GetObject<Kingdom>` — returns null for campaign kingdoms). Persistence = the store dict JSON-encoded then **split across chunk keys** (`_taom_wotr_momentum_v3_count` + `_v3_{i}`, each ≤10,000 chars) via `MomentumSyncChunker` — a `Dictionary<string,string>` didn't round-trip `IDataStore` at ~1000 entries, but the single-string replacement (`_v2`) then crossed the engine's 32,767-byte archive-entry limit and CORRUPTED saves at write time past day ~50 (the v2.0.9 "A problem occured…" reports; `ArchiveSerializer` truncates entry length to int16). Chunking caps every synced string below the limit; already-bricked saves recover offline via `tools/repair_sav_strings.py` (player how-to: `docs/SAVE-REPAIR-GUIDE.md`). RCA `docs/reviews/rca-momentum-save-corruption-2026-07-07.md`. UI: on-map bar (`MapView`+`GauntletLayer`) whose value is a RELATIVE-BALANCE ratio `(free−evil)/(free+evil)` in −100..+100, **positive = Free = fills right/green** (a threshold fraction pinned in a long war); click → popup (banners via `BannerImageIdentifierVM`, leaders/allies, breakdown+tooltips, total stats) with a colored bounded Total (green Free / red Evil). The bar wears custom LOTR art (`GUI/SpriteParts/ui_taom/WarOfTheRing/wotr_{frame,fill,ring}.png` + `WarOfTheRing.Bar.*` brushes in `BalanceOfPower.xml`): opaque obsidian+gold frame (Eye of Sauron left = Evil, White Tree right = Free) as background, red\|green track sized to its recessed channel (55.4%×34.2%, centered), One Ring as the sliding handle — bake the 3 loose PNGs via the editor `SpriteSheetGenerator` before they render (loose = blank). Config `momentum/momentum.json` + MCM "War of the Ring/Momentum". Depends on Diplomacy (`IWarOfTheRingService` phase machine) + Execution (`IAlignmentService`, `execution/alignment.json`). See `docs/features/war-of-the-ring-momentum.md` + the play-test RCA `docs/reviews/rca-wotr-momentum-2026-07-03.md`.) |
| SaveLoadDiagnostics | `Main/Features/SaveLoadDiagnostics/` (always-on `[SaveLoad]` lifecycle logging — the engine swallows the real load exception behind the generic "A problem occured while trying to load the saved game." dialog. 15 thin Harmony hooks (`Patch61_SaveLoadDiagnostics`, see the Harmony table) stamp the exact failing type/`SaveId`/chunk to `Logs/taom_debug_*.log`; save-side hooks catch bad WRITES at write time; `TAOM_Build` metadata stamp identifies each save's build. Root-caused the v2.0.9 momentum >32 KB save corruption 2026-07-07. Offline companions `tools/inspect_sav.py` (triage) + `tools/repair_sav_strings.py` (recovery; player how-to `docs/SAVE-REPAIR-GUIDE.md`). See `docs/features/save-load-diagnostics.md` + RCA `docs/reviews/rca-momentum-save-corruption-2026-07-07.md`.) |
...
| Giant Spider | `Main/Features/Spider/` (Dol Guldur giant spider as a **ridden mount** — `taom_spider_creature` goblin rider + `spider_mount_a` Horse-slot item; vanilla cavalry spawn, NO spawn patch. Per-agent `SpiderBehaviorTree` directional attacks (pounce + left/right swipe by enemy bearing, elephant-mirrored; bite-collision on the front legs joint40-44; lethal damage + 20% per-hit crit) attached by `SpiderMissionBehavior` keyed on `Monster.StringId=="spider"`; pure `SpiderAttackService` with warg-pattern rider damage attribution; mount-lock in `TaomAgentStatCalculateModel`. Data lives in LOTRLOME_Armory: Monster `num_paces=6`/`Mountable`/`rider_sit_bone=chest_m`, `as_spider` (quadrupedal, + `_town_and_village`/`_map` children, explicit canter binding) + full-mount-surface usage set in the ROOT `action_sets.xml`/`monster_usage_sets.xml` (live via `project.mbproj` standard `soln_*` ids ONLY — subfolder copies are superseded), L/R-split meshes + `<AdditionalMeshes>` (split done 2026-06-05; its "~40 per-mesh palette" rationale is **FALSE** — corrected 2026-06-13: the only bone cap is the 64-bone `Skeleton.MaxBoneCount`, NOT per-mesh; one mesh skins the whole ≤63-bone skeleton, so a body is never split for bone count — see `feedback_no_40_bone_per_mesh_limit`). **CRITICAL lesson: movement clips MUST carry the `quad_movement` tag + step points in `_anm.tpac`** — untagged gait clips AV (+0x10) on first tick in every mount context (the 2026-06-10/11 RCA; 4 clips byte-patched on the source elephant clip template, originals `*.bak-untagged`). Detached-combatant architecture (Patch45 spawn-swap + wield guards) DELETED 2026-06-10. Proven in battle 2026-06-11; v1.4.6 river-battle green 2026-06-12; directional attacks + lethal damage/crit + front-leg bite-collision + Patch47/48 dismount guards 2026-06-15. See `docs/features/spider.md` (arch + full RCA) + `spider-skeleton-animation-pipeline.md`.) |
| War Elephant | `Main/Features/Elephant/` (Harad war elephant: ridden mount that auto-attacks, modeled on the warg. **Per-agent behavior tree** `ElephantBehaviorTree` building the SHARED `Main/Features/ElephantLike/` nodes (2026-07-01 Elephant↔Mûmakil unification, #305 — nodes bound via `ElephantCombat.Profile`) mirrors the warg pattern — boundary nodes hold the raw `Agent` and delegate pure decisions to the TaleWorlds-free `ElephantAttackService` (a thin binding of the shared `ElephantLikeAttackService`; `ShouldEngage`/`IsOffCooldown`/`ComputeInflictedDamage`); NO adapter expansion, NO new IoC reg (the BT attaches inside `ElephantMissionBehavior` via `BTRegister`, nodes resolve the service lazily via the profile). **Deterministic cooldown attack model** (replaces the source mod's random per-tick roll): enemy-in-range → trample if off 10s cooldown → else L/R tusk swing by enemy bearing if off 4s cooldown → else idle (engine mount AI continues). Clip roles VERIFIED by Blender trajectory analysis: `act_elephant_attack_3/4`=trample thrash, `_1`=left swing, `_2`=right swing (named consts in `ElephantConfig`; `ElephantCombat.Profile` eager-resolves them + Index-compares for the already-attacking gate; `Initialize` logs if any → `act_none`, the Armory-drift "slide" guard). **Howdah** (`TaomHowdahMachine` + `TaomHowdahStandingPoint` + `Main/_Module/Prefabs/taom_howdah_agent.xml`) instantiates + tracks the elephant — **crew force-spawn + spine bone-tracking are DEFERRED-disabled** (both confirmed "slide" sources: physics bodies inside the elephant collision capsule; re-enable with a shared-`FaceGroupId` crew/floor collision fix). Mount-lock (`CanAgentRideMount=false`+`MountDifficulty=999`) lives in `TaomAgentStatCalculateModel`. Rider troop `harad_elephant_rider` (`troops_harad.xml`, level 51, `Culture.aserai`) recruitable ONLY by `clan_aserai_1` (Ayerikkä) via `VolunteerRecruitmentService.InitializeHaradClans`. Mount-lock + the structural trample/tusk mechanic are a behavioral port of the source mod; the cooldown cadence and the per-kind randomized **damage** (trample 50-100, tusk 50-75, ×0.25 on shield block, per-victim roll) are TAOM's own rebalance, not the donor's per-tick ~20-fixed roll. Rider `harad_elephant_rider` is `Cavalry` armed with a spear (`eastern_spear_4_t4`) + sword so it CHARGES (the 2026-06-15 bow-archer loadout was reverted 2026-06-29 — as a HorseArcher the formation skirmished at range and never closed to trample). Data authored for 1.4.5. See `docs/features/elephant.md` (incl. "Slide root-cause isolation" + "Behavior tree"). Issue #278.) |
| Mumakil | `Main/Features/Mumakil/` (the giant Oliphaunt — a **scaled-up clone of the War Elephant** minus the howdah. Shares the elephant rig (`elephant_skeleton`, 60 bones), the `as_elephant` action set, and ALL elephant clips incl. `act_elephant_attack_*` — so NO new animation/`quad_movement`/action-set work, NO native crash guards. **Size = Horse-item `BodyLength`, NOT a Monster field (there is none): `taom_mumakil` item `body_length="300"` → `SetInitialAgentScale(3.0)`, which scales skeleton + ragdoll + collision capsule uniformly.** Per-agent `MumakilBehaviorTree` (trample 10s → L/R tusk 4s → idle; builds the SHARED `ElephantLike` nodes bound via `MumakilCombat.Profile` since the 2026-07-01 unification, #305) attached by `MumakilMissionBehavior` keyed on `Monster.StringId=="taom_mumakil"`; pure `MumakilAttackService` (24 tests; thin binding of the shared `ElephantLikeAttackService`); mount-lock = 4th injected `IMumakilAttackService` in `TaomAgentStatCalculateModel`. Monster `lotr_monster_mumakil.xml` reuses `as_elephant`/`elephant` usage with an ENLARGED `<body_capsule>` (the elephant-proportioned capsule is too small for the longer 3× mesh — enemies run into the mesh before colliding). Rider `harad_mumakil_rider` (`troops_harad.xml`, `Cavalry`, spear+sword, NO HorseHarness — platform is baked into the mesh) recruitable via `clan_aserai_1`. Mesh + Monster + item live in external LOTRLOME_Armory (not in repo). `tools/audit_mount_parity.py`/`verify_mount_assets.py` extended for mumakil (the latter searches the shared elephant asset dir via `extra_asset_dirs`). In-game confirmed 2026-06-29. See `docs/features/mumakil.md`.) |
...
| SettlementEconomy config | `Main/_Module/ModuleData/settlement_economy/settlement_economy_config.json` (3 town-gold regen knobs: base / perProsperity / regenRate; **ships BUFFED — base 25000 vs vanilla 10000** so drained town markets recover, #317; validated — finite, base [0,200000], slope [0,100], rate [0,1], invalid reverts to shipped default; singleton-cached → app restart to reload) |
| CaravanTrade config | `Main/_Module/ModuleData/caravan_trade/caravan_trade_config.json` (11 knobs across the four levers: master enable + applyToPlayer, rangeMultiplier/distanceDecayExponent/nearFieldFlattenDays/maxCompensation/antiShuttlePenalty (range + re-weight), warTradePolicy string (validated against the known set — M1 typo-trap), budgetFactorFloor/initialTradeGold/maxGoldPerCategory (diversity); validated field-by-field with `FiniteFloatValidator` + ordering + string-set, invalid reverts to shipped default; MCM "Caravan Trade" overrides the headline knobs at runtime; singleton-cached → app restart to reload) |
...
| VolunteerRecruitment service | `Main/Features/TroopProgression/VolunteerRecruitmentService.cs` (per-settlement / per-clan / per-culture pools driving `TaomVolunteerModel.GetBasicVolunteer`. Static-ctor `InitializeXxx*()` methods for hand-written cultures (since 2026-07-01 split into per-culture partial files under `RecruitmentPools/VolunteerRecruitmentService.<Culture>.cs`, #308 — core file keeps maps + lookup cascade + weighted pick); instance-side lazy `EnsureGondorJsonLoaded` for JSON-driven Gondor. `AddSettlementConditional` + `ConditionalSettlementMap` for state-sensitive pools — predicate evaluated per-lookup against live `VolunteerContext`. Ithil Guard at `town_ES2` only when Gondor-owned is the canonical conditional rule. Feature doc: [docs/features/volunteer-recruitment.md](./docs/features/volunteer-recruitment.md).) |
...
| `TaomPartySizeModel` | `DefaultPartySizeLimitModel` | Party size feats (Mordor, Gundabad, DG, Isengard, Gondor) + career PartySize passive + **TroopWeight "elite-tax" limit deflation** (2026-07-11 count→limit rework — `ITroopWeightService.ApplyPartySizeWeightPenalty` subtracts the weight surplus so heavy troops fill the cap at 2× while counts read raw; replaces the removed count-getter weighting) |
...
**Always use `taom-src` first.** It runs `ilspycmd` against the installed v1.4.6 DLLs and caches under `~/.taom-src/v1.4.6/` (the script auto-detects the version from `Version.xml`, so older `~/.taom-src/v1.4.5/` + `v1.3.15/` caches remain on disk but are unused). The dump at `E:\Decompiled_Bannerlord\` is v1.4.6 (regenerated 2026-07-02, matching installed) and fine for browsing namespaces/patterns; for authoritative method signatures still prefer `taom-src` against the installed DLLs (the dump can lag after a future engine bump).
...
| `check-doc-config-drift.sh` | PreToolUse (Bash) | Hard-blocks `git commit` when a staged `docs/features/*.md`, `Main/_Module/ModuleData/**/*.json`, `.claude/pinned-game-version.txt`, `CLAUDE.md`, or API-snapshot markdown has **config-example drift** (a feature-doc `json` example disagreeing with the shipped config it mirrors) or a **version mismatch** (CLAUDE.md target / snapshot header ≠ the pin), via `tools/lint_docs.py --fail-on-drift`. Enforcement for the v1.4.7-bump doc-drift finding (`docs/reviews/rca-v1.4.7-bump-2026-07-08.md`). Fail-open: no python / linter crash / nothing relevant staged never blocks. |
...
- `E:\Decompiled_Bannerlord\` holds the v1.4.7 dump (category tree + dual-build both regenerated 2026-07-08 via `tools/decompile_to_folder.ps1` + `tools/decompile_bannerlord.ps1`; v1.4.6 + v1.4.5 baselines kept at `_categories_v1.4.6\`/`_shipping_build_v1.4.6\` + `_categories_v1.4.5\` + `_shipping_build_v1.4.5\`) — **the category tree is the SHIPPING-CLIENT decompile (strips editor code).** For editor-only code use the dual-build decompile `E:\Decompiled_Bannerlord\{_shipping_build,_editor_build}\` (see the ⚠️ note under "Decompiled source layout" + [docs/reference/bannerlord-engine-and-toolchain.md](docs/reference/bannerlord-engine-and-toolchain.md)). Browse for patterns; `ilspycmd` on installed DLLs at `%BANNERLORD_GAME_DIR%\bin\Win64_Shipping_Client\` remains authoritative for signatures.
...
| `tools/rebalance_lords.py` | Balance lord skills (XSLT + XML) via baseline + cultural mod + age. `CULTURE_MAP` is the culture-attr→TAOM map other tools import — **battania=khand** (NOT mirkwood; fixed 12b06e47 after Variag lords wore elven mods) | `--dry-run`, `--apply`, `--export-csv` |
| `tools/apply_culture_skills_traits.py` | **The lord SkillSet generator** (source of truth for `taom_lord_skill_sets.xml` — never hand-edit the XML). `BASE_ARCHETYPES` (74 archetypes incl. per-culture balance variants `north_orc_*`/`dunland_*`/`gondor_*`/`dale_*`/`rohan_*`) + per-culture `CULTURES` dicts with canonical overrides and `archetype_alias` (6 cultures). **Pre-flight before any `--apply`: regen on a clean tree must produce an empty diff** (the 1f7a7a9a drift lesson); per-culture `--culture X --apply` re-resolution is unsafe on drifted cultures — use the repoint script. See `docs/ai-includes/lord-skills-authoring.md`. | `--skillsets-only`, `--culture <key>`, `--all-cultures`, `--apply` |
| `tools/repoint_evil_lord_skillsets.py` | Balance-pass repoint/parity companion (#322/#323/#326): culture-scoped `skill_template` swap maps (base set → per-culture variant), full inline-`<skills>` sync from the sets XML for every managed-culture lord, `TEMPLATE_ASSIGN` for template-less adults. Post-condition + idempotency checked. | `--dry-run` (default), `--apply` |
| `tools/author_elf_lords.py` | One-off new-lord authoring reference (#324 — Lothlórien 10 adults/+2 clans, Rivendell 20): full NPCCharacter+Hero+Faction wiring, inline skills from live SkillSets, donor face keys, clan party-slot math (parties/clan is tier-capped: <3→1, 3-4→2, 5+→3). Pattern to copy for future lord/clan expansions. | `--dry-run` (default), `--apply` |
...
| `tools/generate_starter_armor.py` | Author low-stat career-archetype **starter armor** — chest (Body) + legs (Leg) × 3 archetypes — for the 12 non-Gondor career cultures by cloning each culture's own chest/boots items (mesh/material/cover flags borrowed) and re-setting only the slot stat to the anchors Ranged ~5 / Cavalry ~7 / Infantry ~9; no `value=` → trivial computed resale (`DefaultItemValueModel` is exponential in tier). Writes `<folder>/starter_armors.xml` per culture (auto-loads via the folder's `Items` registration). Gondor excluded (hand-tuned). See `docs/features/starting-equipment-tuning.md`. | `--apply`, `--armory-path` |
| `tools/wire_career_starter_armor.py` | Rewire `taom_career_starting_equipment.xml` so every career roster sets Body + Leg from the matching `starter_*` items and CLEARS Head/Cape/Gloves (starter kit = chest + legs + weapons only), keeping each roster's weapons + mounts. Idempotent. See `docs/features/starting-equipment-tuning.md`. | `--apply` |
| `tools/raise_party_template_maxes.py` | Set `max_value="50"` on every stack of the 8 bandit raider/boss templates + all `kingdom_hero_party_*` templates in `taom_partyTemplates.xml` (boss 1/1 hero stacks kept; never lowers; idempotent; issue #315) | `--dry-run` (default), `--apply` |
| `tools/analyze_settlement_prosperity.py` | **Read-only** starting-prosperity report: LIVE TAOM_Map vs vanilla SandBox per class (towns/castles separately — combined medians mislead, TAOM is 65% castles), per-region flat-cluster flags (89 castles @600, 31 towns @3500), town gold-equilibrium columns (vanilla + TAOM base), zero-village fiefs. Reports to `tools/reports/settlement-prosperity/`. #317 companion; imports the rebaseline module for target truth | `--stdout`, `--cluster-threshold N`, `--game-dir` |
| `tools/rebalance_settlement_prosperity.py` | Rebaseline TAOM_Map starting prosperity onto vanilla's per-class distribution via rank-preserving quantile map — LIFT-ONLY by default, ties de-clustered by bound-village count + hearth, cap 5600, idempotent, byte-round-trip + `.bak` on the LIVE external file (never the repo shadow). Seeds NEW campaigns only (#317; live saves = the C# regen knob) | `--dry-run` (default), `--apply`, `--allow-lower`, `--town-uplift N`, `--pin-zero-village`, `--preserve`, `--game-dir` |
| `tools/author_settlement_buildings.py` | **Source of truth** for per-fief starting building levels (lore + role, balanced). Every one of the 221 towns/castles hand-assigned a role tier + rationale + overrides; a pinned deterministic expander (tier profile + culture flavor + fort-rationing) → per-culture JSONs (`tools/data/settlement_building_levels/*.json`) + the audit doc. Asserts full coverage. See `docs/features/settlement-building-levels.md`. | (no flags) |
| `tools/dump_settlement_buildings.py` | **Read-only** per-fief building-level dump from the LIVE TAOM_Map settlements.xml (the "was" source + before/after verification; writes `current_state.json`). Towns = 12 `building_settlement_*`, castles = 11 `building_castle_*`; range 0–3, fort floors at 1. | `--culture X`, `--all`, `--towns-only`/`--castles-only`, `--json`, `--game-dir` |
| `tools/apply_settlement_buildings.py` | Safe applier of the per-culture building-level JSONs to the LIVE file. **Two-level** regex (unique `<Settlement id>` block → specific `<Building id>`, since building ids repeat), exactly-once assertion, validates range + fort-floor + town/castle id-set, byte-round-trip + feature-named timestamped `.bak`, dry-run default, idempotent. Seeds NEW campaigns only. | `--dry-run` (default), `--apply`, `--culture X`, `--game-dir` |
