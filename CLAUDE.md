...
| **Troop bows are generated `ladder_*` items** | Reach is the bow's `missile_speed`; skill is accuracy, never range. 130 `ladder_<line>_<bow|xbow>_<band>` items live in the unversioned Armory from `tools/ranged_ladders.json`; a reinstall drops them and archers spawn bowless, no error. `generate_ranged_ladder_items.py --verify`; gates `RANGED_LADDER_INVERSION`, `RANGED_MOUNT_USAGE` (no `long_bow` on a rider). `docs/features/ranged-ladders.md` |
...
95 categories mapping a stack trace to its owning feature -> exact target -> status.
