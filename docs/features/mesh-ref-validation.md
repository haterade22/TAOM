...
> **The second confirmed instance (#599, 2026-09-15).** Not a typo this time: the 2026-09-11 Armory art drop re-exported the elven bows as `wm_elven_bow_a01..a04` (bodies `bo_..._a01..a04`) and the item XML kept `_v1..v4`. An elf start carries `starter_highelf_longbow`, the player character is always first in `FightTournamentGame.GetParticipantCharacters`, and `ArenaPreloadView.OnSceneRenderingStarted` spun the game-loop thread on the Rivendell tournament (full dump, WinDbgX `!clrstack`, no TAOM frame) while a human in the same process played it. `--items <Armory>/ModuleData --scan-bodies` named all 18 bodies and 14 meshes; the refs were repointed to the shipping art, no tpac restored. Nothing runs this tool after an Armory sync, and a session had already run it by hand on 2026-09-13 and only wrote the result into a memory note; the lesson is in `docs/reviews/lessons/xslt-moduledata.md`. The composed, committed form of this check (plus the troop join and the catalogue drift) is [armory-ref-audit.md](armory-ref-audit.md); `validate_moduledata.py` now emits the same Tier C finding as `MISSING_COLLISION_BODY` so the commit hook sees it.
...
| **B** (offline, visual meshes) | Union of Metamesh names across the `.tpac` tables-of-contents | `MISSING_MESH` (ERR), `KNOWN_DEAD_MESH` (WARN, allowlisted), `STALE_DEAD_MESH_ALLOWLIST` (WARN), `UNVERIFIED_MESH` (WARN, degraded), `UNPARSED_TPAC` (WARN) | Reliable offline for *visual* meshes |
...
- `.tpac` present-set: `<game>\Modules\{LOTRLOME_Armory,Native,SandBoxCore}\AssetPackages\*.tpac` (`--game`, `--tpac-modules`). Shared meshes live outside the Armory, so the set is the union. A named module shipping no cooked packs falls back to its `Assets/**/*.tpac` (see Known Scope)
...
- **The scan root is the Armory only, and that is correct, not a gap.** Measured 2026-09-01:
  `Main/_Module/ModuleData` declares **0** visual-mesh and **0** collision-body references, so
  nothing in TAOM's own data goes unvalidated by this scoping. Re-measure before widening `--items`;
  the #352 lesson was that scope is what this tool gets wrong, so the negative is worth keeping current.
- **Corrected 2026-09-26: it resolves against a module's loose `Assets/**/*.tpac` whenever the module has one, and against cooked `AssetPackages/*.tpac` only for a module with no loose tree (Native).** That is the engine's own order: it loads `Assets/` in preference to a cooked tree the module also ships ([armory-guide.md](../reference/armory-guide.md) "Two asset trees"). The Armory gained a cooked tree at 07:36 that day, and the tool, which then preferred packs wherever they existed, read the stale packs and reported the hill troll hammer, imported at 11:39, as `MISSING_COLLISION_BODY`. `validate_moduledata.py` and `audit_armory_refs.py` call `tpac_paths_for_modules` rather than choosing a tree themselves. A named module that contributes no `.tpac` at all gets a WARNING naming it, since every mesh it owns would read as missing. The 2026-09-01 correction came first and closed a hole that had been recorded once and not acted on: [black-numenorean.md](./black-numenorean.md) noted that the glob "has never seen an Armory body", and 744 collision-body refs were being checked against a directory that did not exist. A documented blind spot with no owner stays open. Without a loose-tree read the default module list resolved to Native alone and reported thousands of false `MISSING_MESH`.
- **A `PASS` is clean against the tree the game loads, with one gap.** The engine skips a package that has no RuntimeDataCache `.rdc` entry, and this tool still counts that package's meshes; `tools/check_rdc_entries.py` covers it. Before 2026-09-26, where a cooked tree existed, the present-set came from the packs, which are rebuilt only on a re-cook, so a clean run could sit on art already deleted from `Assets/`: on 2026-08-28 one sat on top of 179 deleted meshes ([armoury-mesh-cleanup.md](./armoury-mesh-cleanup.md)). To diff the two trees, use `tools/audit_deleted_mesh_impact.py`, which compares `AssetPackages/*.tpac` against `Assets/**/*.tpac` and joins the difference through to items and troops.
- **Its "missing" set is narrower than "imported but not cooked".** This tool walks item XML, so it reports only meshes an item actually names. A mesh imported into `Assets/` that no item references yet is invisible to it. On 2026-08-28 that made the two counts 10 here and 11 in the impact audit, the extra being `gondor_horse_plate_armor`. The difference is the expected shape of the two questions, not a discrepancy.
...
- 2026-08-28: **Blind spot found, and a sibling tool built for it.** An asset cleanup deleted 179 meshes and this tool returned `PASS`, because its present-set comes from the cooked `AssetPackages/*.tpac` and those had not been re-cooked since the deletions. Nothing changed in this tool: the limitation is inherent to resolving against packs, and it is now stated in Known Scope. `tools/audit_deleted_mesh_impact.py` answers the other question by diffing packs against `Assets/**/*.tpac` and joining gone mesh to item to troop. Two facts worth carrying: the exit code IS meaningful (`return 1` on any ERROR, so it gates a build; a `$?` reading 0 after piping through `tail`/`grep` is the pipeline's status, not Python's), and its missing-set is deliberately narrower than "imported but not cooked" because it only walks item XML. Full record: [armoury-mesh-cleanup.md](./armoury-mesh-cleanup.md).
...
- [docs/features/armoury-mesh-cleanup.md](./armoury-mesh-cleanup.md)
