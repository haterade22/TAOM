...
- **Corrected 2026-09-01: for a module with NO cooked tree it resolves against `Assets/**` instead, and says so.** This closes a hole that had already been recorded once and not acted on: [black-numenorean.md](./black-numenorean.md) noted that the glob "has never seen an Armory body", and 744 collision-body refs were being checked against a directory that does not exist. A documented blind spot with no owner stays open. `tpac_paths_for_modules` prefers `AssetPackages/*.tpac` and falls back to that module's loose `Assets/**/*.tpac`, emitting a WARNING naming the module. `LOTRLOME_Armory` has been in that state since v2.0.23 (0 cooked packs against 4,364 loose), as have `SandBoxCore` and `SandBox`. For the Armory the stale-pack blind spot below is therefore currently **unreachable**, and a `PASS` does mean clean. Without the fallback the default module list resolved to Native alone and reported thousands of false `MISSING_MESH`. The paragraph below still holds for any module that does ship cooked packs.
- **Where a cooked tree DOES exist, it resolves against the COOKED packs, so it cannot see art deleted after the last cook.** The present-set is built from `<module>/AssetPackages/*.tpac`, which are rebuilt only when someone re-cooks them. Delete a mesh from `Assets/` or `AssetSources/` and the packs keep shipping it, so this tool returns `PASS` while the authoring tree is already broken; the breakage lands at the next re-cook. Confirmed 2026-08-28: a clean run here sat on top of 179 already-deleted meshes ([armoury-mesh-cleanup.md](./armoury-mesh-cleanup.md)). **A `PASS` is "clean against the last cook", not "clean".** To compare the two trees, use `tools/audit_deleted_mesh_impact.py`, which diffs `AssetPackages/*.tpac` against `Assets/**/*.tpac` and joins the difference through to items and troops.
