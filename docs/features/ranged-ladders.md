...
by the inventory tooltips). One coupling to know before ever flipping `is_merchandise` back on:
...
| `docs/reference/ranged-troops.html` | The tracked HTML: every ranged troop per kingdom, skills beside weapon speed, reach, accuracy, spread, cadence, damage; regenerated by the tool, never hand-edited |
