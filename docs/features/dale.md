...
- 2026-09-04 (#541): The bronze armour ladder was generated on the wrong axis. `generate_dale_armor.py` `VARIANT_TIER` reads `a01..a04, b01..b04` as one eight-step tier ladder while `generate_dale_troops.py` uses `a`/`b` as the bronze/silver LINES and `01..04` as the tiers, so infantry `a01` and `a02` carried identical stats (the L21 Guardsman had less armour than the L16 Footman) and archer `a02` sat above `a03` (Longbowman to Royal Archer dropped 21). Roster-side first: `dale_squire` now wears infantry `a01` (same stats it had), `dale_militia_veteran_archer` and `dale_militia_veteran_spearman` wear the silver `b01`, because the roster-tiered restat anchors an item to its lowest wearer and those three pinned `a02`/`a03` low. Then `rebalance_armor.py --cultures dale --tier-source roster --no-lower-armor --keep-materials` raised infantry `a02`, infantry `a03` and archer `a03` to the heavy row on both Armory copies; nothing lowered. Note the tier labels in this doc run one above the engine's: `T7` here is level 31, engine tier 6 (`ceil((level - 5) / 5)`). Dale still caps at L31 by decision; Rohan reaches L41 and Gondor L51.
