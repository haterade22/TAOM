...
| Clan-screen party row | `ClanPartyItemWithPartyVM.UpdateProperties` and `ClanPartyItemWithHeroVM.UpdateProperties` (the `ClanPartyItemVM` base went abstract at v1.5.0; one patch, two `TargetMethods`) | `PartySizeText` + the subtitle rebuilt from it |
