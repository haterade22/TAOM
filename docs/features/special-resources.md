...
giving any troop a `recruit_cost` gates + charges it with no code change. The trolls (`cave_troll`, `hill_troll`, 2026-09-25) carry `recruit_cost` 50 and `daily_upkeep` 5 and 4 but are in no volunteer pool and no tavern, so the charge lands only when the player recruits a captured troll, the ungated prisoner path; AI lords recruit prisoners through `OnTroopRecruited` and pay nothing. A Free-culture player pays in their own resource and then loses the troll to alignment desertion on the next daily tick (a known limitation, not yet decided).
