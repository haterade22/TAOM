...
| 6 | Native2Managed allowlist (`Native2ManagedTargets.All`, 16 shims): in `EngineCallbacksGenerated`, `EngineScreenManager_PreTick`, `EngineScreenManager_LateTick`, `EngineScreenManager_Update`, `ManagedScriptHolder_TickComponents`, `ThumbnailCreatorView_OnThumbnailRenderComplete`, `RenderTargetComponent_OnPaintNeeded`; in `CoreCallbacksGenerated`, the ten mission combat callbacks `Mission_MeleeHitCallback`, `Mission_MissileHitCallback`, `Mission_ChargeDamageCallback`, `Mission_FallDamageCallback`, `Mission_MissileAreaDamageCallback`, `Mission_OnAgentHitBlocked`, `Mission_GetDefendCollisionResults`, `Mission_OnAgentRemoved`, `Mission_OnAgentDeleted`, `Mission_OnAgentShootMissile` | Native-to-managed callback shims whose managed work no row above is known to wrap; attached by hand at startup. The combat entries' traces are under Risks, "Mission combat callbacks" |
...
**Off-main-thread captures (#650).** Native decides which thread raises a callback shim, and several combat callbacks arrive off the main thread (`harmony-patches.md`, "Which thread runs your target"). `AppDomainExceptionHook.Subscribe()`, called from `SubModule.OnSubModuleLoad` on the main thread before the bridge is attached, records that thread's managed id, logs it (`[CrashReport] main thread id N recorded at Subscribe()`) and exposes it as the internal `AppDomainExceptionHook.MainThreadId`. When the bridge captures (Enable Native-to-Managed Capture on) on any other thread, it passes `offMainThread: true` through `HandleAndSwallow` to `ICrashReportService.HandleException`, as the AppDomain hook does for its own captures; both use `AppDomainExceptionHook.IsOffMainThread`. The verdict is a parameter rather than a mark in `Exception.Data`, because a read-only `Data` (the runtime's preallocated out-of-memory and stack-overflow exceptions return one) would drop the mark and send a worker-thread capture down the full path. `CrashReportService` then skips the Campaign and Mission collectors (recorded as `OffMainThreadSkipped` collector failures) and the on-screen inquiry, and logs `[CrashReport] off-main-thread capture; UI inquiry skipped`. An id of 0 (the hook never subscribed) also counts as off-main: the safe direction, since a main-thread capture then loses only those two sections and the inquiry. `MissionThreadGuard` is not the reference because it learns its thread only at the first mission tick and reports every thread as main before that. The remaining collectors (modules, assemblies, Harmony, TAOM state, MCM, process, GPU, logs) still run on the capturing thread, as they already did for an AppDomain capture; for a bridge capture the game keeps running meanwhile. Whether the thread that runs `OnSubModuleLoad` is the thread that later runs the game loop is UNVERIFIED: if it is not, every bridge capture takes the reduced path. The logged id, compared with a `MissionThreadGuard` "main N" line from the same session, settles it.
...
|---|---|---|---|
| Master | Enable Crash Capture | true | Master toggle. When off, the AppDomain hook does nothing, and every Patch37 finalizer hands exceptions back uncaptured, with the throw site recorded (`RethrowStackPreserver`), so Harmony's rethrow keeps it with or without PatchShield. A later finalizer on the same method, such as ButterLib's BetterExceptionWindow finalizer on four of the Patch37 targets, then sees the exception without its live frames. Live: the patches are always installed. |
...
- `Native2ManagedTargetsTests`: every allowlisted shim resolves against the installed engine (`BindingVerification`); the list is exactly the reviewed shims, pinned by literal name (the assertion also rejects a duplicate or an extra entry); a missing assembly, type or method, or a lookup that throws, is reported and skipped
- `Native2ManagedBridgeTests`: with the toggle off the bridge hands back the same exception with `TAOM.ThrowSite` recorded; with the toggle on and the service unreachable its hand-back still names the throw site after a `throw` of the same object (what Harmony's wrapper does); with a reachable service (`RecordingCrashService`) a capture is swallowed with the callback origin, and the service is told it is off-main on a worker thread (also when the exception's `Data` is read-only) or with no recorded main thread, and not on the recorded main thread; `Finalizer` compares against the id the hook recorded (not off-main on the subscribing thread, off-main on a worker); with the toggle off on a worker thread the service is never called; with no exception it returns null
- `AppDomainExceptionHookTests`: `Subscribe()` records the subscribing thread as `MainThreadId`; `IsOffMainThread` is false only on the recorded thread, and true for an unset id; `OnUnhandled` passes the service `offMainThread: false` on the subscribing thread and `true` on a worker
- `CrashReportPatchHelperTests`: `HandleAndSwallow` with the service unreachable hands back the same instance, which keeps its throw site across that `throw`; with a reachable service it swallows and passes the origin; when the service throws it hands back the original with its throw site; re-entered from inside the service it hands the inner exception back without capturing it; with no exception it returns null
...
- **Boot cost: one `harmony.Patch` per target, plus one PatchShield attach per Native2Managed target at the first game start** (PatchShield's pass 2 shields every patched method not declared in a TAOM assembly, outside its namespace exclusions, and `ManagedCallbacks` is not excluded). Each attach cost about 120 to 190 ms on the maintainer's desktop on 2026-09-23 (the old sweep of all 247 `*CallbacksGenerated` methods cost 29 to 33 s on 30 of 30 launches, and PatchShield timed 186 ms per attach in the same process). On 11 player processes the same 247-method sweep took 0 to 1 s, under about 8 ms per attach ([followup-patch-tax.md](../../plans/_audit/2026-09-23-opus/followup-patch-tax.md)), so the saving is large on that desktop and about a second for players. The 16-entry allowlist drops 231 attaches at boot and about as many PatchShield attaches at the first game start; the ten combat entries added on 2026-09-24 (#650) have no measured attach time yet. `[CrashReport] Native2Managed: attached N of M Finalizer(s) in X ms` in `taom_debug.log` shows the current cost.
- **Steady-state cost: small, not zero.** Harmony runs a finalizer on every call of its target (with a null `__exception` on success, which returns at once), and the patched method becomes a replacement wrapped in try/catch. The combat callbacks run once per hit, block, missile or agent removal; that cost is unmeasured. Until PatchShield excludes `ManagedCallbacks` (plan 007), its pass 2 also puts a finalizer that binds `__originalMethod` on every shim, so Harmony's wrapper calls `MethodBase.GetMethodFromHandle` (a reflection-cache lookup that allocates and takes a lock) on every call, per hit for the combat entries ([lessons/harmony-il.md](../reviews/lessons/harmony-il.md), #331).
...
- **Other mods' `OnSubModuleLoad` throws are not captured.** TAOM applies Patch37 inside its own `OnSubModuleLoad`, and a finalizer on the base `MBSubModuleBase.OnSubModuleLoad` would never see an override's throw, so none is attached; those throws land in vanilla or BUTR. TAOM applies its own categories through `TryPatchCategory`, so a binding that no longer resolves logs a `[PatchApply]` error and stops that one category at its failing class instead of failing the load (a class whose attribute names a type the engine no longer has is skipped and reported on its own: `PatchCategoryIndex` indexes the categories class by class, where Harmony's own index would fail every category, #653). Patch37 is still registered first so its tick finalizers are live as early as possible.
- **Most native-to-managed callbacks are no longer wrapped.** The 2026-09-24 allowlist keeps 16 of the 247 shims. A managed throw inside any of the other 231 (for example `Agent_OnDismount`, `Agent_OnMount` or `Agent_UpdateAgentStats` in `CoreCallbacksGenerated`) is caught only if it unwinds through native code into a Patch37 finalizer such as `Mission.Tick`; whether it can is UNVERIFIED, so such a throw may now reach vanilla's handler, or crash to desktop with no bundle. The sixth entry is the tableau render callback `RenderTargetComponent_OnPaintNeeded`, which raises `RenderTargetComponent.PaintNeeded`, the render function each `TableauView.AddTableau` caller registers (`CharacterTableau`, `BasicCharacterTableau`, `ItemTableau`, `BannerTableau` and `BrightnessDemoTableau` in `TaleWorlds.MountAndBlade.View`, `MapConversationTableau` in `SandBox.View`, v1.5.3). It replaced `BannerlordTableauManager_RequestCharacterTableauSetup` on 2026-09-24 (#650), because nothing in v1.5.3 assigns that one's `RequestCallback`. Which thread native runs the tableau and thumbnail callbacks on is UNVERIFIED; a capture there off the main thread takes the reduced path like any other bridge capture.
- **Mission combat callbacks (on the allowlist since 2026-09-24, #650).** The maintainer put the ten combat callbacks below back on the allowlist once the bridge told the service about its off-main-thread captures (see "Off-main-thread captures" under Catch points). The trace (installed v1.5.3) finds a path into TAOM code for each. Several can arrive off the main thread (`harmony-patches.md`, "Which thread runs your target"), and such a capture writes a bundle without its Mission and Campaign sections and shows no inquiry. A capture swallows the throw, so native gets the callback's default result (for example `false` from `Mission_MissileHitCallback`, 0 from `Mission_OnAgentHitBlocked`), as it did under the 247-shim sweep before 2026-09-24. Swallowing is not recovery: the rest of the managed method is abandoned. A throw in one behaviour's `OnAgentRemoved` skips the later behaviours, `_activeAgents.Remove` and `Agent.OnRemove` (`Mission.cs:3019-3040`), and a throw in `OnAgentDeleted` skips `_allAgents.Remove` and `Agent.OnDelete`, while the game continues. Native also reads back every `ref` and `out` argument as the managed side left it: `Mission.MissileHitCallback` writes `out extraHitParticleIndex` only after `AgentApplyDamageModel.DecideMissileWeaponFlags` (`Mission.cs:5789-5790`), which `TaomCombatMechanicsModel` overrides, and the hit callbacks pass their collision data by `ref`. What native does with such values is UNVERIFIED.
...
- 2026-09-24: **Maintainer decisions** (#650). The sixth allowlist entry is now `RenderTargetComponent_OnPaintNeeded`, the tableau render callback, in place of the never-armed `BannerlordTableauManager_RequestCharacterTableauSetup`. Every exception the bridge or `CrashReportPatchHelper.HandleAndSwallow` hands back now goes through `RethrowStackPreserver.PreserveForRethrow`, so Harmony's rethrow keeps the throw site. The bridge stays at Harmony priority 400, the suppression log keeps its powers-of-ten cadence with no time floor, and capture stays on by default with Enable Native-to-Managed Capture as its toggle.
- 2026-09-24: **Combat callbacks and off-thread captures** (#650). The bridge marks a capture made on any thread other than the one `AppDomainExceptionHook.Subscribe()` recorded at `OnSubModuleLoad` (or with no recorded thread) as off-main, so the service takes its reduced path. The ten mission combat callbacks with a traced path into TAOM code are back on the allowlist, now 16 entries; mount, dismount and alarmed-state stay out.
- 2026-09-24: **Review follow-ups** (#650). The off-main verdict now reaches the service as a `HandleException` parameter from both the bridge and the AppDomain hook, instead of a mark in `Exception.Data` that a read-only `Data` silently dropped; the hook logs the main thread id it records. The Enable Native-to-Managed Capture hint names the tableau and combat callbacks.
...
- **Issue:** #650 [Crash capture: 247 callback patches per boot, dead toggles, 4 finalizers that never fire](https://github.com/haterade22/TAOM/issues/650)
- **Status:** Open. Owed in game: the boot time and the `attached 16 of 16` line (the combat entries' attach time is unmeasured), an off-main-thread capture's bundle (a QA throw in a TAOM `OnAgentRemoved` override, which reaches the bridge through `Mission_OnAgentRemoved`, often off the main thread), the `main thread id N recorded` line against a `MissionThreadGuard` "main N" line from the same session, live MCM toggling, a large-battle frame-time check of the per-hit cost, and the probe for a throw in a dropped callback, raised by native through a shim that is not on the list (for example a behaviour's `OnAgentDismount`, through `Agent_OnDismount`, when a mount dies under its rider)
