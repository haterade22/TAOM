...
- **No MCM settings parity: reported, not yet exchanged.** TAOM ships **238** settings across
  four MCM classes (the 284 here counted `[SettingPropertyGroup]` lines alongside the properties;
  the split is 224 in `TaomSettings`, 7 in `BattleLoadDiagnosticsSettings`, 6 in
  `CrashReportSettings` and 1 in `BlowDiagnosticsSettings`).
  **179 are simulation-relevant**, traced to the feature that consumes each one and kept when that feature
  ships a GameModel, CampaignBehavior, MissionBehavior or Harmony patch; all 179 are in
  ...
  short strings instead of 179 values read off two screens.
  ...
  **A matching fingerprint means the MCM settings agree, and nothing wider.** It covers the 175
  ...
  or any difference outside TAOM. None of those three flags gates a setting in the 179 (each
