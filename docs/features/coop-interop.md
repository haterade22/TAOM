...
- **No MCM settings parity: reported, not yet exchanged.** TAOM ships **199** settings across
  four MCM classes (the 284 here counted `[SettingPropertyGroup]` lines alongside the properties;
  the split is 185 in `TaomSettings`, 7 in `BattleLoadDiagnosticsSettings`, 6 in
  `CrashReportSettings` and 1 in `BlowDiagnosticsSettings`).
  **141 are simulation-relevant**, traced to the feature that consumes each one and kept when that feature
  ships a GameModel, CampaignBehavior, MissionBehavior or Harmony patch; all 141 are in
  ...
  `SettingsFingerprint` hashes those 141: one code per MCM group plus a global, culture-invariant
  ...
  short strings instead of 141 values read off two screens.
  ...
  **A matching fingerprint means the MCM settings agree, and nothing wider.** It covers the 141
  ...
  or any difference outside TAOM. None of those three flags gates a setting in the 141 (each
