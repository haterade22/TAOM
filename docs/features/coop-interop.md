...
- **No MCM settings parity: reported, not yet exchanged.** TAOM ships **227** settings across
  ...
  **168 are simulation-relevant**, traced to the feature that consumes each one and kept when that feature
  ships a GameModel, CampaignBehavior, MissionBehavior or Harmony patch; all 168 are in
  ...
  `SettingsFingerprint` hashes those 168: one code per MCM group plus a global, culture-invariant
  ...
  short strings instead of 168 values read off two screens.
  ...
  **A matching fingerprint means the MCM settings agree, and nothing wider.** It covers the 168
  ...
  or any difference outside TAOM. None of those three flags gates a setting in the 168 (each
