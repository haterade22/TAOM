...
- **No MCM settings parity: reported, not yet exchanged.** TAOM ships **239** settings across
  four MCM classes (the 284 here counted `[SettingPropertyGroup]` lines alongside the properties;
  the split is 225 in `TaomSettings`, 7 in `BattleLoadDiagnosticsSettings`, 6 in
  `CrashReportSettings` and 1 in `BlowDiagnosticsSettings`).
  **180 are simulation-relevant**, traced to the feature that consumes each one and kept when that feature
  ships a GameModel, CampaignBehavior, MissionBehavior or Harmony patch; all 180 are in
  ...
  short strings instead of 180 values read off two screens.
  ...
  **A matching fingerprint means the MCM settings agree, and nothing wider.** It covers the 175
  ...
  or any difference outside TAOM. None of those three flags gates a setting in the 180 (each
