...
- **No MCM settings parity: reported, not yet exchanged.** TAOM ships **255** settings across
  four MCM classes (the 284 here counted `[SettingPropertyGroup]` lines alongside the properties;
  the split is 240 in `TaomSettings`, 8 in `BattleLoadDiagnosticsSettings`, 6 in
  `CrashReportSettings` and 1 in `BlowDiagnosticsSettings`).
  **189 are simulation-relevant**, traced to the feature that consumes each one and kept when that feature
  ships a GameModel, CampaignBehavior, MissionBehavior or Harmony patch; all 189 are in
  `TaomSettings`. The 66 excluded (counted 2026-09-19 from the lists below) are instrumentation, player-local inventory convenience,
  ...
  `SettingsFingerprint` hashes those 175: one code per MCM group plus a global, culture-invariant
  ...
  short strings instead of 183 values read off two screens.
  ...
  **A matching fingerprint means the MCM settings agree, and nothing wider.** It covers the 175
  ...
  or any difference outside TAOM. None of those three flags gates a setting in the 183 (each
...
- [docs/features/fief-granting.md](./fief-granting.md)
