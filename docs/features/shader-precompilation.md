...
> **STATUS: ACTIVE again since 2026-09-11 ([#560](https://github.com/haterade22/TAOM/issues/560)), after the 2026-08-20 park.** The main-menu option is registered, `Patch21_ShaderPrecompilation` is applied, and both MCM toggles are visible. What changed on the way back: the walk now runs the roster in **batches** (every loaded character exactly once, 1,000 per custom battle) instead of one capped battle that silently dropped TAOM's own troops; each battle takes the vanilla shape (a one-character player party, the batch on the enemy side) so the engine sets the player agent itself and deployment auto-finishes; scene passes are **off by default for every install** through a renamed MCM property (`EnableShaderPrecompileScenePasses`); and the shipped text now says to re-run after any mod-list change, which is what 1.4.8's cache deletion requires. A five-agent deep review and a Codex adversarial pass (GPT-6-Astra, ultra) both ran before the first in-game run and drove the teardown, ownership and wording fixes recorded under "Roster batching". **The cold in-game walk on 1.4.8 is owed** (see "Verification owed" below).

> **Park history.** Parked 2026-08-20 as "no longer needed" (three commented blocks: the `AddInitialStateOption` block in `SubModule.cs`, the Patch21 trio near the top of `OnSubModuleLoad`, and the two MCM attributes in `TaomSettings.cs`). Player bundle b18f3441 (TAOM v2.0.26, Bannerlord 1.4.8, 2026-09-04) contradicted the rationale: a 95-agent field battle sat 305 s at `SceneView.ReadyToRender()` while the engine compiled cold character shaders serially (818 `compile_shader` lines in the engine log; 478 `Missing shader from sack` misses in the 14-second load window, 430 of them `pbr_metallic`). That is the cost the character pass prepays; see [battle-load-diagnostics.md](battle-load-diagnostics.md). The MCM master toggle could never have done the park on its own: `TaomSettings` persists as json2, so every existing install had `EnableShaderPrecompilation = true` on disk.
...
| `Main/SubModule.cs` | Applies `Patch21_ShaderPrecompilation`, calls `InitializeHooks`, registers the menu button + inquiry in `OnBeforeInitialModuleScreenSetAsRoot()`, adds the guard in `OnMissionBehaviorInitialize` while a walk runs |
...
`TAOM.Tests/Migration/ReflectionSiteBindingTests.cs` pins `Mission._initialPlayerAgent` against the installed engine; `SettingsFingerprintTests` pins the settable-property count the rename must not move; `SettingRequireRestartPostureTests` requires `RequireRestart = false` on both restored attributes.
...
- **LoadingScreen patch:** Runs every frame during loading screens; returns after one null check unless a walk is active, then copies the runner's status line into `DescriptionText`.
...
- [#560: Re-enable shader pre-compilation on 1.4.8: batch the roster, scene passes opt-in](https://github.com/haterade22/TAOM/issues/560): 2026-09-11, OPEN until the cold in-game walk
