...
- **`PrecompileSceneProvider`** reads the scene list (below) and falls back to a baked default (the 21 TAOM `_forceatmo` scenes — 8 open-field battles + 9 custom siege settlement scenes + 4 custom village scenes, incl. the #287 crash scene `taom_mordor_battle_003_forceatmo`).
...
| `Main/_Module/ModuleData/shader_precompilation/precompile_scenes.txt` | Editable scene list (defaults to the 21 TAOM `_forceatmo` scenes — 8 battle + 9 siege + 4 village) |
