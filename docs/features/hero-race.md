...
**The Nine keep their XML race (#644, 2026-09-23).** `RestoreHeroRaces` skips every hero `INazgulRegistry.IsWraith` names, on both the legend and the legacy path, and logs `kept the XML race for N Ringwraiths (#644)`. #644 made the Nine `race="nazghul"` in `lords.xslt`; without the skip every save written before it would put back the captured human (six of them) or uruk (three), and the change would reach new campaigns only. The capture still records them, and the capture that follows the restore rewrites the map with nazghul, so the next save is clean. The skip overrides three runtime writers, deliberately: the player's face editor (`CharacterObject.UpdatePlayerCharacterBodyProperties`) and character import (the `campaign.import_main_hero` cheat, v1.5.3 `CharacterData.cs:186`) on a Player Switcher wraith, and co-op join reconciliation (`JoinReconciliationService.ApplyRace`) on a wraith a joining client is handed. A wraith stays a wraith (Mike, 2026-09-23). The other `SetHeroRace` callers, character creation and named companions, never reach one of the Nine.

**Session launch restores THEN captures (2026-08-03), and the order is load-bearing.** Capture used to run only on `OnBeforeSaveEvent`. A multiplayer host handing its world to a joining client never raises that event, so a joiner received a world with no race data at all — the map was empty at serialization time. `RacePersistenceBehavior.OnSessionLaunched` therefore calls `RestoreHeroRaces()` and then `CaptureHeroRaces()`. Capturing first would snapshot every hero at whatever race the raw XML gave them and write that over the good map the restore is about to apply. Both orderings are pinned: `OnSessionLaunched_RestoresThenCaptures` and `OnSessionLaunched_DoesNotCaptureBeforeRestoring`.
...
RacePersistenceBehavior (CampaignBehaviorBase)
    ...
    |       refuses when the table has < 2 races (keeps the existing map + legend)
    |-> OnSessionLaunched -> RestoreHeroRaces() THEN CaptureHeroRaces()   [order is load-bearing]
    ...
    |       the trailing capture covers save transfers that never raise OnBeforeSaveEvent
...
| `Main/Features/HeroRace/RacePersistenceService.cs` | Captures and restores hero races across save/load: ints + a `;`-joined race-name legend so restore is robust to skins.xml merge-order shifts (#330). Refuses to capture against a table of fewer than 2 races, which is what a TAOM-less co-op host looks like. Leaves the Nine's XML race alone on restore (#644) |
...
- `INazgulRegistry` (NazgulFamily): names the Nine, whose XML race the restore keeps (#644)
...
- `RacePersistenceServiceTests.cs`: verifies that `CaptureHeroRaces` stores ALL heroes including humans (#130 P2) plus the ordered race-name legend, that `RestoreHeroRaces` translates saved ints through the legend to CURRENT ids (shifted-index, removed-race skip+warn, out-of-range skip, translated-id-equal no-op) and falls back to the legacy raw-int path when the legend is absent, that `SyncRaceData` clears stale state on load and syncs both keys, and the full save→load round-trip under a shifted load-side mapping. Five further cases pin the degenerate-table refusal: `CaptureHeroRaces_OneRaceLegend_DoesNotOverwriteRicherCapturedData`, `_OneRaceLegend_ThenRestore_DoesNotMassHumanizeHeroes`, `_OneRaceLegend_LogsWarning`, `_OneRaceLegend_EmptyPriorState_StaysEmpty`, and `CaptureHeroRaces_EmptyRaceList_DoesNotOverwriteRicherCapturedData`. #644 adds four: a wraith keeps its XML race on the legend and the legacy path, a non-wraith beside it is still restored, and the kept count is logged.
- `RacePersistenceBehaviorTests.cs` — verifies event registration bindings, plus the session-launch ordering (`OnSessionLaunched_RestoresThenCaptures`, `OnSessionLaunched_DoesNotCaptureBeforeRestoring`).
...
## Changelog
- 2026-09-23: #644, the Nine are race nazghul, and `RestoreHeroRaces` leaves their XML race alone so existing saves adopt it (section above).
...
## GitHub Issue
- **Issue:** original unknown; #644 (2026-09-23) added the Nine's restore rule
- **Status:** #644 open (in-game smoke owed)
