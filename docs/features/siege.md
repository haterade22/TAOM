...
4. Otherwise, with neither set of frames, it sets `__result` to `settlement.GatePosition` and returns `false` to skip the original entirely.
...
- 2026-09-24: the folder is null-clean and its `.editorconfig` sets the nullable warnings to errors (plan 019); explicit no-settlement branch in the prefix.
