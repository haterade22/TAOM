...
| Skeleton (loose, standalone) | `Assets/creature/spider/meshes/spider_skeleton_geo.tpac` (4.5KB) | The 62-bone `spider_skeleton` Skeleton resource (SkeletonDefinitionData + SkeletonUserData). **Added 2026-06-13** after the user's Blender-loop mesh re-export shipped `spider_correct_geo.tpac` mesh-only — dropping the skeleton → `CreateAgentSkeleton` null → riderless spiders. Extracted bit-identical from `meshes/sk_spider_forest_c_geo.tpac.backup` (item [2]) via `tools/tpac_skeleton_extract.py` into a skeleton-only tpac (no mesh-duplicate vs the split mesh). Mirrors how the working elephant carries `elephant_skeleton` in its live loose `adod_elephant_geo.tpac`. The action_set's `skeleton="spider_skeleton"` resolves here. *(In-game spawn-with-rider verification owed.)* |
| Meshes | `Assets/creature/spider/animations/spider_correct_geo.tpac` (2.78MB, split — 2026-06-13 rework) | `sk_spider_forest_c` / `_c_2` L/R-split meshes (≤38 bones/half; the unsplit 58-bone mesh AVs `PreloadForRendering`). Mesh-only post-rework (skeleton now lives in `spider_skeleton_geo.tpac`, above). Physics transplanted. Backup of the un-split a/b/c original: `meshes/sk_spider_forest_c_geo.tpac.backup` (also holds the skeleton, the fix's source) |
