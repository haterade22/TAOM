...
- **Ability effects:** `CareerAbilityEffectRegistry` dispatches to per-career `ICareerAbilityEffectExecutor` implementations. 3 role-based archetypes (Infantry/Ranged/Cavalry) serve all 50 careers with XML-tunable values via `taom_ability_tuning.xml`. All three archetypes apply AoE friendly-troop buffs within a 50-unit radius (standardized in templates): Infantry (damage + damage reduction), Ranged (speed + ranged damage + draw speed), Cavalry (mount speed + charge damage + damage). Buffs applied via `CareerAbilityBuffTracker` with separate hero and ally buff dictionaries (read by `TaomAgentStatCalculateModel`, so they survive a stat recalc). The two mount fields of the Cavalry buff (`MountSpeedBonus`, `ChargeDamageBonus`) apply on the MOUNT's driven properties through the rider's ids (`ApplyMountStatModifiers`, #611), because the base models write `MountSpeed` and `MountChargeDamage` only in their horse branch and the engine reads them off the horse; the buff apply and restore paths refresh `MountAgent` beside the human so the horse recomputes at once. Before 2026-09-17 both fields multiplied the rider's copies (default 0) and did nothing. Known limitation: the bonus is keyed on the horse's CURRENT rider at recompute time, the way vanilla's own Riding-skill maneuver and speed bonuses on the horse are (`UpdateHorseStats` reads `agent.RiderAgent`); a rider who dies or swaps horses mid-buff leaves the first horse on its last computed numbers until the engine recomputes that horse, and whether native does so on a rider change is not provable from managed code (`OnMount` / `OnDismount` recompute the RIDER). Whatever refresh vanilla's rider bonus gets, this one gets; snapshotting the specific horse in the restore closures was rejected (captured agent handles across frames, the #592 class, for a case bounded by vanilla parity). Smoke item: a buffed cavalry ally remounts or dies mid-buff, then check the abandoned horse's charge in the combat log.
...
**Magnitude scale ↔ application method (IMPORTANT).** `CareerPassiveService` stores the summed `Magnitude` per type (`GetPassiveMagnitude`) plus a per-(type, mask) breakdown (`GetMaskedMagnitude`); each consumer chooses flat vs factor by calling `ApplyFlat` / `ApplyFactor` or applying the magnitude directly. `ApplyFactor` is a plain `result.AddFactor`. **`ApplyFlat` is a RESULT-frame add**, not a plain `result.Add`: it divides the accumulated factor frame back out so an authored count is worth exactly that count (2026-08-18). So a passive's authored magnitude scale MUST match its consumer's method:
...
- **Whole-count magnitude (2 = +2 units)** → `ApplyFlat`. Only **`PartySize`** (`TaomPartySizeModel`) and the agent-stat flat `Health` / `CompanionLimit`. Applying a whole-count via `ApplyFactor` multiplies the base (`AddFactor(2)` = ×3), the "+2 to +150" bug fixed 2026-05-29. A second, quieter version of the same bug lived in `ApplyFlat` itself until 2026-08-18: `ExplainedNumber` resolves as `BaseNumber * (1 + SumOfFactors)`, so a base-frame `Add` is multiplied by every factor anyone else put on the number. With only a culture feat in play that was a rounding error; once AiPartySize (#461) contributed a large factor, a "+4 party size" perk inside a player-owned garrison was worth about +13. `ApplyFlat` now divides the frame out, so the career screen tells the truth. Pinned by four tests in `CareerPassiveServiceTests`.

**attack_type_mask (Damage + Resistance).** `Damage` and `Resistance` honor the mask, which carries two axes since #613: delivery (`Melee`, `Ranged`, `All` for either) and damage kind (`Cut`, `Pierce`, `Blunt`, from the engine's `AttackCollisionData.DamageType` per hit). A pip matches a hit per axis (`AttackTypeMaskMatch.Matches`): its delivery bits, if it names any, must overlap the hit's, and its kind bits, if it names any, must overlap the hit's. So "+5% blunt resistance" (`Blunt`) fires on a mace and on a sling stone, "+10% resistance to melee" (`Melee`) fires on any melee kind, `Melee, Blunt` needs both, and a plain "+5% resistance" (no attribute, `All`) fires on everything, exactly as before. Names are comma or pipe separated, any case; an unknown name or a digit string warns at load and leaves the pip inert (`None`), and `CareerChoicesIntegrationTests` refuses a `None` mask in the shipped XML. Until #613 the enum had only the delivery axis, `ParseEnum` fell back to `All` for the five kind pips, and they resisted every hit. `Damage` is applied multiplicatively on the per-hit **amplification** path (`CareerAgentStatService.CalculateDamageAmplification`), NOT as a flat `DamageMultiplierBonus`, so the mask can gate it.
...
| `Ammo` | 14 | `TaomAgentStatCalculateModel.InitializeMissionEquipment` via `CareerAmmoApplier` | mission | multiplicative refill of every consumable slot (arrows, bolts, thrown) the way vanilla's Deep Quivers / Fletcher / Well Prepared do it: before the agent is built, `MissionEquipment.SetAmountOfSlot(slot, amount, addOverflowToMaxAmount: true)`, which raises the stack's max with the amount. Since #613; before it the refill ran in `OnAgentBuild` through `Agent.SetWeaponAmountInSlot` above an unchanged max (the engine caps pickups at `ModifiedMaxAmount`, so a full stack could not be trusted to take it) and gated on `IsAnyAmmo`, which skips thrown weapons, so the nine pips on the javelineer careers did nothing |
...
| `MountChargeDamage` | 72 | `CareerAgentStatService.ApplyMountStatModifiers` | mission | the MOUNT, keyed on its rider (#611); multiplicative. Until 2026-09-17 it multiplied the RIDER's copy, which no base model writes and the engine never reads, so the pip was a phantom for three months while listed as consumed |
...
| `StealthBonus` | 37 | `TaomMapVisibilityModel.GetPartySpottingRatioForMainPartySeeingRange` | campaign map | **NOT WIRED IN PRACTICE (#614):** that engine method decides whether an AI party is visible to the PLAYER (its only caller is `CalculateMobilePartyVisibilityAndInspected`), the player's party is never its argument, and careers are player-only, so the 49 pips have never applied. The AI detects the player through a flat engage radius, not a seeing range; replan pending |
...
became multiplicative). `MountChargeDamage` was wired to the wrong agent that day (the rider's
properties instead of the mount's) and stayed a phantom until #611 (2026-09-17): the consumer set
answers "is anything reading it", not "is it read where the engine looks", which is the blind spot
`PassiveEffectConsumers.cs` names. A load-time gate (`CareerConfigProvider.ValidatePassiveConsumers`) plus a
...
3. Add it to `PassiveEffectConsumers` and give it a line in `CareerPerkConsumerMap.Describe` (the report names the consumer for every type)
4. Ask where the ENGINE reads the value, not only whether TAOM writes it: a mount's `Character` is null, its charge and speed are read off the horse, and the spotting ratio is the player seeing others (#611, #614)
