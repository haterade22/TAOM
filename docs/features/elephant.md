...
caches now live in `ElephantCombat.Profile` (eager-resolved once, shared by the tasks + the engage gate; was `ElephantAttackActions` before the 2026-07-01 unification), and
...
```
main (Selector)
├─ has rider           [HasRiderDecorator]        ← reused base; ANY rider, the player included (#643)
...
Until #643 (2026-09-23) an `IsAiControlledDecorator` level sat under "has rider", and a player-ridden elephant fell
through to a one-second sleep. The node names above are the 2026-06-10 ones; since #305 they are the shared
`ElephantLike*` nodes in the Key files table.
```
...
### Player-ridden elephants attack too (#643, 2026-09-23)
...
**The blow is the rider's** (Mike, the same day: "the rider, like the warg"). The shared hit used to pass the creature as the attacker, so the engine handed the hit and the kill to the mount agent, which has no Character (the engine builds it with null, v1.5.3 `Mission.cs:4611`). It now passes the rider, and falls back to the creature only when the rider has gone, as `WargAttackService` does. A hit's affector is read from `Blow.OwnerId` (`Agent.cs:5465`), so `OnAgentHit` and `OnScoreHit` now see the rider. The kill goes through native: `Agent.Die` hands the blow to `IMBAgent.Die` (`:4694`), which calls back `Mission.OnAgentRemoved` with the killer (`Mission.cs:3006-3012`); how native picks the killer is unread, presumably the same `OwnerId`. A player rider's creature blows now reach `CareerPerkMissionBehavior.OnScoreHit`, whose "+N from ability" line would be false for a blow that skipped the damage model, so it skips TAOM's own blows (`CustomAttacksUtils.IsRegisteringSyntheticBlow`, set while the engine raises the hit callbacks for one); a punch or kick keeps its line, because the ability's damage bonus reaches those. In a campaign battle the kill model also counts the killer's party in the survival roll, which it does only for a human affector (`SandboxAgentDecideKilledOrUnconsciousModel`). The hit's position moves with it: `TakeDamage` places the blow halfway between attacker and victim, so a rider high on the beast lifts it. The managed hit path uses that point for the hit sound and its alarm radius (`Agent.cs:5475-5483`); what native does with it is unread. **The sound itself changes too** (Codex, 2026-09-23): for a weaponless blow `BlowWeaponRecord.GetHitSound` picks the charge-damage sound when the owner is not humanoid and a punch or kick sound when it is, so a rider-owned creature blow would sound like a punch, as the warg's bite always has. Mike kept the creature's sound: the shared hit passes `chargeImpactSound: true`, which marks the blow `NoSound` and has `CustomAttacksUtils` replay the engine's own sound block (`Agent.cs:5466-5484`) with the charge-damage event, the same "Armor Type" parameter and the same alarm.

**The combat log now reads Pierce.** It reads `CombatLogData.DamageType`, which vanilla fills inside `MissionCombatMechanicsHelper.GetAttackCollisionResults` and a synthetic blow never reaches, and the `CombatLogData` constructor sets Blunt. So every creature blow, the warg's and the spider's included, logged Blunt until `TakeDamage` began setting the field (2026-09-23).

The elephant stays mount-locked (`TaomAgentStatCalculateModel`), so a player only rides one they spawned on.
