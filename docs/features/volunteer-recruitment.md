...
24 chance groups (23 regular + the conditional Ithil Guard rule) covering all Gondor regions (Anórien, Lebennin, Lossarnach, Belfalas, Lamedon, Pinnath Gelin, Anfalas, Harondor) plus per-town pools for capital settlements (Minas Tirith, Pelargir, Cair Andros, Dol Amroth, Linhir, Caras Tolfalas, Ost Arndir, Morlad, Serelond, Lond Cirion, Methir). Together they cover all 97 live `TAOM_Map` EW settlements (27 towns and castles, 70 villages, counted 2026-09-13 after #597), each in exactly one group; the 98th key is the `town_ES2` conditional.

**Weighting standard (2026-09-13).** One rule per settlement type:

- **Towns.** Where a group mixes a regional line with a household / settlement-specific line, the regional line totals **60%** and the household line **40%**. Minas Tirith and Osgiliath run **50% Anórien / 40% settlement-specific / 10% Ithilien Ranger**. Dol Amroth follows the rule, so `gondor_da_noble` + `gondor_da_footman` share the 40%.
- **Castles.** A castle that has a household line offers **only** that line: Glanhir (Ringlo Vale), Morlad (Blackroot Vale), Bar-en-Siril (Serelond), Caras Tolfalas, Linhir, and Cair Andros at 90% Cair Andros / 10% Ithilien Ranger. A castle without one keeps its pure regional pool.
- **Villages.** The regional line only, except a village named for a vale, which follows the vale: Upper Ringló and Vale Village (Ringlo Vale), Morlad's four villages and Blackroot Village Haven (Blackroot Vale). One pre-existing exception was left as found: Bar Melui's three villages share the town's Lossarnach Regular + Noble group, so they carry the 40% noble share too.

Single-line groups total 100%, and trailing remainders absorb into the last troop of a split (`13.3333 / 13.3333 / 13.3334`) so every group totals exactly 100. History: the town split was 60/40 until 2026-07-27, then 80/20 (a user-directed rebalance that also took Dol Amroth off its 90/10 Swan Knight exemption), then back to 60/40 on 2026-09-13 after a player reported the household lines (Swan Knights, Blackroot Vale bowmen, Methir archers) as too rare next to the regional levies; the castle and vale-village rules date from the same change, which also corrected four fiefs that recruited the wrong region for where they stand (Glanhir's Ringlo villages gave Lossarnach, Morlad's villages Pinnath Gelin, Blackroot Village Haven Belfalas, Bar-en-Siril Anfalas beside Serelond's own line). `GondorJsonLoader_ProductionJson_HouseholdLinePools_OfferOnlyThatLine` and `GondorJsonLoader_ProductionJson_TownHouseholdLineShareIsFortyPercent` pin all three rules against the production JSON.
...
- 2026-09-13: Four Gondor fiefs recruited the wrong region for where they stand (Glanhir's Ringló villages gave Lossarnach, Morlad's four villages Pinnath Gelin, Blackroot Village Haven Belfalas, Bar-en-Siril plain Anfalas beside Serelond's household line). Fixed, and on player feedback the household lines were made commoner: a castle with a household line is now exclusive to it and towns run 60/40 instead of 80/20. Two production-JSON tests pin the rules; the hand-written mirror and its roll tests follow.
