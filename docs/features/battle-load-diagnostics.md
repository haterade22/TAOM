...
**Known limitation 2 — the window measures the wrong quantity ([#425](https://github.com/haterade22/TAOM/issues/425), open).** Of the three closers above, only `FirstMapTick` can fire *within* the current exit — `ResetLifecycle` and the next `Mission.Initialize` both belong to the **next** battle's lifecycle. And `FirstMapTick` requires a real campaign map tick, which does not occur while the player is in a menu or a conversation. The window therefore measures **time until the player returns to the map**, not **time the exit took**, and every second the player spends reading a post-battle screen counts as stall time. This is what makes `ExitStallSampler` fire on healthy exits (see the warning under that section). Until #425 lands, read `ExitBegin→MapResumed` as the real teardown cost and treat `MapResumed→FirstMapTick` as player time.
...
> ⚠ **The "normal exits never log a false sample" claim was FALSE — corrected 2026-08-08, see [#425](https://github.com/haterade22/TAOM/issues/425).** A 45-minute field session produced **four** `[ExitStall]` ERROR lines, none of them a hang. The threshold is not the problem; the *window* is. `ExitBegin` opens it and — within a single exit — `LogFirstMapTick` is its only closer, and `FirstMapTick` needs a real campaign map tick, which never arrives while the player sits in a post-battle menu or a conversation. **So the window measures "time until the player is back on the map", not "time the exit took".** Measured: `MapResumed` landed within ~1s on all five exits in that session (teardown is always fast), yet two windows ran 123s and 21s — and the player was provably mid-quartermaster-conversation 12 seconds into the 123s "stall". Every capture reported `0 frames`, the signature of a healthy main thread in native code rather than a wedge. Because each false positive suspends the main thread up to three times, this is a live deadlock exposure traded against a non-event, not just log noise. **Do not treat an `[ExitStall]` line as evidence of a hang without first checking `MapResumed`'s `t=+`: if it landed ~1s after `ExitBegin`, the teardown was fine and the rest is player time.** Known accepted risk (documented in the class): suspending a thread mid-GC and allocating before resume can deadlock the sampler — acceptable for dev-machine diagnostics on reproducible stalls. **This sampler named the #331 round-2 sink in a single repro** (`PatchShield.ShieldFinalizerVoid` atop the `WidgetTemplate.OnRelease` recursion) after two multi-agent static rounds had bounded it wrong — see the RCA round-2 section and LESSONS-LEARNED "sample the live stack".
