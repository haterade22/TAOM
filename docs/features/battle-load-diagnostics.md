...
| 5 | `MapResumed` | `MapState_OnActivate_ExitPhase_Patch` (Postfix) | `MapState.OnActivate()` — loading screen over; stamps GC delta + `isSaving` (`SaveHandler.IsSaving`); **disarms the stall sampler** (#425) |
...
**Exit-window gating.** `ExitBegin` opens a window (`IsExitWindowActive`); every other exit phase is silent outside it. This keeps the probes inert where their targets also fire elsewhere: `ClearUnreferencedResources` runs at mission *load*, `MapState.OnActivate` fires at campaign start/load, and `MapState.OnTick` runs **every map frame forever** (its postfix is a two-read early-out when the window is closed, per the hot-path rule). The window is **campaign-scoped**: `ExitBegin` opens only when `Campaign.Current != null` (custom battles have no `MapState` to complete the lifecycle, so opening there would leak the window). Closers, all **unconditional state transitions independent of the master toggle** (a mid-window toggle-off gates only the logging, never the close — deep-review data-flow finding 2026-07-06): `FirstMapTick` (normal path), the next `ResetLifecycle` (next campaign encounter), the next `Mission.Initialize` (chained mission without map activation), since #425 `SubModule.OnGameEnd` routing to `ResetLifecycle`, so the window dies with the `Game` that opened it (`Game.Destroy()` → `GameManager.OnGameEnd(this)` → every submodule) on the quit-to-**menu** path; and since #440 a prefix on `SandBoxSaveHelper.TryLoadSave` for quit-to-**load**. Five, not four, and the last two are not interchangeable: `Game.Destroy` has exactly two callers on 1.4.7 (`OnStateStackEmpty`, `MBInitialScreenBase.OnInitialize`), neither of which sits early on the in-campaign load path, so `OnGameEnd` cannot be relied on to beat the next campaign’s deserialization. `TryLoadSave` is the Load Game click itself and precedes any teardown of the old `Game` by construction. `Mission.EndMission` re-invocation for the same mission is deduped by identity hash so the stopwatch is never restarted mid-exit. **Known limitation:** quitting to the main menu from *inside* a mission and then loading a campaign in the same process can emit one stale `MapResumed`/`FirstMapTick` pair with an implausibly large `t=+` value (self-heals immediately; cosmetic, and the huge timestamp self-identifies as stale).

**Two lifetimes, not one ([#425](https://github.com/haterade22/TAOM/issues/425), fixed 2026-08-09 by PR #429).** The logging window still runs `ExitBegin → FirstMapTick`, and `FirstMapTick` needs a real campaign map tick, which does not arrive while the player sits in a menu or a conversation. So the *logging* window still measures **time until the player returns to the map** — read `ExitBegin→MapResumed` as the real teardown cost and `MapResumed→FirstMapTick` as player time. What changed is that the **stall sampler no longer rides that window**: `LogMapResumed` zeroes `ExitWindowOpenedUtcTicks` unconditionally, so the sampler is armed only across the teardown it exists to watch, while `_exitWindowActive` stays true and `FirstMapTick` still logs its tail. Accepted consequence: a hang *between* `MapResumed` and `FirstMapTick` (menu/VM re-init) is no longer stack-sampled. With thresholds at 15/30/60 s and teardown measured at ~1 s, that window was never where the sampler earned its keep — and every capture it took there was a false positive.
...
Phase gaps say *where* time went; only a stack says *what the frozen thread was doing*. `ExitStallSampler` is the watchdog's exit-side sibling: a background `Timer` polls the service's `ExitWindowOpenedUtcTicks` latch — armed by `ExitBegin`, disarmed at `MapResumed` (#425) and by every window closer, so it is nonzero exactly while a **teardown** is in progress, which is a shorter lifetime than `IsExitWindowActive` (the name predates the split) — and when a stall crosses **+15s/+30s/+60s** it suspends the MAIN thread (captured as `Thread.CurrentThread` in `SubModule.OnGameInitializationFinished`), walks it with the obsolete-as-warning `StackTrace(Thread, bool)` constructor (direct call under a `CS0618` pragma — verified present in both the net472 reference assemblies and runtime), resumes, and only then logs the frames as `[ExitStall] sample#N` lines (nothing inside the suspended window logs or allocates beyond the walk itself). The `Poll` tick carries an `Interlocked` reentrancy guard so a blocked capture can never overlap the next timer tick. It is **independently disableable** via MCM "Exit Stall Sampler → Enable Exit Stall Sampler" — the only diagnostics component that suspends the main thread, so it gets its own kill switch separate from the master toggle. Three samples of a deterministic stall show whether the top frames are stationary (one loop) or phased. Thresholds sit above the healthy tournament exit (~9.5s residual, measured 2026-07-10).

> **The "normal exits never log a false sample" claim was FALSE — found 2026-08-08, fixed 2026-08-09 ([#425](https://github.com/haterade22/TAOM/issues/425), PR #429).** A 45-minute field session produced **four** `[ExitStall]` ERROR lines, none of them a hang. The threshold was never the problem; the *window* was. `ExitBegin` opened it and — within a single exit — `LogFirstMapTick` was its only closer, and `FirstMapTick` needs a real campaign map tick, which never arrives while the player sits in a post-battle menu or a conversation. Measured: `MapResumed` landed within ~1s on all five exits in that session (teardown is always fast), yet two windows ran 123s and 21s — and the player was provably mid-quartermaster-conversation 12 seconds into the 123s "stall". Every capture reported `0 frames`, the signature of a healthy main thread in native code rather than a wedge. Because each false positive suspends the main thread up to three times, this was a live deadlock exposure traded against a non-event, not just log noise. Worse, a **quit-to-load** fires neither `MapResumed` nor `FirstMapTick`, so the armed sampler ran into the *next* campaign's `MBObjectManager.LoadXML` via `GameLoadingState.OnTick` — the heaviest allocation phase in the process, i.e. precisely the suspend-mid-GC wedge the class header documents. The fix disarms at `MapResumed` and adds `SubModule.OnGameEnd` as a closer (see "Two lifetimes, not one" above).
>
> **Correction, 2026-08-09 (PR #440 review).** The `LoadXML` capture above is from the build BEFORE `OnGameEnd` was a closer, and the PR author conceded as much. With `OnGameEnd` in place that exact reach is unlikely: `SavedGameVM.StartGame` runs `CleanStates(0)` — which reaches `Game.Destroy` → `OnGameEnd` — before `StartNewGame`. The residual PR #440 actually closes is the earlier `MBSaveLoad.LoadSaveGameData` deserialization, not `LoadXML`. Worth closing either way, and the `TryLoadSave` prefix ends the ordering question rather than arguing it — but the stated model would mislead anyone reasoning about the window from here. **Unverified:** that the sampler ever fired inside `LoadXML` *with* the `OnGameEnd` disarm present. Nobody has reproduced that.
>
