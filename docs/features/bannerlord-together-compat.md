...
| Requirement | Details |
|-------------|---------|
| Bannerlord version | **v1.5.2**: TAOM's pin as of 2026-09-14 (Steam beta branch). BT a0.5.3.2's declared dependency version for Native/SandBoxCore/Sandbox/StoryMode/CustomBattle was v1.4.7 when this table was written and has **not been re-checked since the 1.4.8 or the 1.5.2 bump**; BT is not installed on the dev machine, so verify against BT's own `SubModule.xml` before relying on this row |
...
|------------|--------|------------|
| **Settings parity is reported, not exchanged** | TAOM's 259 MCM settings live in a per-user file outside the save; **192 are simulation-relevant** (traced per setting to the consuming feature, see [coop-interop.md](coop-interop.md) "What is NOT done"). Two players with different values still simulate differently (battle autocalc, bandit spawn density, AI army targeting, desertion rates) and nothing on the wire corrects it | `SettingsFingerprint` writes one code per MCM group to each peer's log under the co-op gate, so the manual comparison is a few short strings rather than 192 values. Exchanging them through save metadata and warning on join is the remaining step and needs two machines to verify |
