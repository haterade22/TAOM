...
- **A new/changed sprite PNG** must be deployed to the game install AND the generator re-run there (it rewrites the install's `GUI/<Module>SpriteData.xml` + `AssetSources/` + `Assets/`). **Then sync the regenerated manifest + `AssetSources/` + `Assets/` back into the repo** so a later `./build.ps1` doesn't clobber the bake with the stale repo manifest. Verify the sync with `diff` — repo and install `<Module>SpriteData.xml` should be byte-identical.
  - **Sync back ONLY those three (`GUI/<Module>SpriteData.xml` + `AssetSources/` + `Assets/`) — NOT `GUI/PreFabs/`, `GUI/Brushes/`, `GUI/SpriteParts/`, or `ModuleData/`.** An over-broad install→repo sync (copying the whole module or `GUI/` folder) copies the install's (older) prefabs/brushes/JSON back over the repo and silently reverts any uncommitted edits that hadn't been deployed yet. This bit the WotR-momentum bar repeatedly (2026-07-05): the title-reposition, popup side-swap, the clean frame PNG, and even `momentum.json` all reverted and looked "unchanged in-game" because the repo edits had been clobbered by the bake sync. Source (prefabs/brushes/SpriteParts PNGs/ModuleData) flows **repo→install** (via `build.ps1`), never the other way.
  - **Use `pwsh tools/sync_sprite_bake.ps1`** (add `-WhatIf` to preview) — it copies exactly the manifest + the two `GauntletUI` dirs (mirrored) install→repo and touches nothing else, so it can't revert source edits. This is the safe replacement for a manual folder copy.
