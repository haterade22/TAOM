...
4. **Add equipment roster pairs** for every `title_type` value used by the new culture's youth_menu options. For each option, add a male and female `<EquipmentRoster id="player_char_creation_{culture}_{title_type}_m">` / `..._f` entry to whichever equipment XML covers that culture (`taom_char_creation_equipment.xml` for a custom culture; a vanilla-mapped culture's rosters are generated into `taom_player_start_vanilla_override.xml` by `tools/wire_starter_kit_rosters.py`, which also repoints every slot at the `starter_` twins, see [starting-equipment-tuning.md](starting-equipment-tuning.md)). Without these, the player exits CC with vanilla default equipment and `PlayerEquipmentService` logs a "roster not found" warning.
