...
| File | Purpose |
|------|---------|
| `Main/Features/CulturalFeats/TaomCulturalFeats.cs` | Feat registration + static accessors (43 feats) |
...
48 tests total. GameModel overrides are thin entry points (delegate to `base` + apply feat modifier) and are verified via in-game testing.
...
6. Update the test count in `AllFeatProperties_ReturnFeatObject_CountIs43`
