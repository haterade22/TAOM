...
Bannerlord v1.5.2 on 2026-09-14; in-game verification owed.
...
way `AdvancedStartOptionsManager.Initialize` binds it, on the installed engine).
**Related:** [`rca-v1.5.2-compile-2026-09-14.md`](../reviews/rca-v1.5.2-compile-2026-09-14.md) · the v1.5.2 entries in `CHANGELOG.md`
...
- `str_campaign_starting_options_item_name.<id>` is the list entry, and also the `{Token}` interpolation
  inside scenario text such as `{InvasionScenarioFactionId}` (`AdvancedStartOptions.SetTextVariables`).
- `str_advanced_start_value_name.<id>` is what the in-game Escape menu's starting-options summary reads
  back from the saved `AdvancedStartOptionsData` (`GetDisplayName`), so a missing row shows there, not in
  the picker.
