...
The release doc's three-field table (`docs/reference/release-process.md:47-51`) says `Main/_Module/SubModule.xml` carries a `<DependedModuleMetadata id="TAOM.Dependencies" ... version="v2.0.Y" />` row that must match the Dependencies module's version. It does, and so does the engine-honoured `<DependedModule Id="TAOM.Dependencies" />` beside the comment at `Main/_Module/SubModule.xml:15-22`. <!-- measured: rg -n "TAOM.Dependencies" Main/_Module/SubModule.xml 2026-09-14 --> An earlier revision of this paragraph recorded both as absent and called the release doc wrong. The absence was real but not intentional: `cc1713eb` (2026-08-11, a cultures commit) deleted both elements as collateral, v2.0.23 through v2.0.28 shipped without them, and they were restored on 2026-09-14. The tests and the `/release` skill are the second layer; the manifest element is the one the launcher enforces. The [module-dependencies](module-dependencies.md) chapter owns that story.
