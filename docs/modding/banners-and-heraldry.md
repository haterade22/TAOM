...
|---|---|---|---|---|---|
| `id` | int | Yes | none, the load throws | Identifies the whole sheet or category. It is never seen in game. Two groups with the same id merge into one rather than replacing each other, across modules as well as inside one file. Group ids are their own number space and do not clash with colour ids. | `BannerIconGroup.cs:42` |
...
| Attribute | Type | Required | Default when absent | What it does | Read at (file:line) |
|---|---|---|---|---|---|
| `id` | int | Yes | none, the load throws | The number for one emblem. This is what a `banner_key` names, and in TAOM it is also the filename of the UI sprite. It must be free across every active module. | `BannerIconGroup.cs:49` |
...
|---|---|---|---|
| `<Icon>` | a group with `is_pattern="false"` | Append, first wins, silent. An `<Icon>` is added only when its id is free in this group and in every group already loaded. A duplicate id is dropped with no warning and no log line. | `BannerIconGroup.cs:47-52` |
| `<Background>` | a group with `is_pattern="true"` | Same rule as `<Icon>`. One ordering trap: `is_base_background` is acted on before the duplicate check, so a `<Background>` that is otherwise ignored can still move the engine's default cloth. | `BannerIconGroup.cs:61-69` |
...
| `<BannerIconGroup>` | `<BannerIconData>` | Looked up by id. An existing group is merged into, not replaced, and `Merge` copies only the keys the first group is missing. You can add ids to a vanilla group; you cannot override one. | `BannerManager.cs:236-248` |
...
- **Duplicate ids are dropped with no warning.** An `<Icon>`, `<Background>` or `<Color>` id already taken by an earlier group or an earlier module is skipped, and there is no error and no log line. Your emblem simply does not exist (`BannerIconGroup.cs:52, 69`).
- **Same-id groups merge, they never replace.** At the XML level `MBObjectManager.MergeElements` matches groups by their unique `id` and merges the children in (`MBObjectManager.cs:820-873`), and at the C# level `BannerManager` looks the group up and calls `Merge`, which copies only the keys it is missing (`BannerManager.cs:236-248`, `BannerIconGroup.cs:77-100`). You can add ids to Native's groups, as `NavalDLC` does with 14 icons into group 2, but you cannot override one.
- **A colliding `<Color>` id resolves to the last module, not the first.** `BannerManager` guards with `if (!_colorPalette.ContainsKey(key))`, which reads like first wins, but the guard never fires: the XML merge already collapsed the two nodes into one, and `MergeElementAttributes` writes the later document's attributes over the earlier one's (`MBObjectManager.cs:799-817`). TAOM loads after Native (`SubModule.xml` line 25 pins `order="LoadBeforeThis"` on Native), so for the 46 ids both files define, TAOM's hex is the one that renders. Resolve a palette id against TAOM's file first and fall back to Native's only when the id is absent there ([clan-heraldry.md](../features/clan-heraldry.md) lines 83 to 90).
