...
Two rules govern an override stylesheet, and only the first is absolute. **Every stylesheet needs the global identity template**, `<xsl:template match="@*|node()">` wrapping `<xsl:copy>`, or every node it does not name is dropped from the merged document; both Armory stylesheets open with one (`crafting_templates.xslt:3-7`, `weapon_descriptions.xslt:3-7`). **Inside a template that replaces a specific node, you re-emit vanilla's contents with a second `<xsl:apply-templates>` of your own**, or vanilla's entries under that node are gone. Where that line sits is a choice, not a rule. Copy the attributes with `<xsl:apply-templates select="@*"/>` before you emit any child, because XSLT lets a processor drop an attribute added after content, then put the child passthrough wherever the schema wants those children to sit. Of the 14 override templates in `weapon_descriptions.xslt`, 13 end with the passthrough and one, `OneHandedPolearm`, emits nine more `<AvailablePiece>` rows after it (`weapon_descriptions.xslt:672-683`); all 9 in `crafting_templates.xslt` end with it. <!-- measured: python regex walk of the two Armory .xslt files counting templates and the content after the last apply-templates 2026-09-05 --> TAOM's own `Main/_Module/ModuleData/spcultures.xslt` shows the general shape: attributes through first at `:17`, overrides next, then a passthrough at `:317` that names only the children the block did not replace. See [`docs/reference/item-usage-features.md`](../reference/item-usage-features.md) and the [xslt rule](../../.claude/rules/xslt.md).
...
3. Grep every other folder for your id prefix first. A duplicate item id across two folders is not shadowed, it is merged: both files are registered under the id `Items`, so the two rows collapse into one hybrid before the engine reads either, with the later file winning attribute by attribute and the earlier one filling in the rest (`MBObjectManager.cs:846-874`). Nothing is logged. The canonical home may also not be the culture you expect: `sk_dwarf_iron_*` lives in `iron_hills/`, not `erebor/`. The prefix table is in [`docs/reference/armory-guide.md`](../reference/armory-guide.md).
...
Takes effect: full game restart. `skins.xml` and `action_sets.xml` reach the engine through native callbacks the process runs once at boot (`Module.cs:1363-1366` and `:1387-1389`), off a registry built once in `Module.Initialize` (`Module.cs:267`, `:1031-1032`), so the race does not exist at all until you relaunch. Whether you also need a new campaign depends on what consumes the race, not on this file: `monsters.xml` goes through the managed channel and is re-read on every campaign load (`Game.cs:437`, called from `Campaign.cs:1470`), while anything the save already stores about an existing character does not change.
...
- **A `.bak` file named `*.xml` inside a registered folder is loaded as content.** The folder glob takes every `*.xml` (`MBObjectManager.cs:903`), and same-id rows across two files do not become two items, they merge into one (`MBObjectManager.cs:846-874`). So a stale copy quietly puts back every attribute the live file no longer carries, and where the glob reaches the backup second it overwrites the live values outright. Nothing is logged either way. Use `.bak-<reason>-<date>`; see [`docs/reference/module-backup-sweep.md`](../reference/module-backup-sweep.md).
...
- **A `HorseHarness` is required beside every `Horse` slot, because it may BE the saddle.** `sk_eb_goat_a` and `sk_eb_goat_b` are bare pelts; the war ram's seat is modelled on the eight `sk_eb_goat_bard_*` meshes and nowhere else, so leaving the slot empty removes the saddle rather than only the armour. Nothing in the XML says which mounts carry their own seat (the horse and the warg do, via `warg_low_fur_with_saddle`; the spider does not and has no harness item at all), so the rule is universal and exceptions are argued in `_HARNESSLESS_BY_DESIGN`. Code `MOUNT_WITHOUT_HARNESS`.
...
- **Whether the stale `<DependedModuleMetadata id="Native" version="v1.4.5.*"/>` is enforced against the installed v1.5.3 or advisory.** The game runs, which suggests advisory, but the launcher's version-constraint code has not been read.
...
| `weapon_descriptions.xslt`: 15 templates, 1 identity plus 14 overrides, 13 ending in the passthrough and 1 (`OneHandedPolearm`) emitting 9 rows after it; `crafting_templates.xslt`: 10 templates, 1 identity plus 9 overrides, all 9 ending in it | Python regex walk of both stylesheets, splitting on `<xsl:template match=>` and testing what non-comment content follows the last `<xsl:apply-templates>` |
