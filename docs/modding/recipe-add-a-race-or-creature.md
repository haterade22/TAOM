...
| is a four-legged pack bought on its own rig (a Fab or marketplace creature) with its own clips | **Reskin with its own animation.** Bend the mesh onto `horse_skeleton` keeping its weights, retarget its clips onto the same skeleton, bind them in a child of `as_horse` ([workflow](../ai-includes/quadruped-pack-to-horse-skeleton-workflow.md), worked example [animalia-elk-moose](../features/animalia-elk-moose.md)) | days |
...
| should attack with something other than a kick | **Bespoke**, or author one clip onto the existing rig. The vanilla horse rig's only attack clip is the kick | weeks, or one clip |
...
- **`hill_troll`'s four variants were named `troll_child`, `troll_settlement`, `troll_settlement_slow`, `troll_settlement_fast`** until 2026-09-24. The race id is `hill_troll`, so none of the four was ever found. The action sets on the same race were named correctly (`as_hill_troll_child` exists), which is what made the mismatch easy to miss. `tools/wire_hill_troll_race.py` renamed them `hill_troll_*` in the live file and the snapshot; a reinstall brings the old names back ([lotrlome-hill-troll-changes](../reference/lotrlome-hill-troll-changes.md)).
...
1. Skin the mesh to a skeleton the engine already ships, bone for bone. `horse_skeleton` for anything horse shaped. A mesh bought on its own quadruped rig does not need re-weighting: `tools/blender/reskin_animalia_to_horse.py` bends it onto the horse rest with its own weights, and `tools/blender/retarget_animalia_to_horse.py` brings its clips along through the same fit. Measure the joint gap first; after one uniform scale the legs and spine should sit within about 10 cm of the horse's ([animalia-elk-moose](../features/animalia-elk-moose.md)).
...
### Add a humanoid race on its own skeleton

The dwarf's layout, worked through for the hill troll on 2026-09-24 ([troll-race](../features/troll-race.md), the entries of that date; ledger [lotrlome-hill-troll-changes](../reference/lotrlome-hill-troll-changes.md)). The artist's rig keeps the human's 28 bone names and order. Export it with its skinned meshes as one FBX through `tools/blender/export_rig_for_kit.py`, with `--bone-frames` from `tools/tpac_skeleton_copy_physics.py --reframe`: that turns every bone to the human's anatomical axes with its position kept, and adds the two grip bones (`l_finger0`, `r_finger0`) the Monster hangs weapons on. Without the re-frame every human clip twists the rig, because the engine plays a clip's joint rotations as they are. Name the head's parts `<mesh>`, `<mesh>.eye`, `<mesh>.mouth` and the materials by the Kit's exact names (`--material OLD=NEW`; the Kit binds by name module-wide and a same-named older material wins silently). After the Kit import, copy the human physics into the package (`tpac_skeleton_copy_physics.py --reframed <that JSON> --fit <a skeleton_hit_capsules.py fit --axis bone>`), as [bannerlord-skeleton-authoring](../reference/bannerlord-skeleton-authoring.md) "Ragdoll, IK and hit capsules for a humanoid on its own skeleton" describes. Then the data: every `<skin>` of the race on the new skeleton and meshes (`tools/wire_hill_troll_race.py` is the pattern), the Monster's eye heights, camera offset, arm length and capsule measured from the model, its variants named `<race>_child` and `<race>_settlement*`, and a standalone `as_<race>_warrior` with `skeleton="<skeleton>" movement_system="bipedal"` filled by `patch_dwarf_action_parity.py --set-id as_<race>_warrior --apply`. Human clips do not play right on a rest pose unlike the human's, re-framed or not (a clip stores parent-relative rotations; on the hill troll they pitched the head up 45 to 65 degrees and twisted the wrists), so every action the race plays goes through `tools/blender/retarget_mannequin_to_human.py --engine-skeleton <the skeleton's engine dump>` (`--source-json` over `tools/read_anim_keyframes_tpac.ps1 -ByClip` output for human masters, `--clips` for a pack), a Kit import, `wire_anim_master_skeletons.ps1` and `gen_troll_anim_clips.ps1` (`-CloneByName` for human-sourced masters) with `-TravelScale` set to the retarget report's `pelvis_scale`; the standalone action set then binds those clips.

**Check:** `python tools/validate_xml_schemas.py <monsters.xml>`, `python tools/audit_action_set_parity.py` (add the new root to its `HUMANOID_ROOTS`), `python tools/validate_moduledata.py`, then `python tools/tpac_skeleton_dump.py <geo.tpac> <skeleton>` after every Kit re-import (a changed skeleton wipes the physics).
**Takes effect:** full game restart; the Kit must save the module once so the package's `.rdc` is current.
**Code:** No code changes needed; `RaceManager` reads the race list from the engine.
...
- **A misspelled Monster variant id returns null and says nothing.** `GetMonsterWithSuffix` concatenates and looks up, `FaceGen.cs:44-47`. This is exactly what `beserker_child` does today, and what the hill troll's `troll_*` block did until `wire_hill_troll_race.py` renamed it `hill_troll_*` on 2026-09-24.
...
- **Put a mount's size on its Monster** (`taom_body_length`, TAOM's attribute, copied into every Horse item that
  names the Monster at game init: [monster-size](../features/monster-size.md)) rather than on each item, so one edit
  resizes it and an attack reach flagged `reachScalesWithBody` follows.
- **`body_length` on a mount item scales the mount only.** The managed code reads as if the rider would scale too (`EquipmentIndex.ArmorItemEndSlot` and `EquipmentIndex.Horse` are the same value and the scale block in `Mission.BuildAgent` has no `IsMount` guard), but in game he is not resized ([mumakil](../features/mumakil.md), "RESOLVED"). What does not scale is anything your own code places against the mount, such as an attack reach; derive it from the scale ([creature-mount-authoring](../ai-includes/creature-mount-authoring.md)).
