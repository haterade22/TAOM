...
| A rider sits on bare hide, with no saddle under him | The `slot="Horse"` entry has no `slot="HorseHarness"` beside it in the same `<EquipmentRoster>`. On some mounts the harness IS the saddle: the ram's seat is modelled only on the eight `sk_eb_goat_bard_*` barding meshes, never on the `sk_eb_goat_a`/`_b` body. A harness is required beside every `Horse` slot for that reason. `MOUNT_WITHOUT_HARNESS` | [Items: mounts and harness](items-mounts-and-harness.md#gotchas-what-fails-silently-and-what-crashes); `docs/features/war-ram.md` |
