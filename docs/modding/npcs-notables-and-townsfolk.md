...
|---|---|---|---|---|---|
| `occupation` | enum | in practice yes | `NotAssigned` | The role switch for this whole chapter. `Merchant`, `Preacher`, `Artisan`, `GangLeader`, `RuralNotable` and `Headman` make an entry eligible to become a settlement notable; `Tavernkeeper`, `Armorer`, `GoodsTrader`, `Musician` and friends mark a service NPC. Parsed with `Enum.Parse`, so a misspelling throws and truncates the load at that entry, taking every entry below it with it. | `CharacterObject.cs:539` |
...
1. **`occupation`.** It decides which of the six notable pools the entry joins, and it is the one attribute here that stops the load dead when misspelled, taking every entry below it with it.
...
Takes effect: next save load. `NPCCharacters` is re-read at the start of every campaign, saved games included (`SandBoxManager.cs:362`), and so is the culture template list that registers it (`Campaign.cs:1410`), so the new template is in the pool as soon as the save is open. An existing save keeps the notables it already created; the pool is drawn from again only when the game needs a new one.
...
3. **Give it a `<face>`,** normally `<face><face_key_template value="BodyProperty.fighter_<culture>" /></face>` copied from its siblings. Leaving it out is not neutral: the character's body properties come back all-zero with age 0 and the engine renders it on the toddler skin, silently. Ten cultures shipped that way and their arena fighters appeared as children ([body-properties](body-properties.md) "Gotchas").
4. **Give it a plain, non-civilian `<EquipmentRoster>`.** Only `RandomBattleEquipment` is read, so a civilian-only roster produces nothing and the model falls back to vanilla.
5. **Use items that fit the skeleton.** A dwarf-race dummy in man-sized armour is the same mesh mismatch as anywhere else; item ids come from the Armory ([armory-guide](../reference/armory-guide.md)).
6. **Confirm the item ids resolve.** A typo here is silent: the slot ends up empty and the fighters go out in their underwear.

Check: `python tools/validate_moduledata.py --code BROKEN_ITEM_REF`, then `dotnet test TAOM.Tests -p:DisableModuleCopy=true -p:ModuleId= --filter FullyQualifiedName~CharacterFaceCoverageTests`
...
- **A woman with no `is_female="true"` is a man, and nothing says so.** `is_female` defaults to false (`BasicCharacterObject.cs:479`), so an entry named `townswoman_gondor` without the attribute picks the male skin, the male face range and the male action set. There is no error, no log line and no validator rule. Every female-role entry outside Rohan shipped this way until 2026-09-06: 166 of them across 17 files, so Townswomen, Tavern Wenches, village women, beggars and dancers were men in every culture but one. `TAOM.Tests/Core/TownsfolkAndNotableSexConsistencyTests.cs` now pins it. RCA: [rca-townsfolk-sex-2026-09-06](../reviews/rca-townsfolk-sex-2026-09-06.md).
- **A notable's sex is its template's sex, so an all-male pool is an all-male settlement.** `HeroCreator.CreateNotable` builds the hero with `CharacterObject.CreateFrom(template)` and `HeroInitializationArgs` then reads `IsFemale` back off that hero (`HeroCreator.cs:215-221`, `:301`); nothing randomises it. All 596 notable templates were male until 2026-09-06, Rohan included, against vanilla's 28 female out of 128. The name lists do not save you either: `GenerateFirstAndFullName` picks from `<male_names>` for a male template, so an all-male pool also means no woman is ever *named* as a notable.
- **For every non-human race, the female skin is the male body mesh.** `LOTRLOME_Armory/ModuleData/skins.xml` gives dwarf, elf, orc, uruk, uruk_hai, pale_uruk, dg_uruk, goblin and both trolls the same `body_meta_mesh` for `gender="0"` and `gender="1"` (dwarf women and men are both `sm_dwarf_basemesh_a1_body`). Those skins do set `body_mesh_suffix="_fem"`, so garments resolve their `_fem` variant and the face, animations and name all come out right, but the silhouette underneath stays male until someone authors female base meshes. That file is a game-install asset, so a module reinstall reverts hand edits; fix it upstream in the Armory, not here.
...
- **A misspelled `occupation` truncates the load, it does not lose the file.** `Enum.Parse` at `CharacterObject.cs:542` throws inside the sequential walk at `MBObjectManager.cs:1387-1395`, and the public `LoadXML` swallows it in an empty `catch` at `MBObjectManager.cs:786-796`. Characters read before the bad one are already registered and keep working, the bad one is half built, and every entry after it is skipped with nothing logged. The walk covers the merged `NPCCharacters` document, which is every module's rows in load order, so the characters that go missing can sit in a different file from the one you broke. The same is true of `race`, which indexes a dictionary rather than falling back to human. The validator catches neither: `tools/schemas/taom_npccharacter.json` enumerates `default_group` and nothing else.
...
- **`civilian="true"` on an inline `<EquipmentRoster>` is accepted in silence, with no assert.** That spelling goes through `MBEquipmentRoster.InitEquipment` (`MBEquipmentRoster.cs:88-106`), which reads the attribute and switches the roster to Civilian without complaint. The deprecation assert belongs to the other shape, `civilian=` on an `<EquipmentSet id="..."/>` reference, which is read in `BasicCharacterObject.Deserialize` (`BasicCharacterObject.cs:395-401`) and told to use `equipmentType="Civilian"` before being honoured anyway. Every notable in this folder uses the inline form, so leave it alone rather than "fixing" it. The three-way table is in [equipment-rosters](equipment-rosters.md).
- **The `gear_practice_dummy` attribute on a `<Culture>` wires nothing.** No deserializer in the v1.5.2 dump reads an attribute of that name (still true, re-checked 2026-09-14). The only place the string appears is the id the tournament model concatenates at `DefaultTournamentModel.cs:90`. The lookup is by id, so the attribute is decoration.
...
| 180 female-role entries, all 180 now marked `is_female="true"`; 166 of them were missing it before 2026-09-06 | PowerShell `XmlDocument` walk of `npcs_*.xml` matching the id prefixes `townswoman`, `village_woman`, `villager_female`, `tavern_wench`, `female_beggar`, `female_dancer`, counting the attribute. PowerShell rather than Python because `python` on this machine resolves to the Microsoft Store alias <!-- measured: PowerShell XmlDocument walk of npcs_*.xml 2026-09-06 --> |
| 139 of 596 notable templates female (23%): Merchant 66/220, GangLeader 51/149, Preacher 22/66, and Artisan, RuralNotable and Headman 0 each | same walk, grouped by `occupation` where `is_template="true"` <!-- measured: PowerShell XmlDocument walk of npcs_*.xml 2026-09-06 --> |
| Vanilla, for comparison: 28 female of 128 notable templates (22%), with RuralNotable 0/12 and Headman 0/18 | tag scan of `SandBox/ModuleData/spspecialcharacters.xml`, the only vanilla file defining notable occupations <!-- measured: grep over spspecialcharacters.xml 2026-09-06 --> |
| 18 barbers, all male | the two female ones, `barber_harad` and `barber_rhun`, were made male on 2026-09-06 so the set agrees <!-- measured: PowerShell XmlDocument walk of npcs_*.xml 2026-09-06 --> |
