...
| `_replaceWhileMerging="true"` | Removes every attribute and every child of the earlier element before the later one lands; the only true replace. TAOM uses it in one place: `equipmentsets/taom_player_start_vanilla_override.xml`, on each `EquipmentRoster` that overrides a vanilla player-start roster ([starting-equipment-tuning](../features/starting-equipment-tuning.md)). Put it on the element the schema keys (`EquipmentRoster/@id`), never on a keyless child such as `EquipmentSet`: the merge looks the child up by its unique attributes, so keyless children all resolve to the last one and the replace lands on the wrong set. The lords pipeline deliberately does not use it ([lord-identity-reconciliation](../features/lord-identity-reconciliation.md), lines 24-34) | `MBObjectManager.cs:804-808`, `829-832`, `1092` |
...
Two surfaces sit outside the `MBObject` registry and are worth calling out. Banner icons read like a first-wins surface and mostly are not: they go through the same merge as everything above, and only the leftovers reach a rule of their own. `BannerIcons.xsd` gives groups, icons, backgrounds and colours unique keys, so the merge above pairs same-id rows before `BannerManager` ever runs and the later module's attributes win (`BannerIcons.xsd:39-47`, `69-81`; `MBObjectManager.cs:799-874`). The first-wins guards inside `BannerIconGroup.Deserialize` and the colour loop are reached only by an id the merge left unpaired, which for an icon or a background means a duplicate that landed in a *different* group, and for a colour means never (`BannerIconGroup.cs:47-72`, `BannerManager.cs:236-248`, `258-259`). Gauntlet prefabs key on the file name without extension and a later module's file overwrites the dictionary entry after a `FailedAssert`, so the last module wins (`WidgetFactory.cs:78-93`). Details in [banners-and-heraldry](banners-and-heraldry.md) and [module-taom](module-taom.md).
