...
- **A typo in `skill_template` is silent and leaves the wanderer at zero skills.** The engine creates an empty placeholder under the referenced id and moves on. TAOM's validator catches it only when inline `<skills>` rows sit beside the template: `SKILL_TEMPLATE_MISMATCH` (`tools/validate_moduledata.py`, #626) fires when a template with rows resolves nowhere, or when a row differs from the template's value. `BasicCharacterObject.cs:337-364`, `MBObjectManager.cs:724-729`.
