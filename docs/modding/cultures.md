...
| `fishing_party_template`, `settlement_patrol_template_coastal` | ref `PartyTemplate.<id>` | no | null | Consumed only by NavalDLC (`Culture.FishingPartyTemplate`, `Culture.SettlementPatrolPartyTemplateNaval`, five reads in `NavalDLC__NavalDLC.cs`, present on v1.4.8 and v1.5.2; the earlier "no consumer" claim never searched the module bins), which TAOM declares incompatible (`IncompatibleModules NavalDLC`), so no effect in a TAOM game. No TAOM culture sets them. | `CultureObject.cs:272`, `:280` |
...
| `executioner` | ref `NPCCharacter.<id>` | no in the parser, yes in practice: `HeroExecutionSceneNotificationData` dereferences `Culture.Executioner` unguarded, so a culture without one crashes the moment its player executes a lord | null | New in v1.5.x: the character the execution cutscene dresses as the headsman. TAOM sets it on all 24 cultures, each pointing at that culture's `elite_basic_troop`. Gate: `CultureExecutionerTests`. | `CultureObject.cs:341` |
