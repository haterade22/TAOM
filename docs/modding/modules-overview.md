...
|---|---|---|---|---|---|---|---|
| `Native` | The engine's own data: skeletons, action sets, item modifiers, banner icons, the 39 `soln_*` native ids | Never. Game install, `Official` | `v1.5.3` (`Native/SubModule.xml:5`) | vanilla | 24 `XmlNode` rows across 18 ids | 50 `<file>` rows <!-- measured: for m in Native SandBoxCore SandBox CustomBattle TAOM TAOM_Map LOTRLOME_Armory TAOM.Dependencies; do grep -c '<file ' "<game>/Modules/$m/ModuleData/project.mbproj"; done 2026-09-05 --> | not counted |
...
row at line 19, and `LOTRLOME_Armory` names `TAOM` nowhere. All three pin the engine the same way now:
`v1.5.3.*` on the `Native` metadata row (`TAOM_Map/SubModule.xml:15`, `LOTRLOME_Armory/SubModule.xml:15`,
`Main/_Module/SubModule.xml`), and the installed game reports `v1.5.3` (`bin/Win64_Shipping_Client/Version.xml`,
matching the repo pin file). The two live manifests carried `v1.4.5.*` until 2026-09-14. <!-- measured: cat "<game>/bin/Win64_Shipping_Client/Version.xml"; cat .claude/pinned-game-version.txt 2026-09-15 -->
...
3. `version="v1.5.3.*"` on the `Native` row follows the installed engine (it read `v1.4.5.*` until
   2026-09-14), and is inert on a vanilla launcher.
...
- Engine version `v1.5.3`: `cat "<game>/bin/Win64_Shipping_Client/Version.xml"` and `cat .claude/pinned-game-version.txt`.
