...
|---|---|---|---|---|---|
| `id` | string | yes | none; both the walk at `MBObjectManager.cs:1391` and this line read `node.Attributes["id"].Value` with no null check, so a missing id throws and truncates the load at that entry | The permanent codename. Saves store it, and party templates, upgrade edges, culture bindings and the recruitment pools all point at it. Renaming one is a save break. | `MBObjectBase.cs:61` |
...
| `is_obsolete` | bool | no | false | Read and stored, and for a line troop that is all it does. The one place the flag is consulted is `Hero.PreAfterLoad`, which skips supporter and companion re-wiring for an obsolete character (`Hero.cs:1537`); nothing on the recruitment, upgrade or party-template paths looks at it. It is a marker on a retired troop, not the mechanism that retires one. See the Delete recipe. | `BasicCharacterObject.cs:336` |
| `skill_template` | ref, written `SkillSet.<id>` | no | a fresh empty skill set is created and named after this character's own id | Points at a shared skill block. Through v1.4.8 an inline `<skills>` child beside it was ignored outright; since v1.5.2 the rows are laid over a copy of it, and TAOM requires them equal (`SKILL_TEMPLATE_MISMATCH`). Zero TAOM troops use it, and the reason is under "Gotchas". | `BasicCharacterObject.cs:337` |
...
| `formation_position_preference` | enum `Back` / `Middle` / `Front`, case-sensitive | no | `Middle` | Where in its own formation the troop stands. Parsed with a plain `Enum.Parse`, so a lowercase `front` throws and truncates the load at that entry. No TAOM troop sets it. | `BasicCharacterObject.cs:497` |
...
| Attribute | Type | Required | Default when absent | What it does | Read at (file:line) |
|---|---|---|---|---|---|
| `occupation` | enum `Occupation` | no | `NotAssigned` | The campaign role. `Soldier` is a fieldable line troop, `Mercenary` a tavern hire, `Bandit` a hideout body. Parsed with `Enum.Parse`, so a typo throws and truncates the load at that entry. `Mercenary`, `Gangster` and `CaravanGuard` are priced as mercenaries. | `CharacterObject.cs:539-542` |
...
|---|---|---|---|---|---|
| `<Skills>` `<skills>` | container of skill rows | no | no skills, everything reads 0 | Either capitalisation is accepted. Through v1.4.8 read only when no `skill_template` was named; since v1.5.2 laid over the template's values. | `BasicCharacterObject.cs:353-364` |
...
9. **Name it** with the `{=key}English fallback` form. As of 2026-09-17, all 836 existing troop-name keys are registered in `taom_troop_name_strings.xml` (generated by `tools/generate_name_localization_strings.py`) and translated in all 12 languages, so a new troop's key needs the generator re-run (or a manual row added) before it gets the same treatment; until then its inline fallback is the English text everywhere. See [strings-and-localization](strings-and-localization.md).
...
Do not delete a troop. An id is stored in saves and referenced from party templates, upgrade edges, culture bindings, weight and cost tables and the recruitment pools, so removing the entry breaks every one of them at once. Retire it instead: keep the row so old saves still resolve it, and cut every path that creates the troop. Steps 2 to 4 are the retirement.

1. **Leave the entry, the id, the skills and the rosters exactly where they are.** Add `is_obsolete="true"` as a marker for the next reader if you want one, but do not expect it to do the work: the only engine code that reads the flag is `Hero.PreAfterLoad` (`Hero.cs:1537`), which never sees an ordinary line troop.
...
- **A throw does not lose the file, it truncates the list.** `LoadXml` walks the merged `NPCCharacters` document entry by entry (`MBObjectManager.cs:1387-1395`) and the public `LoadXML` wraps that whole walk in an empty `catch` (`MBObjectManager.cs:786-796`). Every troop read before the bad one is already registered and stays; the bad one is half built; every entry after it is skipped and nothing is logged. That document is all modules' `NPCCharacters` rows in load order, so the troops that go missing can sit in a different file from the one you broke. [load-order-and-dependencies](load-order-and-dependencies.md), "The truncation rule".
- **For a line troop, `IsMounted` and `IsRanged` come from `default_group` alone, never from equipment.** A Cavalry-grouped troop that rolls a mountless roster walks while the AI treats it as cavalry, and an archer left at `default_group="Infantry"` is waged, priced and commanded as infantry. `BasicCharacterObject.cs:494-496` sets both fields once in `Deserialize`, from the parsed group. Failure shape: [`docs/features/black-numenorean.md`](../features/black-numenorean.md) lines 313 to 324.
- **A HERO is the exception, and the sentence above does not apply to one.** `CharacterObject` overrides both properties: `IsMounted` reads `Equipment[10].Item != null`, and `IsRanged` scans `Equipment[0..3]` for a `Bow`, `Crossbow` or `Sling` item type. `GetFormationClass()` splits the same way. So a hero's formation class really is equipment-derived, and note the scan looks for a bow, a crossbow or a sling only: **a javelin never makes a hero count as ranged**. Verified against installed v1.4.8, `CharacterObject.cs` `IsMounted` / `IsRanged` / `GetFormationClass`.
- **A javelin is not a reason to write `Ranged`.** The skirmisher who throws two javelins and then draws
  a sword belongs in the infantry line, and `Ranged` puts him in the backline holding a melee kit. Five
  shipped that way until #554 (the three Harondor `gondor_har_*` skirmishers and the two Rhûn
  `sagarun_*_naffatun`), each also carrying the full Ranged Bow curve for a bow it does not own. Use
  `Ranged` for a bow or a crossbow. A troop that carries a real bow AND a javelin is a genuine archer
  with a sidearm and stays `Ranged`.
- **On 1.4.8 `skill_template` beat an inline `<skills>` block outright**, and the engine said nothing. Until 2026-08-31 that described 44 TAOM militia troops pointing at vanilla Calradian skill sets: `rivendell_militia_spearman` was authored at 850 total and delivered 215. No TAOM troop file uses the attribute today. Since v1.5.2 the rows are laid over the template instead, and `SKILL_TEMPLATE_MISMATCH` requires the two to agree (#626). `BasicCharacterObject.cs:353-364` and [`docs/features/troop-skill-balance.md`](../features/troop-skill-balance.md) lines 145 to 158.
...
| `default_group`: Infantry 491, Ranged 200, Cavalry 143, HorseArcher 23 | the same scan, counting the `default_group` attribute (re-scanned 2026-09-06 after #554 moved five javelin skirmishers out of Ranged) |
