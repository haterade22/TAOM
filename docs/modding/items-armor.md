...
4. **Set the four numbers from the curve.** Since #583 (2026-09-13) `tools/rebalance_armor.py` holds the kingdom-cap model: `KINGDOM_CAPS` (one chest value per kingdom), `SLOT_CAP_RATIO` (helmet .9, bracer .6, pauldron .6, greaves .5) and `BAND_RATIO` (light .40, medium .64, heavy .84, elite 1.0, lord = elite); `primary = round_half_up(cap x slot ratio x band ratio)` via `calculate_stats(tier, slot, folder, item_id=...)`, and the item id routes a sub-line that shares a folder (the three Mordor lines, Dol Guldur's `sk_dg_` kit in `rhun/`). Weight still comes from the legacy `baseline_weight * culture weight_mult`. The band is the item's lowest battle-set wearer, so a new item worn only by a level-46 troop is elite whatever its name says, and the next `rebalance_armor.py --tier-source roster-first` run will restat anything you hand-number. The per-culture generators keep their own `STAT_TIERS` copies (pre-cap values; they author shape, the curve owns the numbers), so treat `rebalance_armor.py` as the source ([tools README](../../tools/README.md) row for `generate_black_numenorean_armor.py`, which imports it and passes `item_id`). Full table: [armor-balance.md](../features/armor-balance.md) "The kingdom-cap curve".
...
- **A career kit is chest and legs only.** Since #629 they are the body and leg items the culture's lowest troops wear, not starter twins, and `FillFrom` leaves Head, Cape and Gloves empty ([starting-equipment-tuning](../features/starting-equipment-tuning.md)).
