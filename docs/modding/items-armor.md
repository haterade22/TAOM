...
- **Vanilla items load too.** `SandBoxCore/SubModule.xml` registers `<XmlName id="Items" path="items"/>`, and `Main/_Module/ModuleData/` holds 8 XSLT files, none of them for items. So there is no override seam for vanilla item stats: a vanilla helmet's numbers can only be changed by editing `SandBoxCore/ModuleData/items/head_armors.xml` itself, which a game patch reverts.
...
1. **Find the canonical folder before anything else.** Grep every folder for the id prefix you are about to use: `grep -rl 'sk_gd_dol_' <armory>/ModuleData/LOTRLOME_items/`. The first folder that already holds that prefix is the home. Authoring into a second folder gives two definitions of one id and the engine silently shadows one. The per-prefix table is in [armory-guide](../reference/armory-guide.md).
...
5. **Never leave a backup ending in `.xml` in a registered folder.** The engine globs every `.xml` in the folder, so `body_armors.bak.xml` loads as real data and duplicates every id in it. Use a suffix that replaces the extension, `body_armors.xml.bak-<topic>-<date>`, per [module-backup-sweep](../reference/module-backup-sweep.md).
...
- **Vanilla armour is live and unmodifiable from here.** TAOM troops wear 32 `SandBoxCore` armour pieces, and `Main/_Module/ModuleData/` has no items XSLT, so those numbers cannot be overridden from the repo. Any claim that the armoury is the only armour tree the game loads is wrong.
