...
|---|---|---|---|---|---|
| `id` | string | Yes | none, the load throws | The kingdom's internal handle. Everything else points at it as `Kingdom.<id>`. Two entries with the same id are one kingdom, the second re-running over the first. | `MBObjectManager.cs:1391` |
...
5. Keep XML comments outside the `<relationships>` block. A comment inside it is iterated as if it were a row and throws.

Check: `python -c "import xml.etree.ElementTree as ET; ET.parse('Main/_Module/ModuleData/taom_spkingdoms.xml'); print('ok')"`
...
- **Two entries with the same id are one kingdom, and the second re-runs the deserializer over the first.** Every attribute the second entry omits reverts to its default rather than keeping the first entry's value: no `banner_key` gives you a random banner, no `color` gives you transparent black (`MBObjectManager.cs:1392-1393`, `Kingdom.cs:756-776`). Relationships and policies are the exception; they accumulate.
...
- **A misspelled attribute name is ignored in silence.** There is no schema: 0 `.xsd` files exist anywhere under the game's `Modules` folder <!-- measured: find . -name '*.xsd' 2026-09-05 -->, so `colour=` instead of `color=` gives you a black kingdom and no warning.
