...
| Gondor | — | Not catalogued yet (referenced in `taom_gondor_kitbash.xml`). 2026-07-28: added harbor ships `sm_gondor_ship_{cog,longship,war}_001` (Tripo AI sources, 1.9M→40k decimation + chart re-UV + high-to-low rebake) + `t_gondor_ship_<name>_{d,n,s}` under `Scenes/Gondor/ships/{,textures/}` — pipeline: [ue-to-bannerlord-asset-pipeline.md](../reference/ue-to-bannerlord-asset-pipeline.md) § Single-prop path. Also 2026-07-28: **Lond Cirion wall kit** started at `blockout/lond_cirion_wall_a.fbx` (`lond_cirion_wall_01` L-section, MT-template tiers base/.lod3/.lod6/bo_), assembled from `meshes/gondor_castle_{wall_L1,tower_L1_a}` pieces by `tools/oneoff/blender_assemble_lond_cirion_wall.py`. |
