...
- [reference/development-machines.md](reference/development-machines.md). **Two machines, and every `E:\` path in these docs is the desktop's.** Which machine is authoritative for content, the three env vars that carry the difference (`BANNERLORD_GAME_DIR`, `TAOM_DECOMPILE_ROOT`, `TAOM_PYBIN`), what ignores them (`.mcp.json`, the decompile scripts, the tracked `settings.local.json`), and why a red validator on the incomplete laptop is the laptop rather than the repo
