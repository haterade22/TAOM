...
- [ranged-ladders](features/ranged-ladders.md): an archer's reach is the bow's `missile_speed`, never the skill; every troop bow is a generated `ladder_<line>_<bow|xbow>_<band>` item from one grid (kingdom rank x tier band, `tools/ranged_ladders.json`), 1,741 inverted pairs to 0, gated by `RANGED_LADDER_INVERSION` (#582)
