...
| Adding a rideable creature mount end-to-end (assets → XML → C# → validation) | [ai-includes/creature-mount-authoring.md](ai-includes/creature-mount-authoring.md): elephant+spider-distilled; the 1.4.6 lookup-hardening rules + 18-gotcha index. A **reskin of a vanilla skeleton skips Phases 1 to 5 entirely** (no new rig, no usage sets; at most an `as_horse` child set and a clip for its own attack): worked example [features/war-ram.md](features/war-ram.md) |
| Bringing a bought quadruped pack (Fab, own rig, own clips) in as a horse-skeleton mount with its own animation | [ai-includes/quadruped-pack-to-horse-skeleton-workflow.md](ai-includes/quadruped-pack-to-horse-skeleton-workflow.md): the 13 stages from Fab to battle, who runs each, its tool and gate, and the gotchas the Animalia elk and moose hit (#646) |
...
- [war-ram](features/war-ram.md): the Dwarves' rideable war ram (#515) and **TAOM's first horse-skeleton reskin**. Both ram meshes and all eight bardings are skinned to the stock vanilla horse rig bone for bone, so the Monster is the `horse_2` shape (`base_monster="horse"` + its own `action_set` + four tuning attributes) and Phases 1 to 5 of [creature-mount-authoring](ai-includes/creature-mount-authoring.md) are skipped, except for one clip: the head-butt, authored on the engine `horse_skeleton` and bound as `act_war_ram_butt` in `as_war_ram`, a child of `as_horse` (2026-09-18). It inherits `Mountable`, `family_type=1`, `monster_usage="horse"` and **all twelve rein attributes**, the only TAOM mount with vanilla's complete rein surface, so it sidesteps that doc's gotcha #18 instead of adding to it. The trade: a reskin SHARES its action vocabulary with the engine, so "our code never fires this" stops implying "nothing fires this", which got the attack clip wrong twice. A second trade, found the same way: both body meshes are bare pelts, so the rider's seat exists only on the bardings and an empty `HorseHarness` renders a dwarf on bare hide, which shipped on `ironpass_ram_herder`. `MOUNT_WITHOUT_HARNESS` now requires a harness beside every `Horse` slot, because nothing in the XML records which mounts carry their own saddle. External data plane (untracked `LOTRLOME_Armory`): [lotrlome-war-ram-changes](reference/lotrlome-war-ram-changes.md)
- [animalia-elk-moose](features/animalia-elk-moose.md): two Fab packs (#646) as horse-skeleton reskins that keep their OWN animation. The meshes are bent onto `horse_skeleton` with the pack's weights kept (no donor transfer: the rig differs), and the pack's clips are retargeted through the same fit, so rest stays the horse rest. The moose keeps its short neck by a per-animal profile, at the cost of a head pivot 0.54 m off. Plays in game; its own antler attack; the moose carries Thranduil and the Mirkwood lords, the Animalia elk the lower cavalry and the `elk_rider` career start. External data plane: [lotrlome-animalia-changes](reference/lotrlome-animalia-changes.md)
- [monster-size](features/monster-size.md): a mount's size lives on its Monster as `taom_body_length`; TAOM copies it into every Horse item naming the Monster at game init, and opted-in creatures' reach follows the live size (#646).
- [elk](features/elk.md): the great elk (#636), the second horse-skeleton reskin, for Mirkwood's top cavalry troop (Thranduil, the lords and the `elk_rider` career moved to the Animalia moose and elk on 2026-09-23). It names the war ram's action set `as_war_ram`, so the ram's head-butt plays as an antler charge and the elk authored no animation data. The bare elk and its saddle are separate meshes, so every elk is paired with `taom_elk_saddle_a`. External data plane: [lotrlome-elk-changes](reference/lotrlome-elk-changes.md)
- [troll-brute-force](features/troll-brute-force.md): the trolls' area smash (#649), a per-troll behaviour tree that knocks down every enemy in a ring when the weapon lands; distances scale with the troll's body size (eye height).
- [troll-race](features/troll-race.md): the two troll races. `cave_troll` runs on `human_skeleton` with the 52 Fab "Cave Troll Lightweight" clips retargeted onto it (2026-09-18); `hill_troll` runs on its own `troll_skeleton_a` since 2026-09-24, the dwarf's layout: KEYForce's rig re-framed to the human's bone axes on export (so joints bend about the human's axes; human clips still keep the human's rest relations and are retargeted onto the hunched rest too), the human ragdoll and IK copied through the bone frames, the Fab set and the human masters the troll plays retargeted onto it with a two-bone leg IK, all ten skins and a standalone `as_hill_troll_warrior`. External data plane: [lotrlome-hill-troll-changes](reference/lotrlome-hill-troll-changes.md); the pipeline: [ue-to-bannerlord-asset-pipeline](reference/ue-to-bannerlord-asset-pipeline.md) § The retarget stage
...
- [starting-equipment-tuning](features/starting-equipment-tuning.md): every player-start item is a weak, cheap `starter_<donor>` twin. The damage and price formulas that decide the design (a blade's factor is proportional; price is 40% fittings, so crafted twins pin `value=`), the three roster layers including the vanilla-six override and where `_replaceWhileMerging` must sit, the generator and wiring tools, the restart rule
- [ranged-ladders](features/ranged-ladders.md): every troop bow is a generated `ladder_<line>_<bow|xbow>_t<tier>` item whose reach, damage and accuracy, and the troop's Bow skill, come from per-tier curves shifted by the kingdom's rank on each stat (`tools/ranged_ladders.json`); the Armory's own bows and ammo are restatted beside it; gated by `RANGED_LADDER_INVERSION` and `RANGED_DAMAGE_CEILING` (#582, #617); the tracked page `reference/ranged-troops.html` lists every ranged troop per kingdom with skill beside weapon; its melee sibling `reference/troop-rosters.html` lists every troop with every battle set, each slot priced (armour, melee damage and DPS, launcher damage, shield hit points), regenerated by `tools/generate_troop_roster_page.py`
- [melee-damage-model](features/melee-damage-model.md): Bannerlord simulates a crafted melee weapon's damage from its four pieces rather than storing it, so `tools/melee_damage.py` ports that physics from the v1.5.3 decompile and `tools/analyze_melee_ladder.py` prices all 364 Armory melee weapons and joins them to the troop rosters: the numbered `I/II/III` lines that go backwards, tier inversions, the vanilla ceiling per class, and the blade fan-out that constrains any restat (read-only, writes no ModuleData)
...
- [lord-identity-reconciliation](features/lord-identity-reconciliation.md): the two halves of a named lord (`characters/lords.xml` + `lords.xslt` say who he is, `characters/heroes.xml` + `heroes.xslt` say what the encyclopedia tells the player) and how they drift. Load order, the localization tiers (lord NAMES now interoperate across three tiers as of 2026-09-17; hero BIOGRAPHIES still don't), and the inherited-vanilla-attribute trap that married Gríma Wormtongue to Éowyn
...
- [reference/development-machines.md](reference/development-machines.md). **Two machines, and every `E:\` path in these docs is the desktop's.** Which machine is authoritative for content, the three env vars that carry the difference (`BANNERLORD_GAME_DIR`, `TAOM_DECOMPILE_ROOT`, `TAOM_PYBIN`), what ignores them (`.mcp.json`, `.codex/config.toml` and the decompile scripts), and why a red validator on the incomplete laptop is the laptop rather than the repo
...
- [bannerlord-skeleton-authoring](reference/bannerlord-skeleton-authoring.md): authoring clips for an ENGINE-native skeleton: the Kit stores FBX bone locals verbatim and turns the root 180 deg (measured 2026-09-18), so build the rig from the engine frames and turn the pose; the war ram's dead ends kept as history.
- [ue-to-bannerlord-asset-pipeline](reference/ue-to-bannerlord-asset-pipeline.md): UE / Fab kits and creature packs into Bannerlord: export, Blender normalisation, textures, materials, (2026-09-17/18) the Fab cave troll retarget, `_anm.tpac` clip and action-set bind stages, and (2026-09-23) a quadruped pack reskinned and retargeted onto `horse_skeleton` (Animalia elk and moose).
...
- [preload-body-guard](features/preload-body-guard.md): Patch90, the runtime guard for the #352 infinite mission load; drains unresolvable collision-body names out of `PreloadHelper.WaitForMeshesToBeLoaded`'s set with a bounded wait and logs them as `[PreloadGuard]` (#601)
- [armory-ref-audit](features/armory-ref-audit.md): `tools/audit_armory_refs.py` + `/armory-audit`: the one command after any Armory sync. Mesh and body refs over the whole Armory plus repo ModuleData, catalogue drift flagged from the LIVE refs, every broken item joined to the troops carrying it, dead item ids, generator drift; committed report `docs/audits/armory-ref-audit.md`. `validate_moduledata.py` carries the hang class as `MISSING_COLLISION_BODY` (#599)
...
- [doc-graph](features/doc-graph.md): query + audit *this* knowledge graph (`/doc-graph` skill + `tools/graph_query.py`): `explain` a doc's links, `path` between two docs, `metrics` (god nodes / bridges / orphans). [ADR-010](adrs/010-knowledge-base-architecture.md) Phase 5; adopted from graphify ([June review](reviews/adopt-graphify-2026-06-08.md), superseded by the [v8 trial](reviews/adopt-graphify-v8-2026-08-18.md)). Whole-codebase **C# structure** (blast radius, architectural hubs) is not this tool's job: that is the [graphify code graph](features/graphify-code-graph.md) (`tools/graphify_taom.py`, [ADR-012](adrs/012-graphify-code-graph-in-the-workflow.md))
...
- [creature-bandits-roadmap](research/creature-bandits-roadmap.md): riderless creature bandits (Design B, chosen, bandits only) with an invisible-rider fallback (Design A); engine facts, render gate, spike plan, rollout.
...
- [releases/2026-09-24-since-v2.0.28-discord.md](releases/2026-09-24-since-v2.0.28-discord.md): the v2.0.31 changelog for Discord, 12 to 24 Sep, three paste-ready messages (4,000 character cap) grouped by category rather than build (v2.0.29 and v2.0.30 shipped on the 1.5 beta; v2.0.31 carries all of it for Patreons). Supersedes the two-day post; every bullet traces to an issue or commit in the table under the post
- [releases/2026-09-13-two-day-discord.md](releases/2026-09-13-two-day-discord.md): 12 and 13 Sep changelog for Discord, one message under the 4,000 character cap, everything after the fortnight post: creature freezes, enlisted fixes, Smart Cavalry AI, nameplates, supply search, the kingdom armour curve, ranged ladders, the translation backlog, the texture downsizing. Nothing in it is released; every bullet traces to an issue or commit in the table under the post
- [releases/2026-09-12-fortnight-discord.md](releases/2026-09-12-fortnight-discord.md): 29 Aug to 12 Sep changelog for Discord, one message under the 4,000 character cap: v2.0.24 to v2.0.28 as released, the fixes on trunk waiting on an in-game smoke, and the Armoury and map work from lotraom-assets. Every bullet traces to an issue or commit in the table under the post; items the August and v2.0.25 posts already carried are left out
