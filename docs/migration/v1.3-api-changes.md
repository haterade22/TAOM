...
### Harmony Version
- 1.2: Harmony 2.2.x
- 1.3: Harmony 2.4.2 (bundled in `Bannerlord.Harmony` module v2.4.2.225)

### Notable Changes (2.3.3 → 2.4.2)

- **CodeMatcher API expanded** (2.3.6+): `RemoveSearchForward`, `RemoveSearchBackward`, `InsertAfter`, `Do` methods added. Consider for future transpiler patches instead of manual `List<CodeInstruction>` iteration.
- **AccessTools.TypeByName() performance improved** (2.3.6): Internal caching added. New `AccessTools.TypeSearch()` method.
- **Struct patching** (2.4.0): `[HarmonyPatch]` can now target methods on structs.
- **Configurable global error handler** (2.4.0): Register a centralized exception handler via Harmony API.
- **Bug fix: `HarmonyArgument` application** (2.3.4): Corrected attribute handling.
- **Bug fix: CodeMatcher infinite loop** (2.3.4): `MatchStartBackwards/Repeat` no longer loops.
- **Bug fix: value type vs object handling** (2.3.4): Improved type conversion.
- **`HarmonyException` no longer `[Serializable]`** (2.4.0): Not relevant to TAOM.
