...
## Dedicated-server deployment (`bin/Win64_Shipping_Server/`)

A Bannerlord dedicated server loads a module's assemblies from `bin/Win64_Shipping_Server/`, not `bin/Win64_Shipping_Client/`. Neither module shipped that folder until 2026-08-03, so a server logged `Cannot find: ...\TAOM\bin\Win64_Shipping_Server\TAOM.dll` and then ran a **vanilla simulation over TAOM's map** — no race capture, no War of the Ring, no campaign systems. Operators were hand-copying the DLLs.

`MirrorWin64ShippingClientToServer` now does it on every deploying build — one target in `Main/TAOM.csproj`, a sibling in `Dependencies/TAOM.Dependencies.csproj`. Both run `AfterTargets="PostBuildCopyToModules"` and copy the **assembled** `Modules/<id>/bin/Win64_Shipping_Client/` folder inside the game install, NOT the project's build output: the vendored natives and NuGet companions only exist after `PostBuildCopyToModules`, and that assembled set is exactly what the hand-copy recipe listed. `Main/TAOM.csproj`'s older `MirrorWin64ShippingClientToEditor` (modding kit → `Win64_Shipping_wEditor`) is the same shape — maintain the two together.

Maintenance consequences:

- **Nothing here is committed.** The mirror writes into the game install only. No repo `_Module/bin/Win64_Shipping_Server/` exists and none is needed, so the Category 4 `.gitignore` allowlist has nothing to say about it.
- **No deploy, no mirror.** The target's condition requires the deployed client folder to already exist, so a `-p:DisableModuleCopy=true` build produces no server folder — the same reason that build does not prove the deploy.
- **Copies are additive.** `SkipUnchangedFiles="true"` with no delete pass — drop a DLL from the client set and its stale copy survives in the server folder. Clear it by hand after removing a vendored binary.
- **The two flag files community server recipes ask for are obsolete.** Under a co-op module, PatchShield already skips install and SaveShield already rethrows save-load faults without any flag — see "Defensive infrastructure" below.
...
3. **Verify TAOM.Dependencies bin deployment:**
   ...
   - The sibling `bin\Win64_Shipping_Server\` should hold the same set (see "Dedicated-server deployment"). The build prints a `mirrored N files` line for each module.
...
- `PatchShield` — **biggest user-value**. Iterates every Harmony-patched method in the AppDomain, attaches a Finalizer to each non-TAOM patch. Catches the trinity (`MissingMethodException`, `MissingFieldException`, `TypeLoadException`) — the canonical errors when a mod was compiled against an old Bannerlord version. Auto-unpatches the offending owner's prefixes/postfixes/transpilers from the failing target. Re-run in `OnGameInitializationFinished` to catch late-registered patches. **Hot-target exclusion (2026-07-10, #331):** targets in `TaleWorlds.GauntletUI`/`TaleWorlds.TwoDimension`/`TaleWorlds.MountAndBlade.GauntletUI` are NEVER shielded (`ExcludedTargetNamespacePrefixes`) — the shield finalizer binds `__originalMethod`, so Harmony's wrapper pays `GetMethodFromHandle` + try/catch per CALL (~50µs); stacked on UIExtenderEx's prefab-system patches (`WidgetFactory.IsCustomType`, `WidgetTemplate.OnRelease`) it turned the tournament UI's ~10^6-call template release into a measured 104-109s frozen exit. Shield value in that layer is nil (its only patcher is BUTR's own UIExtenderEx); the third excluded prefix (`TaleWorlds.MountAndBlade.GauntletUI`) also covers TAOM's own per-frame widget targets like Patch38's nameplate fade (#331 round 2). Before shielding any new layer, cost the wrapper against the hottest conceivable target — see LESSONS-LEARNED "Blanket-patching infrastructure". **Co-op skip (2026-08-02):** `Install()` opens with `PatchShieldPolicy.ShouldInstall(CoopPresence.IsActive, IsDisabled())` and does nothing at all — both passes — when a co-op module is present. Same finalizer tax as #331, applied across BannerlordCoop's AutoSync surface (every declared method and constructor of 43 campaign types, patched on connect *before* our second pass), which a player traced to a frame-rate collapse. The unpatch half was already withheld under co-op by `ShouldUnpatchForeignOwners`, so this drops the remaining swallow half; the trade and its known cost are argued in [coop-interop.md](../features/coop-interop.md#patchshield-is-skipped-under-co-op). **Rethrow keeps the stack (2026-09-22):** everything outside the trinity is handed back to Harmony, which rethrows it with `throw`, and that used to replace the stack trace at every shielded method the exception crossed (player bundle `2d446100` reported a childbirth failure deep in the daily tick as five frames ending at `MapState.OnTick_Patch2`). Both finalizers now return through `RethrowStackPreserver.PreserveForRethrow` and count the rethrow; the session summary prints it as "rethrew N with the stack preserved", apart from the swallow counts, which is the measure of what the preservation costs in normal play. The no-exception path, which Harmony runs on every call, is still one null check. **Coverage is decided by when a pass runs:** pass 2 runs in TAOM.Dependencies' `OnGameInitializationFinished`, which the engine calls before TAOM's own, so TAOM's late-batch categories are patched after it and stay unshielded for a process's first game; pass 2 reruns on every game init and shields them from the second on. RCA: [rca-shield-rethrow-stack-2026-09-22.md](../reviews/rca-shield-rethrow-stack-2026-09-22.md).
  **Callback shims excluded (2026-09-23, plan 007):** the exclusion list also carries `ManagedCallbacks`, the engine's native-to-managed callback shims, of which TAOM's Native2Managed crash capture wraps the 16 on its allowlist (plan 006) with a finalizer; re-shielding them cost one `Harmony.Patch` per shim (247) at the first game start. The list now lives in `PatchShieldPolicy.ExcludedTargetNamespacePrefixes`. **Coverage given up:** the Native2Managed finalizer swallows only on its normal path. With crash capture off, on re-entry, or when the crash service is unresolved or throws, it hands the exception back with its throw site kept (plan 006's `HandBack`), and nothing on a shim then swallows the missing-API trinity; the 231 shims off the allowlist carry no TAOM finalizer at all. With Native2Managed capture off at launch the shims carry no TAOM finalizer at all (the flag only decides whether `AttachAll` runs), so only BEW's three transpiled shims lose PatchShield. A third-party prefix, postfix or transpiler on a shim also loses the rescue; the known patches there are TAOM's finalizers and ButterLib BEW's blank transpilers on three tick shims, a protected owner. **Reach:** the prefix is the whole namespace, 88 classes in v1.5.3: the three `*CallbacksGenerated` shims plus 79 managed-to-native `ScriptingInterfaceOf*` wrappers, `CallbackManager` and `ScriptingInterfaceObjects`, which Native2Managed does not wrap; those are excluded on the #331 per-call rationale alone. `PatchShieldPolicyTests.IsExcludedTargetNamespace_InstalledCallbackShimTypes_ReturnsTrue` (`BindingVerification`) fails if an engine bump moves the shims out of the namespace.
- `SaveShield` — targeted Finalizer on 10 specific TaleWorlds save/load/mission methods. Catches `DuplicateKey` + other save failures; stack-walks to attribute culprit assembly. Writes records to `failed-mods-catalog.txt` for user diagnosis. **Under co-op it still installs**, but `SaveShieldPolicy.ShouldSwallow` rethrows the `SAVE-LOAD` category instead of swallowing it — a half-deserialised campaign that keeps running is precisely what a host-authoritative mod then replicates as authoritative, so a loud load failure is cheaper than two silently corrupted saves. `MISSION-INIT` keeps swallowing in every case (local fault: one broken battle, not a corrupted campaign). The catalog entry is written on both paths. Every rethrow path returns through `RethrowStackPreserver.PreserveForRethrow`, as PatchShield's do; culprit attribution still happens once, at the innermost pass, because it walks frames and frames only ever cover the segment after the last throw.
...
| `saveshield-swallow-disabled.flag` | Install SaveShield BUT re-throw exceptions instead of swallowing — **all** categories, so it is still strictly stronger than the co-op default, which rethrows `SAVE-LOAD` only. Use when investigating which save/mission method is failing. |
| `coop-force-active.flag` | Force `CoopPresence.IsActive` true when detection missed a renamed or unknown co-op module. Consequence for this section: PatchShield stops installing and SaveShield stops swallowing `SAVE-LOAD`. Full description: [bannerlord-together-compat.md](../features/bannerlord-together-compat.md). |
...
> **A flag file is not the only way a shield goes quiet.** Since 2026-08-02, the mere presence of a co-op module suppresses PatchShield's install outright and switches SaveShield's `SAVE-LOAD` chain to rethrow — no flag involved. When diagnosing "the shield didn't run", read `diag.log` first: the skip is logged with which of the two reasons fired, and `CoopPresence` logs the module ids it found among the scanned active modules. `coop-modules.txt`, in the same directory, adds module ids the compiled defaults miss (it can only ADD — a bad edit cannot unprotect the BUTR/MCM stack).
...
In a co-op session the `[PatchShield] shield pass` line is absent by design — both passes bail — replaced by a single skip entry naming the active co-op module ids. That is healthy, not a failure; see the co-op skip above.

**Reading a stack trace that crossed a shield.** Lines of the form `--- End of stack trace from previous location (rethrown by a Harmony finalizer on <method>) ---` are expected, one per shielded method the exception passed through. Read top to bottom: the frames above the FIRST marker are the real throw site, and each marker names the patched method whose wrapper rethrew. The crash report's `Data` block repeats the throw site as `TAOM.ThrowSite`, and its `Stack Frames` / `Patches on throwing call stack` sections then say they cover the last segment only. One exception to the reading rule: ten TAOM feature finalizers (`Patch65`, `Patch88`, `Patch69` and others, listed in the RCA) still hand exceptions back without preserving, as do the crash reporter's own ten on their fallback paths, and on a process's first game, or with PatchShield uninstalled (co-op, the disable flag), nothing preserves for them. If one of those sat below the first marker, the frames above the marker start at that patched method, not at the real throw site. That is expected until those finalizers are converted, not a regression. (A method whose finalizers are all `void` keeps Harmony's `rethrow` and loses nothing.)
