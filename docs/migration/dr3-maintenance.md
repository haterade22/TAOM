...
- `PatchShield` — **biggest user-value**. Iterates every Harmony-patched method in the AppDomain, attaches a Finalizer to each non-TAOM patch. Catches the trinity (`MissingMethodException`, `MissingFieldException`, `TypeLoadException`) — the canonical errors when a mod was compiled against an old Bannerlord version. Auto-unpatches the offending owner's prefixes/postfixes/transpilers from the failing target. Re-run in `OnGameInitializationFinished` to catch late-registered patches. **Hot-target exclusion (2026-07-10, #331):** targets in `TaleWorlds.GauntletUI`/`TaleWorlds.TwoDimension`/`TaleWorlds.MountAndBlade.GauntletUI` are NEVER shielded (`ExcludedTargetNamespacePrefixes`) — the shield finalizer binds `__originalMethod`, so Harmony's wrapper pays `GetMethodFromHandle` + try/catch per CALL (~50µs); stacked on UIExtenderEx's prefab-system patches (`WidgetFactory.IsCustomType`, `WidgetTemplate.OnRelease`) it turned the tournament UI's ~10^6-call template release into a measured 104-109s frozen exit. Shield value in that layer is nil (its only patcher is BUTR's own UIExtenderEx); the third excluded prefix (`TaleWorlds.MountAndBlade.GauntletUI`) also covers TAOM's own per-frame widget targets like Patch38's nameplate fade (#331 round 2). Before shielding any new layer, cost the wrapper against the hottest conceivable target — see LESSONS-LEARNED "Blanket-patching infrastructure". **Co-op skip (2026-08-02):** `Install()` opens with `PatchShieldPolicy.ShouldInstall(CoopPresence.IsActive, IsDisabled())` and does nothing at all — both passes — when a co-op module is present. Same finalizer tax as #331, applied across BannerlordCoop's AutoSync surface (every declared method and constructor of 43 campaign types, patched on connect *before* our second pass), which a player traced to a frame-rate collapse. The unpatch half was already withheld under co-op by `ShouldUnpatchForeignOwners`, so this drops the remaining swallow half; the trade and its known cost are argued in [coop-interop.md](../features/coop-interop.md#patchshield-is-skipped-under-co-op). **Rethrow keeps the stack (2026-09-22):** everything outside the trinity is handed back to Harmony, which rethrows it with `throw`, and that used to replace the stack trace at every shielded method the exception crossed (player bundle `2d446100` reported a childbirth failure deep in the daily tick as five frames ending at `MapState.OnTick_Patch2`). Both finalizers now return through `RethrowStackPreserver.PreserveForRethrow` and count the rethrow; the session summary prints it as "rethrew N with the stack preserved", apart from the swallow counts, which is the measure of what the preservation costs in normal play. The no-exception path, which Harmony runs on every call, is still one null check. **Coverage is decided by when a pass runs:** pass 2 runs in TAOM.Dependencies' `OnGameInitializationFinished`, which the engine calls before TAOM's own, so TAOM's late-batch categories are patched after it and stay unshielded for a process's first game; pass 2 reruns on every game init and shields them from the second on. RCA: [rca-shield-rethrow-stack-2026-09-22.md](../reviews/rca-shield-rethrow-stack-2026-09-22.md).
  **Callback shims excluded (2026-09-23, plan 007):** the exclusion list also carries `ManagedCallbacks`, the engine's native-to-managed callback shims, which TAOM's Native2Managed crash capture already wraps with a finalizer; re-shielding them cost one `Harmony.Patch` per shim (247) at the first game start. The list now lives in `PatchShieldPolicy.ExcludedTargetNamespacePrefixes`. **Coverage given up:** the Native2Managed finalizer swallows only on its normal path. With crash capture off (or Native2Managed capture off), on re-entry, or when the crash service is unresolved or throws, it hands the exception back, and nothing on a shim now swallows the missing-API trinity or preserves the stack (Harmony rethrows a non-void finalizer's result with `throw`). A third-party prefix, postfix or transpiler on a shim also loses the rescue; the known patches there are TAOM's finalizers and ButterLib BEW's blank transpilers on three tick shims, a protected owner. **Reach:** the prefix is the whole namespace, 88 classes in v1.5.3: the three `*CallbacksGenerated` shims plus 79 managed-to-native `ScriptingInterfaceOf*` wrappers, `CallbackManager` and `ScriptingInterfaceObjects`, which Native2Managed does not wrap; those are excluded on the #331 per-call rationale alone. `PatchShieldPolicyTests.IsExcludedTargetNamespace_InstalledCallbackShimTypes_ReturnsTrue` (`BindingVerification`) fails if an engine bump moves the shims out of the namespace.
