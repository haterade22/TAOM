...
- [armory-ref-audit.md](armory-ref-audit.md): GENERATED by `tools/audit_armory_refs.py`, re-run after any Armory sync (`/armory-audit`); the committed state of every mesh and collision-body ref, catalogue drift, dead item ids and generator drift. Its `git diff` is the change log. Feature doc: [armory-ref-audit.md](../features/armory-ref-audit.md) (#599).
