...
| 2 | P2 | SmartCavalryAI (self) | `CavalryChargeService._states` dict | `CavalryChargeService.cs:33` | Lock-free Dictionary; sibling `FormationLayoutService._layoutByFormation` holds explicit lock. Asymmetric threading posture; latent race if `SetMovementOrder` ever fires off-thread. | #170 **RESOLVED 2026-09-13** (verified: `CavalryChargeService.cs:52` holds `_lock`; #595 audit). |
...
| 21 | P2 | CareerSystem (cache) | Plain Dict race vs RefreshCache | `CareerPassiveService.cs:11-12, 21, 72` | `_cache.Clear()` + rebuild while `GetPassiveMagnitude` may be invoked from AI worker threads (`GetTotalWage` desertion path, `GetPartyMemberSizeLimit` party-size). | #173 **RESOLVED 2026-09-13** (verified: snapshot-swap under `_lock`, `CareerPassiveService.cs:17-19`; #595 audit). |
