...
| `action_sets.xml`, `as_hill_troll_warrior`, swings (2026-09-25) | The 32 `act_release_*` and `act_quick_release_*` codes bound to retargeted `anim_hill_troll_*` clips went back to the vanilla clips: melee is engine pose-blend, the swing looks up per-clip pose data by the vanilla clip's index, and a retargeted clip's index is missing, so the first swing crashed (`TaleWorlds.Native.dll` +0x6590B9). `bind_hill_troll_action_set.py` rule 0 keeps every swing vanilla; backup `action_sets.xml.bak-hilltroll-bind-20260925-180817`. |
