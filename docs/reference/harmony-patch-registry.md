...
**Target:** `CharacterTableau.OnTick(float)` (public, Postfix) + `CharacterTableau.SetRace(int)` (public, Postfix). Both in `TaleWorlds.MountAndBlade.View.dll`, which ships in `Modules\Native\bin\Win64_Shipping_Client` — **not** the primary `bin\`.

**Feature:** HeroRace instrumentation for #389 — `Main/Features/HeroRace/Hooks/CharacterTableau_ResidencyDiagnostics_Patch.cs`. **Status:** ACTIVE, DIAGNOSTIC-ONLY (applied in the isolated character-preview batch in `OnBeforeInitialModuleScreenSetAsRoot`).

Isengard `uruk_hai`/`berserker` troops render as pure black silhouettes in the encyclopedia while every other race renders correctly; root cause is undetermined and every offline-inspectable layer checks out clean. This category makes the render path state its own outcome instead of requiring a human to compare pixels.

**Why the loading counter is the signal.** Vanilla `OnTick` decrements `_agentVisualLoadingCounter` only when `_agentVisuals.GetEntity().CheckResources(true, true)` returns true, and shows the refreshed visual only once that counter reaches zero. So "counter never reaches zero" is an exact proxy for "this character's meshes/materials never became resident" — at **zero extra native cost**, because vanilla already made the call whose answer we read. The patch deliberately never calls `CheckResources` itself: that would add a per-frame native round-trip AND queue resources as a side effect, i.e. a diagnostic that changes the thing it measures. Meanwhile `RefreshCharacterTableau` makes the *other* buffer visible with no resource gate at all, and with `SetSceneUsesSkybox(false)` + `SetClearColor(0u)` an unresolved visual reads as an opaque black silhouette.

**Reading the log.** One verdict per troop, keyed on `_charStringId` (`urukhai_fighter`, not `race=5` — a race-level key would collapse every troop of a race into one line and lose the comparison). `Residency STUCK` is logged at ERROR and is the smoking gun; `Residency RESOLVED after N tick(s)` at INFO. If `urukhai_fighter` reports STUCK while `isengard_orc_ravager` reports RESOLVED, per-race resource residency is the mechanism. **If both RESOLVE, residency is exonerated and the blackness is happening downstream of it** — that negative result is as decisive as the positive one.

Verdict logic lives in the pure, 100%-tested `TableauResidencyTracker` (`TableauResidencyTrackerTests`, 16 cases) so the patch stays a thin entry point; the tracker caps at 64 tracked characters because browsing a troop tree walks hundreds, and reports once per character because the failing state repeats every frame (a 6.4 MB session log already happened once — commit ae2ed426). `SetRace` drops the character's verdict because a race change tears down and rebuilds both visuals, genuinely restarting residency.

**Deliberately its own category, not folded into Patch2.** The #389 experiment calls for disabling `Patch2_RefreshTableau` + `Patch3_SetRace` to test whether TAOM's own patches cause the fault; the instrument has to keep reporting while that A/B runs. All six private-field injections plus both method targets are pinned by `Patch67TableauResidencyBindingTests` (`TestCategory=BindingVerification`) — on engine drift Harmony throws while applying the category, which the isolated batch logs rather than crashes, so without the test the instrument would go silently dead exactly when a bump made it most necessary.

**Remove this category once #389 is root-caused.** It is scaffolding, not a feature.
