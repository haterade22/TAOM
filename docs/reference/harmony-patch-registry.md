...
Trolls stood inside each other in formation (Mike, 2026-09-25). The engine spaces every foot unit for a 0.76 m human: `UnitDiameter` is `GetDefaultUnitDiameter`, `BipedalRadius` (0.38, `managed_core_parameters.xml`) times two, whatever the Monster. The postfix raises it to the width `TrollFormationSpacingTracker` stored for the formation once trolls are a tenth of it, vanilla's share for horses (`CalculateHasSignificantNumberOfMounted`): the widest troll's measured shoulder width times its `AgentScale` (`TrollBruteForceConfig.ShoulderWidthByMonster`: cave troll 0.75 m, so about 1.43 m at 1.9; hill troll 2.43 m, so about 2.7 m at 1.11; capped at 4 m). A first cut used 0.76 m times body size, which fits the cave troll but gave the hill troll, twice as broad for its height, 1.78 m. `LineFormation` reads `owner.UnitDiameter` for every slot, file count and rank depth; MixedFormations reads it through `IFormationAdapter.UnitDiameter` (`LayoutPositioner.UnitPitch`).

The order preview, the deployment placement and the spawn frames lay a formation out on a team-less copy (`new Formation(null, -1)`), which no store holds, so the first build still placed trolls at human width and their capsules pushed them to touching (every neighbour 1.6 m apart). The scope patch names the real formation while its layout runs, and the postfix gives a team-less copy that formation's width; `Patch92BindingTests` pins the four targets. The store is keyed by reference: `Formation.GetHashCode` is `Team.TeamIndex * 10 + FormationIndex` and threw a NullReferenceException on the preview's copy in the first build. The tracker refreshes from `TrollBruteForceMissionBehavior.OnMissionTick` twice a second on the main thread and re-forms a formation whose width changed through `Formation.OnUnitAddedOrRemoved` with its cached slots marked dirty (`LineFormation` rebuilds them only on request), and during deployment re-deploys it the way the Deploy button does (`DeploymentHandler.OrderController_OnOrderIssued_Aux(Move)`; `IsTeleportingAgents` is on for the whole field deployment), because the trolls were placed at human width before their width was known and nothing else moves them; the getter also runs on the AI thread and TWParallel workers, so the postfix reads only a concurrent map and swallows any throw. Applied in `OnSubModuleLoad` after Patch30. **ACTIVE** (2026-09-25).
