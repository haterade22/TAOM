...
**Target:** `KingdomDecisionsVM.RefreshWith(KingdomDecision)` (Prefix) + `KingdomDecisionsVM.HandleDecision(KingdomDecision)` (Postfix) + `DecisionItemBaseVM.ExecuteFinalSelection()` (Postfix, `Priority.Last`)

**Feature:** Diplomacy, `Main/Features/Diplomacy/Hooks/`. **Status:** ACTIVE. Issue #547.
...
**Seam B closes through `_onDecisionOver`, not `ExecuteDone`.** `ExecuteDone` opens with `KingdomDecisionMaker.GetChosenOutcomeText()`, which dereferences the election's `_chosenOutcome`, null on a cancelled election. Calling it would trade the hang for an NRE. `Patch80KingdomVoteDeadlockBindingTests` pins that premise, along with `ApplySelection` still reading `IsCancelled`, both patch-target parameter names (Harmony binds by name), and the four private members reached by reflection.
