...
**Targets (6 postfixes):** `PartyUpgraderCampaignBehavior.UpgradeReadyTroops` (enforcement: shed-on-upgrade) · `PartyVM.RefreshPartyInformation` · `CampaignUIHelper.GetMainPartyHealthTooltip` · `ClanPartyItemVM.UpdateProperties` · `RecruitmentVM.RefreshPartyProperties` · `PartyCharacterVM.RefreshValues` (the last five are display-only)
