...
**Feature:** HeroRace instrumentation for #389 — `Main/Features/HeroRace/Hooks/CharacterTableau_ResidencyDiagnostics_Patch.cs` + `Diagnostics/TableauRenderCensus.cs`. **Status:** ACTIVE, DIAGNOSTIC-ONLY (applied in the isolated character-preview batch in `OnGameInitializationFinished`).

Isengard `uruk_hai`/`berserker` troops render as pure black silhouettes in the encyclopedia while every other race renders correctly; root cause is undetermined and every offline-inspectable layer checks out clean. This category dumps **one render census per character** — actual mesh names, material names, shader names, shader flags, per-mesh vertex colours (`Mesh.Color`/`Color2`) and the bound diffuse texture — so the cause can be named by diffing an affected troop against a working one, instead of a human comparing pixels.
