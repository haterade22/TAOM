...
| `TaomVolunteerModel` | `DefaultVolunteerModel` | Four overrides, not two. `MaxVolunteerTier => 6` (vanilla 4, so notable slots stop promoting at level 31-35). `GetBasicVolunteer` routes the ENTIRE LOTR recruitment-pool system through `IVolunteerRecruitmentService`, falling back to the culture's `basic_troop` only when the pool returns null or an unresolvable id. `GetDailyVolunteerProductionProbability` applies the culture respawn feats. `MaximumIndexHeroCanRecruitFromHero` returns -1 to block recruiting at an enemy-aligned settlement (AlignmentRecruitment feature) |
...
| `TaomPartyTroopUpgradeModel` | `DefaultPartyTroopUpgradeModel` | `GetGoldCostForUpgrade`: mounted recruit cost feats (Isengard, Rohan) + career TroopUpgradeCost passive. `GetXpCostForUpgrade`: floors a zero cost so a same-tier upgrade cannot divide by zero in `GetTroopXPTooltip` (#537); the career passive and the feats are gold-only and deliberately do not apply here |
| `TaomPartySizeModel` | `DefaultPartySizeLimitModel` | Party size feats (all 12 cultures `ApplyPartySizeFeats` dispatches on: Mordor, Gundabad, Goblin, Blue Craig, Misty Mountain Orcs, Dol Guldur, Isengard, Gondor, Dunland, Rhûn, Harad, Khand) + career PartySize passive + **TroopWeight elite-tax limit deflation** (2026-07-11 count→limit rework: counts read raw, the LIMIT shrinks, but since 2026-09-06 the UI *renders* that as capacity used, `19 / 20` not `10 / 11`, and this override forwards `includeDescriptions` so the tooltip path takes no penalty at all). Plus **AI lord scaling and a `CalculateGarrisonPartySizeLimit` override** (#461, `IAiPartySizeService`) that, once raised through MCM (ships NEUTRAL at 1.0x / +0), lets AI lords hold their spawned roster; it MUST run before the TroopWeight line, which caches the pre-deflation base the daily shed trims to. All of these land on ONE `ExplainedNumber`, so a heavy roster's tax can cancel a small culture bonus, and any FLAT contributor has to add in the result frame or the factors amplify it. See `docs/features/troop-weight-system.md` and `docs/features/cultural-feats.md` "Evil-culture party-size floor" |
...
| `TaomMarriageModel` | `DefaultMarriageModel` | **TWO rules in the mod's only `MarriageModel` slot.** `AddModel` does not compose (backwards scan resolves exactly one), so a second marriage model would silently shadow one of them; any future marriage rule goes in this class. (1) NazgulFamily: the 9 Ringwraiths are marriage-ineligible (`IsSuitableForMarriage` + `IsCoupleSuitableForMarriage` false for wraiths). (2) MarriageAlignment #542: `IsCoupleSuitableForMarriage` also refuses a Free/Evil pairing, keyed on CULTURE via `IMarriageAlignmentService`; it is the single chokepoint for the AI tick, player courtship, the barter and `MarriageAction`. `IsSuitableForMarriage` stays wraith-only (single-hero predicate, cannot express a pair rule). Everything else falls through to vanilla. See `docs/features/nazgul-family.md` + `docs/features/marriage-alignment.md` |
