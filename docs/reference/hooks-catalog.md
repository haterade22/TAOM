...
> opens all eleven hooks: most often literal `\u` text in a command (a search for `\u2014`
> arrives as `\\u2014`), and in the PostToolUse hook a tool response with colour codes
> (`\u001b`). 4c feeds each of the eleven hooks its word with one letter escaped, and 4d checks
> that five of the nine blocking gates (`check-commit-subject-version.sh`, `validate-push.sh`,
> `block-no-verify.sh`, `block-dangerous-git.sh`, `block-broad-git-add.sh`) answer the escaped
> form as they answer the plain one. After a Claude Code upgrade,
...
| `config-protection.sh` | PreToolUse (Edit\|Write) | Blocks edits to Directory.Build.props, settings*.json and any `.md` under `docs/adrs/` without explicit request. The path is normalised first (backslashes to `/`, case folded) because the Edit and Write tools pass Windows paths: until 2026-09-26 the ADR check matched only `/docs/adrs/` and never fired on the dev machine (`tools/test_hooks.sh` 7f holds every spelling). Override on an explicit request: the flag file `/tmp/claude-config-override-${CLAUDE_SESSION_ID:-none}`, deleted after the edit. CLAUDE.md removed from the protected list 2026-07-02 (user decision: solo dev; the agent maintains CLAUDE.md as living documentation) |
| `check-graphify-usage.sh` | PreToolUse (Bash, PowerShell) | Denies raw `graphify` write and install verbs (`extract`, `update`, `cluster-only`, `label`, `install`, `hook`, `watch`, `graphify-mcp`) and names `tools/graphify_taom.py` instead; the read-only query verbs and commands that only mention graphify pass. The judge is `tools/graphify_taom.py gate` (quote-aware per shell, unwraps `bash -c` / `pwsh -Command`), bounded at 5 s inside a 10 s registration; no jq path, so it is named in the degraded banner. ADR-012, #677; `tools/test_hooks.sh` 7e |
