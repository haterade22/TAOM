...
| `notify-test-results.sh` | PostToolUse (Bash) | Summarizes `dotnet test` results to stderr (`TEST RESULTS: PASSED`, `PASSED WITH SKIPS` or `FAILED`, with counts; a non-zero skip count from the first summary line is shown). The hook exits 0, so the banner reaches the debug log only and Claude never sees it: read the `Skipped:` count from the tool result. |
