...
| `notify-test-results.sh` | PostToolUse (Bash) | Summarizes `dotnet test` results to stderr (`TEST RESULTS: PASSED/FAILED` with counts). The hook exits 0, so the banner reaches the debug log only and Claude never sees it. It does not report skips: a skipped test checked nothing, so read the `Skipped:` count in `dotnet test`'s own output. The binding gate is strict through `TAOM.Tests/binding-gate.runsettings`, which fails a skipped test and a filter that matches no test. |
