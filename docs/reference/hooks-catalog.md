...
| `config-protection.sh` | PreToolUse (Edit\|Write) | Blocks edits to Directory.Build.props, settings*.json and any `.md` under `docs/adrs/` without explicit request. The path is normalised first (backslashes to `/`, case folded) because the Edit and Write tools pass Windows paths: until 2026-09-26 the ADR check matched only `/docs/adrs/` and never fired on the dev machine (`tools/test_hooks.sh` 7g holds every spelling). Override on an explicit request: the flag file `/tmp/claude-config-override-${CLAUDE_SESSION_ID:-none}`, deleted after the edit. CLAUDE.md removed from the protected list 2026-07-02 (user decision: solo dev; the agent maintains CLAUDE.md as living documentation) |
| `check-graphify-usage.sh` | PreToolUse (Bash, PowerShell) | Keeps every graphify write going through `tools/graphify_taom.py` (ADR-012, #677). An allow-list: the query verbs, `diagnose`, `benchmark`, `help` and `version` pass; any other `graphify` verb (`extract`, `update`, `cluster-only`, `label`, `watch`, the install verbs, ...) and `graphify-mcp` are denied with the wrapper named instead, and a command that only mentions graphify as data passes. The judge is `tools/graphify_taom.py gate` (quote-aware per shell, unwraps `bash -c` / `pwsh -Command`), so it does not read through `taom_hook_command`; it is bounded at 5 s inside a 10 s registration and answers `ask` on an overrun. With no safe Python, or a failed judge, it fails open and says so (no jq path, so it is named in the degraded banner). `tools/test_hooks.sh` 7f; the case table is `tools/tests/test_graphify_taom.py` ([code graph](../features/graphify-code-graph.md)) |
