...
| `check-graphify-usage.sh` | PreToolUse (Bash, PowerShell) | Denies a raw graphify write verb (`extract`, `update`, `cluster-only`, `label`, the install verbs) and `graphify-mcp`, so every write goes through `tools/graphify_taom.py` (#677); allows the query verbs and a command that only mentions graphify as data. Its judge is `tools/graphify_taom.py gate`, which splits both shells itself, so it does not read through `taom_hook_command`. No jq path: with no safe Python it fails open and says so. `tools/test_hooks.sh` 7f; the case table is `tools/tests/test_graphify_taom.py` ([code graph](../features/graphify-code-graph.md)) |
