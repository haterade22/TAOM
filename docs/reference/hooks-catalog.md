...
| `notify-test-results.sh` | PostToolUse (Bash) | Summarizes `dotnet test` results prominently to stderr (`TEST RESULTS: PASSED`, `PASSED WITH SKIPS` or `FAILED`, with counts; a non-zero skip count is always shown) |
