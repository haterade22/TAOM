...
| TAOM `characters/spider_creature.xml` (repo, not this module): the Spider Rider becomes a three-rung line, L21 Giant Spider, L26 Brown Spider, L31 Pale Spider, with weight and War Spoils rows for the rungs | the warg line's shape; size per rung is the item's `body_length` above | git |
