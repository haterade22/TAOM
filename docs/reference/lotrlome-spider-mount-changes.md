...
| **9 `_anm.tpac` files byte-patched onto ADOD_Beasts's elephant templates** — `an_spi_walk_2`, `walk_left`, `walk_right`, `idle` (file `new_animation_clip_3`), `idle2` (4), `turn_left` (5), `turn_right` (6), `jump` (7) on the *canter* template; `an_spi_run` on the *gallop* template (adds `cyclic` + gallop speed params, since run serves paces 3–5) | **THE ROOT CAUSE of every mount-context AV:** the clips were Kit-compiled without the `quad_movement` clip usage + step points that every working quadruped's gait clips carry. A `movement_system="quadrupedal"` action set measuring untagged gait clips builds a null native gait structure → AV (`+0x10`) on the first `Skeleton.TickAnimations` in thumbnails, inventory tableaus, AND missions. The patch grafts ADOD_Beasts's proven post-name layout (tags, step points, params) while keeping each clip's own GUIDs/name/duration — full recipe in spider.md "How-to"; Kit-editor field locations in the pipeline doc §3c |
| NOT modified: the 12 remaining clip pairs (deaths, hits, directional attacks, charge…) | Byte-diff proved them structurally identical to the playback-proven files; non-gait clips correctly do NOT carry `quad_movement` (ADOD_Beasts's attacks/deaths don't either — they carry `lock_movement`/`make_bodyfall_sound`-class polish flags, a Kit-recompile polish item) |
...
| `ModuleData/LOTRLOME_items/LOTRAOM_horses.xml`: `spider_mount_brown` (mesh a, additional `_a_2`, "Brown Spider", `body_length` 110) and `spider_mount_pale` (mesh b, `_b_2`, "Pale Spider", `body_length` 125) after `spider_mount_a`; same Horse block otherwise (Monster.spider, 85/65/15), no `<Materials>` | Warg-shaped items (`warg_brown` / `warg_dark` / `warg_albino`): one id per skin so rosters can name them; `body_length` is the engine's mount scale (`Mission.cs:4056`, `0.01f * BodyLength`), so the two upgrade rungs read 10 and 25 percent bigger | remove the two `<Item>` blocks |
...
| TAOM `characters/spider_creature.xml` (repo, not this module): the Spider Rider becomes a three-rung line, L20 Giant Spider, L25 Brown Spider, L30 Pale Spider, with weight and War Spoils rows for the rungs | the warg line's shape; size per rung is the item's `body_length` above | git |
