...
- **Decorator/wrapper (`AddModel<T>` + `MBGameModel<T>`):** `gameModel.Initialize(GetModel<T>())` hands the *previous*
  model to the new one so it can delegate. **ADOD used this** (its `ADODAgentStatCalculateModel` wraps the previous
  model); TAOM prefers inheritance + `base` (simpler — see the [ADOD comparison](../adod-beasts-architecture-and-taom-port.md)).
