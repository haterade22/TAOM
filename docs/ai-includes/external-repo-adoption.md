...
2. **Security pass FIRST** (standing requirement — gating). Pull the runnable surface verbatim and read it:
   ...
   - Before porting any file from a vendored drop, sweep it for inline package credentials, archives included, because grep cannot see inside a `.tar.gz` (extract a `.zip` or `.nupkg` first): `grep -rlE 'packageSourceCredentials|ClearTextPassword|<password|<token' <vendor-dir>`, then `for a in <vendor-dir>/*.tar.gz; do tar -xzOf "$a" | grep -qE 'packageSourceCredentials|ClearTextPassword|<password|<token' && echo "$a"; done`. The same `-E` pattern works in the Grep tool, where `\|` would match a literal pipe. Scrub any hit, and tell upstream when the token is theirs ([plan 005](../../plans/005-security-hygiene.md); the second `[SEC-02]` in `plans/_audit/2026-06-12-harvest.md`).
