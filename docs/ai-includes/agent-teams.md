...
## Case studies (migrated from harness-facts.md, 2026-07-12)

The distilled rules live in `.claude/rules/harness-facts.md` ("Worktree isolation for parallel agent
runs" + "Parallel builder briefs"); these are the full incident write-ups behind them.
...
### Parallel builder briefs: the CombatMechanics seam findings (EMPIRICAL: TAOM 2026-07-02)

The rule ("any sub-problem appearing in >=2 briefs gets ONE prescribed solution in the shared contracts") came from four seam divergences shipped by independently-correct builders:
...
All four seam findings were caught by the cross-cutting review agents (data-flow + efficiency), none by per-component work. RCA: `docs/reviews/rca-combat-mechanics-2026-07-02.md`; LESSONS-LEARNED "Build, Tooling & Workflow".

### Worktree isolation: sources + invocation detail
...
If two parallel Agent calls in the same message both need to edit a single-owner file, BOTH must use `isolation: "worktree"`. After they return, the orchestrator sequentially merges the diffs from each worktree branch back into the main tree — the watcher cascade only triggers on simultaneous edits to the same working tree, not on sequential merges. Housekeeping: prune stale worktree checkouts under `.claude/worktrees/` once their branches are merged or abandoned (`git worktree list` / `prune` — 4 forgotten trees cost 22 GB by 2026-07-11).
