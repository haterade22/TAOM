...
| Browse engine source for patterns | `Read`/`Grep` under `E:\Decompiled_Bannerlord\` | **v1.5.2** dump (`_*_v1.4.8`, `_*_v1.5.0` baselines beside it). Fine for browsing; for authoritative signatures still prefer `taom-src`. ⚠️ SHIPPING-CLIENT build: editor-only types (`MBEditor`, `AnimalSpawnSettings`, FBX toolchain) only exist in `_editor_build\`. "Absent from dump" ≠ "doesn't exist." |
...
| API signature snapshot | `pwsh tools/snapshot_api_surface.ps1 [-Check]` | Regenerate / verify the committed signature snapshot (self-labels the installed version, currently v1.5.2). |
...
**Target engine version is Bannerlord v1.5.2** (Steam beta branch, since 2026-09-14; branch `bannerlord-1.5.x`). Anything in an agent prompt or doc that still names "v1.3.15" / "v1.4.5" / "v1.4.6" / "v1.4.7" / "v1.4.8" as the *current* target is stale; trust the installed DLLs + `taom-src` (which auto-detects the version). **This rule is broader than the checker that enforces it.** Since #399 `lint_docs.py` fires only on marker-word phrasing (`current` / `target` / `now` / `builds against` / …), and its patterns cover `1.3.15` / `Bannerlord 1.3` / `v1.3.x` only; v1.4.5 through v1.4.8 are not matched at all. A clean doc-health run does not mean the rule above holds; read for it. Gaps tracked in [#405](https://github.com/haterade22/TAOM/issues/405).
