...
**Target engine version is Bannerlord v1.5.0** (installed, Steam beta branch). Anything in an agent prompt or doc that still names v1.3.15 / v1.4.5 / v1.4.6 / v1.4.7 / v1.4.8 as the target is stale. One deliberate exception: the Modding Kit (`bin/Win64_Shipping_wEditor`) really is still v1.4.8, so a doc naming that for EDITOR offsets is correct.
