...
**Target engine version is Bannerlord v1.4.7.** Anything in an agent prompt or doc that still names "v1.3.15" / "v1.4.5" / "v1.4.6" as the *current* target is stale — trust the installed DLLs + `taom-src` (which auto-detects the version). **This rule is broader than the checker that enforces it.** Since #399 `lint_docs.py` fires only on marker-word phrasing (`current` / `target` / `now` / `builds against` / …), and its patterns cover `1.3.15` / `Bannerlord 1.3` / `v1.3.x` only — v1.4.5 and v1.4.6 are not matched at all. A clean doc-health run does not mean the rule above holds; read for it. Gaps tracked in [#405](https://github.com/haterade22/TAOM/issues/405).
