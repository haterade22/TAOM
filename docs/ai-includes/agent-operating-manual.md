...
| **C# code graph (blast radius)** | `python tools/graphify_taom.py affected "<Type>" --depth 2` (+ `explain`, `path`, `god-nodes`, `query`; `refresh --if-stale` first) | Dependents of a TAOM type with file and line, from graphify's code graph kept outside the repo. On "Ambiguous", rerun with the repo-relative `.cs` path it lists. Never run `graphify` directly: a hook denies its write verbs. No XML, and engine types are per-file nodes. Full ref: [`docs/features/graphify-code-graph.md`](../features/graphify-code-graph.md). |
