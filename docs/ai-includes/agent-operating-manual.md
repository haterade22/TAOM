...
| **Build** | `dotnet build Main/TAOM.csproj -p:DisableModuleCopy=true -p:ModuleId=` | Use this, NOT `./build.ps1`, during agent work (avoids `out/` contention). `-p:ModuleId=` is what actually stops the copy into the game; see the caveat below. |
