...
| 7 | **Editor-assigned materials.** An FBX carries only the material slots the DCC tool assigned; anything bound in the editor lives in the compiled tpac alone and does not survive a re-import | two symptoms that look unrelated: every colour variant renders as the base material, and the creature is absent in the world while a close-up UI preview looks correct, because the lost bindings sit one per LOD | diff the mesh-to-material bindings against the donor tpac before trusting the import. Resolve every 16-byte guid inside each mesh item against the material items in the tree, then compare the two sets per mesh | re-assign in the Modding Kit, per mesh per LOD. **Patching it from files needs the checksum recomputed:** adding a binding grows the item's metadata, and the item checksum is xxHash64 (seed 0) over the int64 metadata length plus the metadata (known since 2026-09-17, `tools/tpac_clone_metamesh.py`); the 2026-08-29 warg fix predates that and was made in the Kit. Warg 2026-08-29, [lotrlome-warg-changes.md](../reference/lotrlome-warg-changes.md) section 12 |
| 7b | **Colour variants through `<Materials>` on the Horse item need a `horse_body` sub-mesh tag.** `MountVisualCreator.SetMaterialProperties` (TaleWorlds.MountAndBlade.View) applies the chosen material with `SetMaterialToSubMeshesWithTag(mat, "horse_body")` on the BASE mesh and hands `AdditionalMeshes` only a colour factor | a `<Materials>` block on a creature authored without the tag does nothing, silently, and an `<AdditionalMeshes>` half never recolours either way (the spider, 2026-09-17: 0 `horse_body` hits in its tpac; Byak0's `warg_low` carries 3, which is why `warg_dark` works) | count `horse_body` in the mesh tpac before writing the block | tag the sub-meshes in the Kit, or clone the proven mesh per skin from files with `tools/tpac_clone_metamesh.py` (same-length name + material swap, fresh GUIDs, geometry verbatim, own tpac, and BOTH tpac hashes recomputed: the per-segment field is xxHash64 (seed 0) of the decompressed payload and the item checksum is xxHash64 of the sized metadata; then open the module in the Modding Kit and save so it cooks `RuntimeDataCache/<package GUID>.rdc`, because the client renders only cached packages and skips the rest with nothing in the log; the spider's `spider_variants_geo.tpac`). See [lotrlome-spider-mount-changes.md](../reference/lotrlome-spider-mount-changes.md) |
| 8 | **The skeleton's SkeletonUserData**, i.e. every bone's Body Type, mass and collision capsule plus all ragdoll joint constraints. A re-import rebuilds the bone definition and leaves the physics half empty | `Body Type: none`, `Mass 0.000`, `Collision Body Radius -1.000` and an empty Joints node in the Skeleton Inspector. Bones and `Usage` look correct, which is what makes it easy to miss | `python tools/tpac_skeleton_dump.py <tpac> <skeleton>` and compare Bodies and Constraints against a known-good compile. A healthy creature here has 48 to 61 constraints and typed bodies; a broken one has 0 and all `none` | if an older compile of the same skeleton survives, transplant it with `python tools/tpac_skeleton_swap.py <target> <source> <skeleton> <out>`, which keeps the donor item verbatim and re-points references. If none survives, `tools/tpac_skeleton_transplant.py` synthesises defaults. A humanoid on its own skeleton (the dwarf model) takes the human's instead, through `tools/tpac_skeleton_copy_physics.py` (`bannerlord-skeleton-authoring.md` "Ragdoll, IK and hit capsules for a humanoid on its own skeleton"). Warg 2026-08-31, [lotrlome-warg-changes.md](../reference/lotrlome-warg-changes.md) section 13 |
