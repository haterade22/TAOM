...
| `tpac_skeleton_extract.py` — extract ONE Skeleton (+ its 2 data segments) into a standalone skeleton-only tpac. Use when a mesh re-export dropped the skeleton from the geo tpac: rebuild a loose skeleton tpac from the backup WITHOUT reintroducing the backup's meshes (no collision with the new mesh). `python tools/tpac_skeleton_extract.py <src.tpac> <skel_name> <out.tpac> [--dry-run]`. Validate the output with `tpac_skeleton_scan.py`. Born 2026-06-13 (spider rework dropped `spider_skeleton`). | [view](https://github.com/haterade22/TAOM/blob/bannerlord-1.4.5/tools/tpac_skeleton_extract.py) | [raw](https://raw.githubusercontent.com/haterade22/TAOM/bannerlord-1.4.5/tools/tpac_skeleton_extract.py) |
