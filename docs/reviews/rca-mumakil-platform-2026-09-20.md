...
| `Main/_Module/SubModule.xml` declares no dependency on `LOTRLOME_Armory` | **Deliberate and standing, not an open item** (Mike, 2026-09-21). The Modding Kit force-loads every declared dependency, so a declaration turns a focused editor session into a full content load. The runtime benefit is not worth that: prefab and item lookups are by name, and every consumer here logs a clear error when the Armory is absent. Do not re-raise this, and do not file an issue for it |
