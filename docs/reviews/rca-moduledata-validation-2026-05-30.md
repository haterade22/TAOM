...
**Feature:** `tools/validate_moduledata.py` + `tools/taom_schema.py` + `tools/schemas/*.json` — a schema-driven cross-reference + validation engine for TAOM ModuleData XML, adopted (idea-only, MIT) from `TheOldRealms/TOR_Tools`.
...
| 3 | MED | Dead fields: `Schema.id_space`, `Registries.taom_npc_def_files` (cost a full rglob), unused `Schema.description` | Aspirational plumbing | Ported TOR's schema-model richness without wiring every field to a consumer in the same change. | Removed `id_space` + `taom_npc_def_files`; gave `description` a consumer (CLI prints it per schema). Repeat of `feedback_no_aspirational_enum_values`. |
