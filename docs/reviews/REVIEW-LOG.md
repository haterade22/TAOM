...
| 90 | 2026-09-01 | Armoury dead-mesh wave 2 (83 item definitions deleted, 7 re-pointed, 3 gated; `validate_mesh_refs.py` Assets fallback + `KNOWN_DEAD_MESHES` allowlist; `audit_deleted_mesh_impact.py` degraded mode + crafting-piece ref shapes) | not run (no C#, 6 Claude agents instead) | ISSUES FOUND (0C/0H/3M) | 3 confirmed: M1 `ar_ardunian_elite_armour` kept `has_gender_variations="false"` after its mesh was re-pointed to one that ships a `_slim` variant (female Umbar enlistee renders the male shape) - fixed; M2 all 6 re-pointed crafting pieces kept `length`/`BuildData` tuned to the old geometry - deferred by decision, `length` is also the reach stat on a career starting weapon; M3 `audit_deleted_mesh_impact.py` reported live crafting pieces as ORPHAN because its matcher only expresses `Item.<id>` - fixed in code with 5 tests. Plus 3 wrong counts propagated from an unverified subagent number (17 vs 15 moriaorc, 38 vs 39 easterling, "88 items" vs 87 items + 6 pieces), all corrected. | 2 (both agents' "positional MBGUID corrupts saves" claim: they read `ISerializableObject`, which `TaleWorlds.SaveSystem` references zero times) | 0 | 6-agent adaptive (tooling / XML integrity / adversarial deletion / tests+completeness / engine load path / cross-system data flow) |
| 91 | 2026-09-01 | Armoury dead-mesh wave 2, round 2 (fixes from review 90, plus a doc pass over six files that described a cooked asset tree the Armory does not have) | not run (3 Claude agents) | ISSUES FOUND (0C/1H/4M) | 5 confirmed: H1 `CLAUDE.md:191` still asserted the Armory's cooked packs are "what the game loads", in the always-loaded file most likely to re-propagate it - fixed; M1 **review 90's own M1 was WRONG** - `_slim` is the slim-BUILD suffix on the engine's NON-female branch (`BasicCharacterTableau.cs:531-537`), not the female variant, so the `has_gender_variations` flip was a small regression - reverted, and the correct reading made 13 redundant `_slim` items deletable; M2 the RCA fabricated "271 Saveable* references" (actual 52; the grep also counted `AutoGenerated*`) inside its own section about verifying numbers - corrected; M3 the deferred crafting-piece geometry was called cosmetic when `length` becomes the weapon's live combat reach - restated; M4 `tools/README.md` omitted both tools' new fallback behaviour - fixed. Engine log settled the central premise: `Loading packages $BASE/Modules/LOTRLOME_Armory/Assets`, and loose wins even where both trees exist. | 1 (a claimed 134-item `has_gender_variations` defect class, which dissolved once `_slim` was understood; 14 of the 128 were legitimate two-item pairs and the rest rest on the same misread) | 0 | 3-agent adaptive (doc accuracy / tooling+tests / adversarial) |
| 92 | 2026-09-01 | KEYforce Armoury cleanup: 7 commits reorganising LOTRLOME_Armory, synced live. Audit of the mesh/item reference surface plus a generated catalogue | not run (3 Claude agents) | ISSUES FOUND (2 HIGH / 2 MED / 1 n-a) | 5 confirmed: H1 the mesh gate reported 63 errors while the real damage was 275 - `validate_mesh_refs` and `validate_moduledata` see disjoint halves and neither sees the other's; H2 212 dead item refs across 159 consumers (Denethor, Forlong, Angbor, Golasgil, all Gondor char-creation and career starts) from 3 shield definitions deleted with their refs left behind; M1 16 missing collision bodies, the #352 hang class, of which 8 were a case mismatch the same commit introduced; M2 22 elven arrow items broken by a commit titled "Fixes" that deleted art without touching the XML naming it; M3 24 Gondor sword meshes attributed to the cleanup by two agents in opposite directions, proven pre-existing by scanning all 4,287 base-commit tpacs. All repaired: 275 errors to 0. | 3 (agents claimed the troll art was undeployed not deleted, that dale_armor_meshes.txt was a rotted list safe to retire when it is a live generator input, and a 134-item gender-flag defect class built on the `_slim` misreading) | 0 | 3-agent adaptive (commit delta / reference surface / catalogue design) |
| 93 | 2026-09-01 | KEYforce Armoury cleanup, tailored round 2 (catalogue generator, crafted-weapon assembly, arrow substitution, claim accuracy across every number written that day) | not run (5 Claude agents) | ISSUES FOUND (1 CRIT / 2 HIGH / 5 MED) | 8 confirmed. C1 the catalogue's rename classifier used a 0.60 similarity floor and greedy assignment: reproduced two real names scoring exactly 0.600, so a REFERENCED collision-body deletion reported as a rename with zero deletions, the one direction that says nothing was lost when something was. Raised to 0.90 plus a 0.10 margin over the runner-up, greedy replaced by sorted assignment, and the `referenced` flag now rides RENAME and MOVE as well as DELETE. H1 I wrote "129 vanilla `mp_*` entries already dangle" in three files from an agent's number without checking: of 1,233 `<AvailablePiece>` ids in Native, all 429 `mp_*` resolve and exactly one dangles. H2 the elven arrow regrouping split on `_q`/`_v2`, which encode the QUIVER not the arrow; the original art was 11/11 by quiver parity with zero exceptions, and the deleted art is recoverable (279,027 real bytes, all 8 meshes) though the cull stands by decision. M1 `weapon_descriptions.xslt` kept all 24 dead piece ids after its sibling lost them, inert but asymmetric, removed from both trees. M2 the sword rebuild is a live balance change never stated: `a05`/`a10` lose 31 and 29 cm of reach across 71 references, blades of 101.55 cm against a longest survivor of 82.96. M3 485 arrow references counted 100 hits inside `.bak-enlist-*` files the engine never globs; live figure 385, wrong in 5 places. M4 `4,490` survived one more time in CHANGELOG.md, 111 lines below the entry claiming it had been corrected everywhere. M5 the catalogue README's culture and category tables were left behind by a regeneration, 10 of 17 figures stale. Verified and left standing: the collision-body-from-blade claim (decompiled twice, data and engine), 275 = 63 + 212, 159 consumers, 24,648 asset names with 0 uppercase, 4,287 base-commit tpacs, 37 fabricated LFS renames, and the rgl_log `Assets` precedence. | 1 (the recommendation to restore the arrow art, which is technically right but overrides a standing decision to treat the cull as intended) | 0 | 5-agent adaptive (catalogue generator / crafted-weapon assembly / arrow substitution + claim accuracy / XSLT + orphans / engine gates) |
| 94 | 2026-09-02 | Umbar re-equip: naked civilian rosters, whole-culture kit, lord mounts, plus two new validator checks and a commit-gate fix | not run (5 Claude agents) | ISSUES FOUND (2 CRIT / 3 HIGH / 5 MED) | 10 confirmed. C1 the commit hook filters validate_moduledata to an explicit --code list and FOUR of the nine ERROR codes were absent, including the one added that day, so they could never block a commit; wired all four and added CommitGateCoverageTests asserting the two lists agree both ways. C2 the new check's roster regex had no `/>` alternation, so a self-closing <EquipmentRoster /> (326 in vanilla) ate the next roster's close tag: reproduced both a false negative and an ERROR false positive on a dressed troop. H1 civilian detection was a substring test, reading civilian="false" and id="..._civilian_..." as civilian. H2 single-quoted slot='Body' read the set as empty and errored on a dressed troop. H3 THE BALANCE REVIEW: the first kit grid put Umbar at the 100th percentile of its level cohort at EVERY tier, a level-6 bandit at 168 against a level-6 max of 71, +62.5% mean armour unlogged, and 5 upgrade edges LOSING armour because `_b` is an art variant at 85 not a promotion over `_a` at 89. Rebuilt on measured totals: 58th-97th percentile, 0 regressions. M1 a full 5-slot Black Numenorean kit floors at 160, so it cannot dress a low tier at cohort weight; split donors Harad/BN, which also cut a 92% item overlap with Mordor's own line. M2 the notable rotation was `idx % 4`, collapsing 18 distinct looks to 4 and re-rolling everyone when a troop is inserted; now a crc32 of the id over 8 variants. M3 civilian tiers t2/t3 were wearing the L6 and L11 infantry BATTLE plate, leaving a townsperson better armoured than the front line. M4 dead code and a WRONG engine claim survived a reversal: covers_body="false" is what an ABSENT attribute also produces and means skin shows alongside the garment, not nakedness; the misreading had shipped into ModuleData as a comment. M5 validate_all_troop_refs counted the <NPCCharacters> container (every culture one high, forever) and its prefix filter knew no `sm_`/`harad`, so it checked 4 of Umbar's 44 armour ids. | 3 (an agent's "8 lord templates" was 10; "Gondor and Harad both mount all of theirs" is false for one file each; "3 bodyless troops repo-wide" is 3 in troops_*.xml and 17 repo-wide) | 0 | 5-agent tailored (XML structural integrity / engine slot+gender+mount semantics / adversarial validator / balance and tier coherence / claim accuracy) |
...
- [rca-marriage-alignment-2026-09-06.md](rca-marriage-alignment-2026-09-06.md): RCA, MarriageAlignment (#542) deep review (5 agents, 0 HIGH; 5 findings fixed, 4 rejected with evidence, and 1 retention leak no agent reported) (2026-09-06)
...
`docs/reviews/rca-powershell-gate-coverage-2026-09-24.md`. Convergence pass owed. Nothing merged or pushed.
