...
| D-A | HIGH | The D1 fix pruned `TAOM.Dependencies` outside `bin/` against `Dependencies/_Module/`, which would delete 55 install-only MCM assets; the TAOM half also swept `RuntimeDataCache/` (final convergence) | Other: wrong premise carried forward | D1 counted "never compared `TAOM.Dependencies`" as a defect. Nobody walked the live module or read `module-dependencies.md:825-829`, and the same report's follow-up (an MCM allow-list) already contradicted it | Prune `Modules/TAOM/` only, leave `RuntimeDataCache*`, never prune TAOM.Dependencies outside `bin/`; a read-only walk of the live install proved the rule removes exactly 12 TAOM leftovers when paths are compared case-insensitively (final convergence E-1: a literal comparison would delete the 49 live `GUI/Prefabs/` files) |
...
**Final convergence, E-1 and E-2.** The rule and the report were written from a walk that lowercased both sides without saying so, so the written rule was case-sensitive while the proof was not. Prevent: a procedure proved by a script states every normalisation the script applied (case, separators, excluded folders) in the procedure itself.
