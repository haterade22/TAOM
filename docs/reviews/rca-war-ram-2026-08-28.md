...
> **Corrected 2026-09-18 (engine review):** `Agent.IsInBeingStruckAction` uses the half-open `MBMath.IsBetween(type, 48, 52)`, so `MountStrike` (52) is NOT in the being-struck band. The rejection of `act_horse_strike_front` stands on the clip, which is the horse's hit reaction.
