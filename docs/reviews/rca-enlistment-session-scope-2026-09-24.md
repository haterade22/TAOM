...
| 3 | LOW | `enlistment.md` "Every clock-keyed latch ... on both lifecycle edges", one line after the same doc says a co-op client's load skips the reset; the next clause, that a future stamp "read as 'a moment ago'", was false for the rhythm cache, whose cache test is an equality check, so a future stamp simply misses | Overbroad claim | Written as the summary of the three new clears, not checked against the gate on the load path | Same lesson as 2; the doc now names the host-only load and the unreset values |
