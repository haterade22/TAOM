...
### The assets-repo mirror of TAOM's own ModuleData drifts; port artist commits per file, three-way, from the revision he edited
`E:/repos/lotraom-assets/v1.x/TAOM/ModuleData` is a copy of `Main/_Module/ModuleData` the artist works from. On 2026-09-12 six of the ten files his commit d7d5f75b touched sat on a base identical to HEAD and four were older (skill_template and face edits, a comment), so a blind copy of the four would have reverted repo work; `git merge-file` on `git show` extracts merged all four without conflict. His commit was also the repair for the 238 references the 2026-09-01 sword rebuild broke, which the validator had been reporting since. **It happened again on 2026-09-25 (#669) with the check followed to the letter:** KEYforce's commit `429746b2` sat on Mike's "Full Update of TAOM" sync, whose files equal HEAD, but his `troops_gondor.xml` and `troops_umbar.xml` were edited from the mirror's 2026-09-15 revision, so "parent equals HEAD" passed and the copy reverted #609, #617 and #631.
- **Why missed:** nothing syncs the mirror from the repo, and the artist's mapping was context dependent (one dead sword id became three different ids across rosters), so no swap script could reproduce it. The second time, "base" was read as the commit's parent; the parent is only the base if the artist pulled it before editing.
- **Prevent:** the base of an artist file is the mirror revision of that file with the SMALLEST diff to his version (`git log -- <file>` in the mirror, then diff each revision's blob against his), not his commit's parent. Copy verbatim only where that base equals HEAD; otherwise merge three-way from that base. For troop files, merge per slot and match his fanned-out rosters to their source by content (he lays copies out as `[r0, r0, r1, r1]` as often as `[r0, r1, r0, r1]`). Re-sync the mirror from the repo after landing so his next base is current.
- **Source:** #568, 2026-09-12; #669 and `docs/reviews/rca-keyforce-lamedon-port-2026-09-25.md` findings 1 and 2.

### A balance rule is verified by measuring the property it promises, never by the case that motivated it
Mike's rule on 2026-09-25 was "nobles wear better armour than regular troops of their level". The first implementation exempted the noble troops from the ladder, which fixed the Ringlo militia (a level-11 noble in a `_med_` chest priced as light) and was applied to the live Armory. Measured afterwards, it priced most noble kit below a regular of the same level (Ithil Guard armour 57 to 36 on a level-31 noble), because the Ringlo case was the only one where the item's id tier sat above the wearer's band. The replacement (anchor a band up) then dragged a regular heavy helmet a level-11 noble wore down to the medium band for every regular wearer, found by the same measurement.
- **Why missed:** the motivating example was the test; nobody computed noble against regular medians per tier before writing live stats.
- **Prevent:** before a live restat, compute the rule's promised property on the dry-run numbers (here: per-level noble median above the regular median, and no regular item moving down); a restat that fails it does not ship. Put the rule in the tools (`_NOBLE_LINE_TROOPS`, `rebalance_armor.noble_anchor_level`), not in an exemption list that also blinds the gates.
- **Source:** #669; `docs/reviews/rca-keyforce-lamedon-port-2026-09-25.md` findings 3 and 5.
...
### A rider cannot draw a `long_bow`; a tool that hands an item to a troop checks the item's usage flags against the troop's spawn state
Native's `item_usage_sets.xml` defines `long_bow` as `base_set="bow"` plus two flags, `requires_no_mount` and `requires_no_shield`, and nothing else; the inventory tooltip says "Can't use on horseback" from the first (`CampaignUIHelper`, `ItemUsageSetFlags.RequiresNoMount`) and a mounted AI archer holding one spawns with it on its back. The ranged-ladder rewrite (#582) cloned each kingdom's donor bow verbatim and rostered the clone onto every troop in the band by class and speed alone, so seven horse archers (Harad R and V, Rohan E, Dunland R and V) whose old bows were vanilla `bow`-usage steppe bows received a `long_bow` clone. The Armory's own answer exists beside the problem: `wm_mirkwood_bow_a02` "LongBow II - Horse" is the a01 mesh with `item_usage="bow"`.
- **Why missed:** the tool compared launchers by `weapon_class` and `missile_speed`; every other attribute rode along from the donor unread, including the one that decides whether the troop can use the item at all. The before/after diff looked clean because the OLD bows were fine. Six review agents were briefed on classes and speeds and checked those; Codex tabulated every mounted troop's usage.
- **Prevent:** `ranged_ladder.mount_barred_usages()` reads the `requires_no_mount` set from the install (None when unreadable, and the caller says it skipped); `planned_edits(barred=)` refuses to roster a mounted troop into a cell whose effective usage is barred; the validator's `RANGED_MOUNT_USAGE` names any that exists; the spec's per-line `"usage": {"Bow": "bow"}` override is the fix that keeps the kingdom's mesh, and `--verify` checks it. General form: when a tool assigns an existing item to a troop, enumerate the item's usage constraints (mount, shield, race) against the troop's spawn state, not only its class.
- **Source:** Codex pass on #582, 2026-09-13, `docs/reviews/rca-ranged-ladders-codex-2026-09-13.md` finding 1.

### A `notes` array in a data file is a claim about that file; grep the groups for exceptions before writing the rule
`recruitment_pools/gondor.json` carries its weighting rules as prose in `notes` that the loader never reads. The 2026-09-13 retune wrote "Every other village keeps its regional line" from the design being implemented while `village_EW7_1..3` had shared Bar Melui's mixed town group since before July, so the file contradicted itself on the line after it was written, and a pre-existing note ("only that town/castle receives the specific pool, not its bound villages") had already been wrong for two months without anyone noticing. Nothing at runtime pays for a wrong note, so nothing checks it. The clan fallback pools drifted the same way in the same file family: no live settlement reaches them, so no test observes them.
- **Why missed:** the rule was written from intent, not derived from the data; none of the five deep-review agents compares a file's own notes to its groups, they trace data outward. A self-check after the agents returned found it.
- **Prevent:** before writing a rule sentence into a data file or its doc, run the one grep that would falsify it (here: every village id listed in a group that also names a town). When a layer has no live consumer (fallback pools, notes), either give it an observer (a validator gate, a lockstep test) or say in the file that it is unobserved.
- **Source:** `docs/reviews/rca-gondor-recruitment-vales-2026-09-13.md` F3 and F4.

### A block the engine ignores is one engine bump away from being read: gate the mirror, not the comment
TAOM's lord generators emit an inline `<skills>` block beside every `skill_template`, kept as a documentation mirror of the SkillSet because through v1.4.8 `BasicCharacterObject.Deserialize` read the block only when the template did not resolve. v1.5.2 copies the template into a fresh `MBCharacterSkills` under the character's own id and applies the block on top, so a mirror that had drifted from its set (64 lords in `characters/lords.xml`, 1,138 values, and 19 vanilla-id lords written by `lords.xslt`, 329 values, Sauron and the Witch-king among them; both directions) would have changed those lords' stats with no error anywhere. The stylesheet was the second miss: the first draft of the gate read `*.xml` only.
- **Why missed:** the contract lived in a comment (`analyze_lord_balance.py`: "the engine ignores the inline block, that's documentation only") and in `lord-skills.md`, and the analyzer's mismatch check was advisory. The loader diff had been read at the compile commit for the risk it created (a `SkillSet` registered under a character id) and cleared; nobody asked what the block being applied meant for the mirrors that were already wrong.
- **Prevent:** when data carries a value the engine does not read, either delete it or gate its parity with the value the engine does read; a comment is not a gate. `LordInlineSkillParityTests` (BindingVerification) fails on any drift and `tools/sync_lord_inline_skills.py --apply` restores it; run the sync after every SkillSet change. On an engine bump, ask of every changed `Deserialize` body both questions: what new object does it create, and what previously inert input does it now consume.
- **Source:** `docs/reviews/rca-v1.5.2-bodydiff-2026-09-14.md` finding 4; the v1.5.2 body diff, batch F (2026-09-14).

### An artist's mesh tier is not a troop tier; a fan-out that reaches for every mesh in the folder anchors the lord line to the recruit
The Armory names its meshes by tier (`_light_`, `_med_`, `_heavy_`, `_elite_`, `_lord_`) and the 2026-05-19 roster fan-out (`b88a25e7`) padded the level-11 Gundabad snaga and hunter with all six `_lord_` chests as extra battle sets. Under the kingdom-cap curve (#583) an item takes the band of its LOWEST wearer, so the whole lord line restatted to 20 body armour under a 25 kg plate mesh while the medium line, worn from level 16, sat at 31, and the level-41 rider fought in the gutted chest. The curve did what it was told; the roster was the defect, and 251 (troop, item) pairs across 9 cultures had the same shape (#609).
...
### A player-facing allow-list is checked against what a player can BE, and a fail-closed gate on editable data says when nothing can match
Field Commission shipped `allowedRaceNames` as `human`/`dwarf`/`elf` on the #376 premise of a Free Peoples player. `charactercreation/cultures.json` offers seven evil races across six cultures, so for every one of those campaigns the feature was inert from the day it shipped: merit banked, `CanPromote` failed every troop, nothing logged (#612, found by a player). The same quiet outcome follows a typo in the list, because an unknown name never matches a troop.
- **Why missed:** the design brief recorded the intent (creatures unpromotable) and the mechanism (a race allow-list) but not the enumeration the list had to be checked against, so it encoded the author's campaign. Fail-closed is right for an unknown race id, and it means a short list, a misspelt list and a working list are indistinguishable from outside; no test asserted the default's relation to anything, only its value.
- **Prevent:** before shipping an allow-list a player can hit, diff it against the set of things the player can be or field (here the union of `cultures.json` races), and pin the copies of the default to each other and to the engine's enumeration (`ShippedJson_AllowedRaceNames_*`). Where the gate fails closed on user-editable data, have the consumer warn once when an entry can match nothing (`FieldCommissionMeritService.WarnUnknownRaceNamesOnce`); the config-validation rule's "string the consumer branches on" case includes list membership, not only switch-with-default.
- **Source:** `docs/reviews/rca-field-commission-races-2026-09-17.md`, issue #612.
