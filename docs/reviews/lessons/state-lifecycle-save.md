...
### A singleton holding an ABSOLUTE campaign timestamp is worse than a stale cache: it fires immediately, it does not self-heal
Second instance of the process-singleton lesson above, and the failure mode is a step harder. A stale keyed cache mis-scores a few entities and heals as it is overwritten (CaravanTrade, #335). A stale absolute campaign DAY does not heal: campaign days are absolute, so a leftover anchor from a campaign that ended at day 5 makes `nowDays - anchor` enormous the moment a later save loads, and any `elapsed >= threshold` gate built on it passes on the FIRST tick. Enlistment's `_staleBattleLatchSinceDays` (#551) gated a destructive `PlayerEncounter.Finish`, so the leak would have torn down a live loot screen with no waiting at all, the exact action the surrounding policy rule (R1b) exists to prevent, committed by the safety net written to prevent it.
...
### A persisted MCM default cannot be flipped: rename the property
`TaomSettings` persists as json2, and MCM populates the settings object from that file over the compiled default on every launch. So changing `= true` to `= false` on a property that has ever been live reaches fresh installs only; every install that launched TAOM while the attribute was visible keeps `true` forever. This is the trap that defeated the 2026-08-20 park of ShaderPrecompilation (it had to comment out the attributes instead), and it would have defeated shipping scene passes off by default on the way back.
- **Why missed:** a compiled default looks like the single source of truth, and the test suite reads the compiled class, so nothing red says the file wins. The park entry had written the trap down in the CHANGELOG and the feature doc; the fix pattern (rename) was not written anywhere.
- **Prevent:** to change the effective default of a shipped MCM setting for everyone, give the property a NEW name with the new default and drop the old one (the orphaned key in `TAOM.json` is ignored by MCM's `PopulateObject` load), update the co-op exclusion list, and keep the pinned settable-property count. Say in the C# comment why it was renamed, so the next reader does not "simplify" it back.
- **Source:** #560 (`EnableScenePassPrecompilation` to `EnableShaderPrecompileScenePasses`), 2026-09-11; `.claude/rules/csharp-architecture.md` MCM hint rule added the same day by #559.

### A step that defines the plan needs a terminal state for never having run
When a walk or pipeline discovers its own plan mid-flight (the shader walk's batch 1 reads the roster and re-plans the batches, because `MBObjectManager` does not exist at the main menu), every way that step can fail to complete (timeout, throw, empty result, stale callback) must map to an explicit outcome the rest of the machine reports. The first cut handled each of those for the ITEM and none for the PLAN: on a start timeout the placeholder plan stayed in place, `_index++` walked off its end, and `Finish()` said COMPLETE with nothing compiled.
- **Why missed:** the state machine was traced item by item, which is how the existing guards are written; nobody asked "what does completion mean if the step that defines the plan never ran". The 120-second start bound was inherited from a scene-pass load and never re-costed against a load that now includes the full CustomGame XML pass. A counter added in the same change (`_abortedItems`) missed one of four skip paths for the same reason: the paths were listed from memory.
- **Prevent:** for any new step the rest of the machine depends on, write the list of its non-completion outcomes and the report line for each BEFORE wiring it; make "never ran" a first-class terminal state (here `_rosterDiscovered` and an INCOMPLETE status). For any new counter, grep every call site of the method it counts (`BeginEnd()` had four) rather than the three you remember. Re-cost every inherited timeout when the work behind it changes.
- **Source:** `docs/reviews/rca-shader-precompile-reenable-2026-09-11.md` findings 1 and 2 (#560); caught by the deep-review data-flow agent tracing both orderings of "renders first" versus "times out first".

### A requested engine teardown is not done until the engine says so
The shader walk's runner treated its own state as the truth about the engine: `Ending` meant "we asked for a teardown", `Running` meant "we started a mission". Three real defects followed. `MBGameManager.EndGame()` is `async void` and dereferences `Game.Current` once no manager is current, so calling it with no game (a cancel in the settle window after the game was already gone, or after `StartNewGame` threw) throws past the caller's catch. Advancing on a teardown TIMEOUT started the next game while the old one could still be loading (`EndGame` waits on `IsLoaded`; the old `GameLoadingState.OnTick` can still fire `OnLoadFinished`). And a player who exited the shader battle from the scoreboard kept the game alive, started a normal custom battle, and received the walk's mission guard and, later, the runner's `EndGame()`.
- **Why missed:** five review agents traced the runner's state machine; none opened `EndGame`'s body, none asked what the engine does when the timeout fires with the game still present, and the "what if the player starts another mission" route was assumed impossible because the runner "owned the game" (it owns a game, not the missions started inside it).
- **Prevent:** every engine transition a state machine relies on gets a check on the ENGINE's side before the next action: `Game.Current != null` before `EndGame()`, a resolved teardown (menu reached) before starting anything, and an explicit claim of the mission you act on (`TryClaimMission` from `OnMissionBehaviorInitialize`), with a stand-down, not a teardown, when it is not yours. A cancel is a request that finishes through the normal teardown path, idempotent, never a second engine call. A timeout on a teardown is a failure to report, not a permission to proceed.
- **Source:** Codex adversarial pass on #560 (GPT-6-Astra, ultra), findings 1, 3 and 4; `docs/reviews/rca-shader-precompile-reenable-2026-09-11.md`.

### A singleton CampaignBehaviorBase re-added each campaign does not accumulate listeners or dialogue lines
`CampaignEvents` exposes static accessors, but `CampaignEvents.Instance => Campaign.Current.CampaignEvents` (installed v1.4.8, `CampaignEvents.cs:589`) and that object is constructed per campaign (`Campaign.cs:1368`), as is `ConversationManager` (`:1576`). `AddNonSerializedListener` in `RegisterEvents` and `AddDialogLine` in `OnSessionLaunched` therefore register into objects that die with the campaign; a `Reuse.Singleton` behavior that `SubModule.OnGameStart` re-adds is the established shape (FieldCommission, Enlistment, WandererAllegiance) and needs no session reset for that reason. Per-campaign STATE held in the singleton's fields is a different question and still needs the reset story in `.claude/rules/csharp-architecture.md`.
- **Why missed:** the review-time efficiency agent read `ReferenceMBEvent.AddNonSerializedListener` (no dedupe) and the static accessor, assumed the event object was process-static, and rated it HIGH; nothing written down said the accessor is a per-campaign indirection.
- **Prevent:** before rating a listener-accumulation finding, open `CampaignEvents.Instance` and the `Campaign` constructor; the question is who owns the event object, not whether the add call dedupes.
- **Source:** deep review of #575 (2026-09-12), efficiency agent HIGH, disputed; `docs/reviews/REVIEW-LOG.md` Review 100.

### An engine lifecycle virtual's firing set is read from its caller, not its name
`MBSubModuleBase.OnGameLoaded` sounds like "the game finished loading". On the installed v1.4.8 it is called from `Campaign.OnInitialize` only in the `SavedCampaign` branch (`Campaign.cs:1420-1427`), never for a new campaign, and BEFORE `LoadBehaviorData`; `OnGameInitializationFinished` (`:1452`) is the one that runs after `AllBehaviorDataLoaded` and on every loading type. A `[SaveLoad]` memory stamp was wired on both with comments, a feature doc and a CHANGELOG entry all asserting the wrong order and the wrong coverage; the code was fine, the three documents were not.
- **Why missed:** the seam was chosen from the virtual's name and the measurement's need. The API agent verified the signature existed, which is what its prompt asks; the call order was only checked because the data-flow prompt named the question. Same shape as the NavalTravel finding (open the engine CONSUMER, not just the override) applied to a lifecycle hook.
- **Prevent:** before wiring any `MBSubModuleBase` / `GameManager` virtual as a phase marker, open its caller in `Campaign` / `Game` / `Module`, and write into the comment at the seam WHICH `GameLoadingType`s reach it and where it sits relative to the engine step it is meant to bracket. A stamp whose documented position is wrong is worse than no stamp: the next reader bisects the wrong window.
- **Source:** deep review of the memory instruments (2026-09-12), data-flow agent; `docs/reviews/rca-memory-instruments-2026-09-12.md` finding 1.
- **Second occurrence, four days later (#606, 2026-09-16):** `MissionBehavior.OnBehaviorInitialize` sounds like "the behavior is being initialized". On v1.5.3 `Mission.AfterStart` dispatches it to the behaviors already in the list (`Mission.cs:3827`) BEFORE it calls `SubModule.OnMissionBehaviorInitialize` (`:3831`), where TAOM adds every one of its behaviors, and `AddMissionBehavior` calls only `OnCreated` (`:4699`). SignatureStrikes evaluated its mission gate there and was inert in Mike's first battle with no log line; the grep for other overriders found SmartCavalryAI, MixedFormations and SiegeDismount doing per-mission setup in the same dead callback, and none of their init log lines exists in any log on disk. Five deep-review agents and a Codex ultra pass read the file and never asked whether the callback fires for a behavior added the way TAOM adds them; the DreadAura precedent avoids the trap by reading its gate every tick, and the copy "improved" on it by caching at init. Stronger prevention this time: `MissionBehaviorLifecycleTests` fails on any new TAOM override of `OnBehaviorInitialize` not on its allowlist, `harmony-patches.md` (the rule that loads on `Hooks/**`) states the firing set, and the deep-review compatibility prompt now asks for the caller of every overridden lifecycle virtual.

### An evicted slot goes back to its allocator, or eviction becomes growth
`FormationLayoutService.ForgetAgent` removed a deleted unit's `(row, file)` from the slot map and stopped. `AssignNextSlot` hands a newcomer `NextMeleeIndex++`, so every replacement took a row deeper than the last, and in the front/back layouts row `ceil(melee / files)` is the other class's first row: ten melee, ten ranged, one melee death, and the replacement stands on the first archer. Codex (review 109) compiled the allocator and printed the collision; the unit test that shipped with the eviction asserted only that the recycled index got a different coordinate, which the defect satisfies.
...
### A fail-closed rule is checked on every exit path, and traced through TAOM's own finalizers
`ModuleRunner.Run` had two correct rules on the same state: a faulted module is skipped for the session, and a module that owns save data throws in the steps that decide whether its `SyncData` runs. The skip came first (`if (_faulted.Contains(module.Id)) continue;`), so a save owner that faulted in a fail-open step (a main-menu phase, a mission start) or on a retried campaign start was skipped silently, and the campaign's next save would have dropped its data (`CampaignBehaviorManager` saves only the behaviors present). Each rule had its own test; the only test of the campaign-start step passed `failClosed: false`, which production never does. A second layer changes what the throw does: TAOM's `Patch37_CrashReport` finalizer on `Module.OnApplicationTick` swallows it while crash capture is on, and `GameLoadingState` re-runs the loading step on the next tick. With the fix, the save owner throws on every retry, so the load never finishes (stuck on the loading screen, save data kept); with crash capture off, the exception reaches the engine. Traced through the code, not seen in game.
- **Why missed:** the rules were written and tested one at a time, and the rethrow was traced up the engine's call chain to "no managed catch" without asking whether TAOM patches a frame on that chain.
- **Prevent:** when a guard can both skip and throw, test the SEQUENCE where both apply (fault in step A, then the fail-closed step B, then a retry of B), and let the class that documents the rule own the flag rather than a literal at the call site. Before relying on an exception to stop the game, grep `harmony-patch-registry.md` for a finalizer on every TAOM-reachable frame above the throw (`Module.OnApplicationTick`, `ScriptComponentBehavior.OnTick`, `MissionView.OnMissionScreenTick`).
- **Source:** `docs/reviews/rca-composition-root-first-steps-2026-09-24.md` findings 1, 2 and 12; Codex P2 and lenses 2, 5 and 6 (plan 018).
