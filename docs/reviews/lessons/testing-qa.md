...
### A rule over "the troop's value" says which equipment set, and the side that must win is judged by its worst one
The ranged ladder's inversion rule compared MAX `missile_speed` with MAX for both troops of a pair, and its docstring rationalised MAX for both ("the fastest one is what the player can meet"). The engine draws each battle set independently, so a higher-tier troop with a fast bow in one set and a slow bow in another can spawn slower than the tier below, and the gate saw only the fast set. Inert on the committed rosters (one launcher per class per troop) and a live blind spot the moment anyone authors an alternate set.
- **Why missed:** the rule was written from the single-launcher case, and "the troop's speed" read as one number; the fixture troops had one set each, so the tests could not disagree.
- **Prevent:** any rule that ranks troops by an equipment number states which set it reads, and takes the WORST set for the troop the rule requires to be better (`troop_min_speed`) and the best set for the other; the first fixture is a troop with a fast and a slow set. Same shape as `INCONSISTENT_ARMOUR_SLOT` and `MOUNT_WITHOUT_HARNESS`, which are judged per set for the same reason.
- **Source:** Codex pass on #582, 2026-09-13, `docs/reviews/rca-ranged-ladders-codex-2026-09-13.md` finding 3.
