...
Folding a master toggle inside the SERVICE is not the same as restoring vanilla. A `GameModel` is not "our feature" -- it is a policy oracle the ENGINE consults for every entity, including ones our feature never touched. BannerBearers (2026-07-16) folded `Enabled` correctly at the service layer (`GetDesiredBearerCount` returned 0 when off) and the model passed that 0 straight to the engine. Vanilla returns `1`. So with the feature "disabled", a formation bannered by vanilla's own hero-captain path (`GeneralsAndCaptainsAssignmentLogic`) or by the player's Order-of-Battle screen -- neither of which TAOM intercepted at the time; since #576 `TaomBattleInitializationModel` closes that screen for an enlisted soldier, and the banner model still defers -- kept its banner item and got **zero bearers**: strictly worse than vanilla, not equal to it. Three sibling overrides leaked the same way (a tuned troop-count threshold the engine's own `CanFormationDeployBannerBearers` reads, a race gate narrowing vanilla's eligible set, an equipment backstop).
...
### A flag you flipped for its first consumer has other consumers: enumerate them before flipping
The 2026-08-12 army join (#443) made `MapEvent.IsPlayerSergeant()` true so the commander's troops would share the player's team, and reasoned about the flag through the one consumer the change was aimed at, `Team.SetPlayerRole`. Its second consumer, `SandboxBattleInitializationModel.CanPlayerSideDeployWithOrderOfBattleAux`, opened the Order of Battle deployment screen at every rank, the sergeant-choice view auto-selected the player as captain, and `AssignSergeant` then set `Formation.PlayerOwner`, whose setter turns `IsAIControlled` off. With TAOM's `(false, false)` roles nobody could command that formation (#576). The feature doc predicted the consequence in writing as "a consequence to watch in-game"; the issue closed without that test; the prediction sat 900 lines away from a "do not re-derive" table that still said the screen was unreachable.
- **Why missed:** the flag was audited through the consumer being fixed. A "consequence to watch" is not an open item unless it has an issue; a "do not re-derive" banner outlives the premise it was written under.
- **Prevent:** before flipping any engine-read flag (`IsPlayerSergeant()`, `Army != null`, `IsActive`, a `MapEventSide`), grep the decompile cache for every reader (`IsPlayerSergeant()` has 8 in 4 files on 1.4.8), classify each as "intended", "harmless" or "needs a seam", and give every "needs a seam" its own issue before the flip ships. Delete "do not re-derive" banners when the premise they guard changes; state the premise in the table title instead.
- **Source:** #576, the deployment-screen captaincy; `docs/features/enlistment.md` "No rank holds a command".

### Before adding an engine side effect at an event, read the caller chain of the operation, not the layer you subscribed to
A heap release was built on `ScreenManager.OnPopScreen` for the inventory, party and character screens on the claim that "the engine only collects on a full screen teardown or a native timer, never on a plain PopScreen". True of `ScreenManager`. Those screens close through `GameStateManager.PopState`, and `GameStateManager.OnPopState` ends with `Common.MemoryCleanupGC()` (installed v1.4.8, `GameStateManager.cs:306`; `OnPushState` too at `:278`), so the release was a second full collection immediately before the engine's own, and the 2.6 GB it was meant to return had already survived two engine collections and was rooted. Removed the same day.
- **Why missed:** the callers of `MemoryCleanupGC` were grepped and the `GameStateManager` hits were in the list; only the `ScreenManager` one was opened because the class subscribed to a `ScreenManager` event. The review agents were briefed with the conclusion as a premise, so none opened the screens' close paths; Codex did (`InventoryManager.CloseInventoryPresentation`, `PartyScreenHelper.CloseScreen`, `GauntletCharacterDeveloperScreen.CloseCharacterDeveloperScreen`, all down to `PopState`).
- **Prevent:** for any side effect added at an engine event (a GC, a cache clear, a flush, a re-read), trace the concrete operation being reacted to from its user-facing entry point down through every manager layer, and list where the engine already performs the same side effect on that chain. Brief reviewers with the question, never the answer. A grep hit you did not open is not evidence.
- **Source:** Codex adversarial pass on the memory instruments, 2026-09-12, P2; `docs/reviews/rca-memory-instruments-2026-09-12.md` finding 3.

### A full-replacement GameModel override is a frozen copy of a vanilla body: name the copy, and diff it on every engine bump
`TaomPartyWageModel.GetTroopRecruitmentCost` replaces vanilla wholesale and re-implemented the v1.4.8 shape of the buyer-perk discounts (each on the BUYER's own perks, `Frugal` gated on `IsPartyLeader`). v1.5.2 moved the seven secondary-role discounts to `PerkHelper.AddPerkBonusForParty` on the buyer's party, where every one of them has `SecondaryRole PartyLeader`: the party LEADER's perks, none for a buyer without a party, no leader gate, and `LimitMin(1f)` outside the null check. The signature never moved, so the build, `HarmonyPatchBindingTests` and the API snapshot all stayed green through a body that no longer matched the engine.
- **Why missed:** every gate TAOM has compares signatures against the installed DLLs and re-derives on each run; a copied body has no equivalent. The entry that introduced the copy records the v1.4.5 body it was verified against and nothing marks it as a copy to be re-diffed. An additive override (`base.` first, TAOM layered on top) inherits an engine change for free; a replacement inherits nothing, and the two look the same to a signature gate.
- **Prevent:** an engine bump includes a member-level body diff of every bound member (`bodydiff.py` shape: decompile both versions, diff each bound method's body, rank full-replacement overrides first because they are the only ones that can drift silently). When writing a replacement override, say in its comment which vanilla body it copies and at which version, so the next diff can find it by grep. Prefer the additive shape wherever the engine's own terms are wanted: the 28 additive overrides in this pass needed no change against a `BattleEnvironment` rework and a new personality-trait system.
- **Source:** `docs/reviews/rca-v1.5.2-bodydiff-2026-09-14.md` finding 1; the v1.5.2 body-diff pass (2026-09-14), 62 members, one drift.

### A model slot registered from two starters gets every new rule in both, and a binding test pins both
`AgentStatCalculateModel` is added twice: `TaomAgentStatCalculateModel` from the campaign starter and `TaomCustomBattleAgentStatCalculateModel` from `RegisterCustomBattleModels`. #610 wired the charge multiplier into the campaign one only, so the cheapest smoke route (Custom Battle cavalry) would have shown nothing even after the lookup bug was fixed.
