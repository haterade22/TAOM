...
| 3 | LOW | The "reword when plan 006 lands" list named `PatchShieldPolicy.cs:91-92` and the CHANGELOG but not `dr3-maintenance.md:261` or `lessons/harmony-il.md:613` (same claim, same branch), nor the older `dr3:304` and `harmony-il.md:572`. The review follow-up commit added three of the four; the convergence pass found `harmony-il.md:613` | Incomplete cross-branch record | The list was written from memory of what the decision touched, not by grepping for the claim's words; the follow-up's own search terms ("preserves the stack", "fallback paths") do not match `harmony-il.md:613`'s "preserved the stack" | New lesson: build a cross-branch reword list by grepping the claim |
