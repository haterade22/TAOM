...
confirmed defect sat in the prose around it and in how it was verified: the new credential check
...
| 6 | LOW | Backslash-pipe alternation is literal in ripgrep, so the pattern reports clean in the Grep tool | Tool dialect | The command was written for GNU grep; Claude Code's preferred search tool is ripgrep | Fixed; lesson "Write documented search patterns in `-E` form" |
...
| 8 | LOW | Plan says `py_compile` is read-only (it writes a `.pyc`) | Plan wording | Assumed, not checked against the Python docs | FOLLOW-UP FU9 |
