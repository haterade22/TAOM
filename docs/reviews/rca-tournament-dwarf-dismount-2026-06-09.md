...
1. **Memory:** `feedback_harmony_private_field_injection_underscore_count` (distilled in [lessons/harmony-il.md](lessons/harmony-il.md)) — Harmony private-field injection is `___` (three) + the field's literal name; a field named `_x` needs `____x` (four). Count from the field name, not by eyeball. The crash message `Parameter name: <stripped>` tells you exactly how many it stripped.
