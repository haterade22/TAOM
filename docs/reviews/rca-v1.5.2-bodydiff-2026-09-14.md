...
| 4 | MED | `BasicCharacterObject.Deserialize` now applies an inline `<skills>` block even when `skill_template` resolved (through v1.4.8 the block was skipped), and 64 lords in `characters/lords.xml` plus 19 vanilla-id lords written by `lords.xslt` (Sauron, the Witch-king, the Nazgûl, the Black Númenóreans) carried documentation-mirror blocks that had drifted from their SkillSet: their stats would have changed silently, both directions. The first draft of the fix read only `*.xml`; the data-flow review agent found the stylesheet's 329 values. | Loader semantics change under an unchanged signature | The loader diff was read at the compile commit for the id-collision risk it created (`CreateObject<MBCharacterSkills>(StringId)`) and cleared; the second consequence, "the block is applied now", was not asked. The tooling contract that made the mirrors safe (`analyze_lord_balance.py`: "the engine ignores the inline block") lived in a comment, not a gate, so nothing failed when the engine stopped ignoring it. Batch F asked the second question. | `tools/sync_lord_inline_skills.py` re-synced 1,138 values; `LordInlineSkillParityTests` (BindingVerification) fails on any drift; the contract in `lord-skills.md` and both tool headers now says through which version it held. Lesson appended to `data-content-cultures.md`. |

A finding from the lords review, adopted: the first draft of the parity gate and the sync tool
resolved a SkillSet id declared in two files by taking the last file whole. The engine merges
before any object exists (`MBObjectManager.MergeElements`): a later module's SkillSet with an id
already seen overrides the skills it lists and keeps the ones it omits, unless it carries
`_replaceWhileMerging="true"`. No TAOM id collides with a vanilla one, but vanilla itself declares
`infantry_heavyinfantry_level1_template_skills` in both SandBoxCore and SandBox with different
values, so the wrong rule was live for one vanilla set; both readers now merge per skill in load
order, and `tools/tests/test_sync_lord_inline_skills.py` pins the rule.

A finding from the lords review not adopted: the efficiency agent proposed pre-filtering the
parity test's XML walk by file name (`*npc*`, `*lord*`, `*character*`, `*wanderer*`) to skip
~240 files that hold no `NPCCharacter`. A name filter would miss a definition in a file that does
not match (the troop files hold `NPCCharacter` rows and do not), and the saving is about a second
in a one-shot gate; the sibling gates walk every file for the same reason.
