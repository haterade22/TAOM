...
**Not a defect, needs Mike.** After D45, `PatchClasses_AreRegisteredInAllThreePlaces` runs in no
CI step: the unit step drops `RequiresGame` and the gate selects only `BindingVerification`
(`.github/workflows/csharp.yml:67,83`). Agents 2, 5 (DF-2) and 6 (proposal 2) propose tagging it
`BindingVerification`, as its Patch82, 84, 85 and 87 siblings are. Behaviour-changing (gate 338 to
339) and not run on the gate; it also changes D45's literal outcome.
...
the other 102 classes it governs.
...
- build-tooling-workflow: "A decision applied to a tool's output is also applied to the tool's
  input and carries its conditions"
