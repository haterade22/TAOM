...
### Second pass, the data-flow and API agents (findings 6 to 11)

Findings 1 to 5 above were triaged from the first three agents. The API-compatibility and data-flow
...
### Fourth pass, the re-run API and data-flow agents (findings 13 to 15)
...
- **Agent 1 (Standards)** found #4 and #5 and correctly passed everything else. It is not scoped to
 ask whether tests exist, so #1 to #3 were out of its remit by design.
