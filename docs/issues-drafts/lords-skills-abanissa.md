# feat(lords-skills): Abanissa — lore-driven skills + traits for 8 adult NPCs

## Motivation

Following the Gondor Lord Review session (2026-05-26) and Gondor skills+traits pass
(2026-05-27), Abanissa adult NPCs in TAOM had empty `<skills/>` blocks or vanilla-default
values — boring, lore-inconsistent stats. This issue covers the Abanissa portion of the
...
- All adult Abanissa NPCs receive one of the shared TAOM archetype templates (10 base
...
TAOM-invented coastal sub-culture (8 male warlords ages 28-52). Same treatment as Shaghana — default lord/elder_lord archetypes via age.
...
**Touched**: 8 NPCs (lords.xml + lords.xslt). Children (age <14) skipped.
...
- `tools/apply_culture_skills_traits.py` — generalized generator with this culture's
  entry under `CULTURES['abanissa']`

**Apply command**: `python tools/apply_culture_skills_traits.py --culture abanissa --apply`
...
**Notes:** Minimal pass — no per-NPC overrides.
...
- Coverage check: confirmed all Abanissa adult NPCs now have populated `<skills>` blocks.
...
2. Open Encyclopedia → Heroes → filter Culture = Abanissa.
