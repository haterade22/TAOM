# feat(lords-skills): Mirkwood (Woodland Realm) — lore-driven skills + traits for 29 adult NPCs

## Motivation

Following the Gondor Lord Review session (2026-05-26) and Gondor skills+traits pass
(2026-05-27), Mirkwood (Woodland Realm) adult NPCs in TAOM had empty `<skills/>` blocks or vanilla-default
values — boring, lore-inconsistent stats. This issue covers the Mirkwood (Woodland Realm) portion of the
...
- All adult Mirkwood (Woodland Realm) NPCs receive one of the shared TAOM archetype templates (10 base
...
Thranduil (Elvenking, `elf_king` archetype: OneHanded 290, Bow 295, Charm 285, Leadership 290), Legolas (Bow 295, Athletics 295 — Fellowship member, master archer), Lothuial (TAOM queen), Feren (captain), Galion (butler/steward). Other Mirkwood elves use `elf_warrior` / `elf_archer` / `elf_lady`
...
**Touched**: 29 NPCs (lords.xml + lords.xslt). Children (age <14) skipped.
...
- `tools/apply_culture_skills_traits.py` — generalized generator with this culture's
  entry under `CULTURES['mirkwood']`

**Apply command**: `python tools/apply_culture_skills_traits.py --culture mirkwood --apply`
...
**Notes:** Introduced 7 elf archetypes (`elf_king`/`elf_queen`/`elf_lord`/`elf_warrior`/`elf_archer`/`elf_lady`/`elf_young`). Centuries-trained → combat AND diplomacy AND crafting all push 240-295. Race-aware default ignores age (immortal).
...
- Coverage check: confirmed all Mirkwood (Woodland Realm) adult NPCs now have populated `<skills>` blocks.
...
2. Open Encyclopedia → Heroes → filter Culture = Mirkwood (Woodland Realm).
