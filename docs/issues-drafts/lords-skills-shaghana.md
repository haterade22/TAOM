# feat(lords-skills): Shaghana — lore-driven skills + traits for 9 adult NPCs

## Motivation

Following the Gondor Lord Review session (2026-05-26) and Gondor skills+traits pass
(2026-05-27), Shaghana adult NPCs in TAOM had empty `<skills/>` blocks or vanilla-default
values — boring, lore-inconsistent stats. This issue covers the Shaghana portion of the
...
- All adult Shaghana NPCs receive one of the shared TAOM archetype templates (10 base
...
TAOM-invented southern-desert sub-culture (9 male warlords ages 27-55). No canonical Tolkien characters. All use default `lord` / `elder_lord` archetypes via age-driven default (no keyword archetypes defined for this culture).
...
**Touched**: 9 NPCs (lords.xml + lords.xslt). Children (age <14) skipped.
...
- `tools/apply_culture_skills_traits.py` — generalized generator with this culture's
  entry under `CULTURES['shaghana']`

**Apply command**: `python tools/apply_culture_skills_traits.py --culture shaghana --apply`
...
**Notes:** Minimal pass — no per-NPC overrides. Future work: derive Shaghana-specific archetypes if/when TAOM defines distinct combat/diplomatic style.
...
- Coverage check: confirmed all Shaghana adult NPCs now have populated `<skills>` blocks.
...
2. Open Encyclopedia → Heroes → filter Culture = Shaghana.
