# feat(lords-skills): Khand (Variags) — lore-driven skills + traits for 56 adult NPCs

## Motivation

Following the Gondor Lord Review session (2026-05-26) and Gondor skills+traits pass
(2026-05-27), Khand (Variags) adult NPCs in TAOM had empty `<skills/>` blocks or vanilla-default
values — boring, lore-inconsistent stats. This issue covers the Khand (Variags) portion of the
...
- All adult Khand (Variags) NPCs receive one of the shared TAOM archetype templates (10 base
...
TAOM Khand has Welsh/Gaelic-named warband leaders + wives. Auto-assigned to `variag_lord` / `variag_lady` / dunland_raider archetypes via bio keyword scan.
...
**Touched**: 56 NPCs (lords.xml + lords.xslt). Children (age <14) skipped.
...
- `tools/apply_culture_skills_traits.py` — generalized generator with this culture's
  entry under `CULTURES['khand']`

**Apply command**: `python tools/apply_culture_skills_traits.py --culture khand --apply`
...
**Notes:** Slavic-Mongol cavalry archetype: high Riding (275), balanced melee + Bow. Honor 1, Mercy -1 (mercenary mindset). No canonical Tolkien overrides (Variags are only named as a group in canon).
...
- Coverage check: confirmed all Khand (Variags) adult NPCs now have populated `<skills>` blocks.
...
2. Open Encyclopedia → Heroes → filter Culture = Khand (Variags).
