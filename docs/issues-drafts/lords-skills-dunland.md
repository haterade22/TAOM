# feat(lords-skills): Dunland (Hillmen / Saruman's auxiliaries) — lore-driven skills + traits for 68 adult NPCs

## Motivation

Following the Gondor Lord Review session (2026-05-26) and Gondor skills+traits pass
(2026-05-27), Dunland (Hillmen / Saruman's auxiliaries) adult NPCs in TAOM had empty `<skills/>` blocks or vanilla-default
values — boring, lore-inconsistent stats. This issue covers the Dunland (Hillmen / Saruman's auxiliaries) portion of the
...
- All adult Dunland (Hillmen / Saruman's auxiliaries) NPCs receive one of the shared TAOM archetype templates (10 base
...
TAOM Dunland is all-female shieldmaiden + raider warband leaders (Eldith Grey-Claw, Sigga Wyrmbane, Yrsa the Winter Boar, Freya Clawrend, etc.) — Norse-themed names. Auto-assigned to `dunland_warrior` / `dunland_raider` / `dunland_brenin` archetypes via bio keyword scan. No canonical Tolkien named characters in TAOM's roster.
...
**Touched**: 68 NPCs (lords.xml + lords.xslt). Children (age <14) skipped.
...
- `tools/apply_culture_skills_traits.py` — generalized generator with this culture's
  entry under `CULTURES['dunland']`

**Apply command**: `python tools/apply_culture_skills_traits.py --culture dunland --apply`
...
**Notes:** Introduced `dunland_warrior` / `dunland_raider` / `dunland_brenin` archetypes. Honor 0-1, Valor 2, Mercy -1 (raider mindset). Wulf-canon ancestry referenced in feature doc only.
...
- Coverage check: confirmed all Dunland (Hillmen / Saruman's auxiliaries) adult NPCs now have populated `<skills>` blocks.
...
2. Open Encyclopedia → Heroes → filter Culture = Dunland (Hillmen / Saruman's auxiliaries).
