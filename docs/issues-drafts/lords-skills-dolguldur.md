...
(2026-05-27), Dol Guldur adult NPCs in TAOM had empty `<skills/>` blocks or vanilla-default
values — boring, lore-inconsistent stats. This issue covers the Dol Guldur portion of the
...
- All adult Dol Guldur NPCs receive one of the shared TAOM archetype templates (10 base
...
All TAOM-invented orc names (no canonical Khamûl entry in lords.xml — he'd be a separate Nazgûl). 6 compound chieftains (D1_1, D2_1, D3_1, etc.) marked as `orc_chieftain`; the rest as `orc_warrior` / `orc_female`
...
**Touched**: 59 NPCs (lords.xml + lords.xslt). Children (age <14) skipped.
...
- `tools/apply_culture_skills_traits.py` — generalized generator with this culture's
  entry under `CULTURES['dolguldur']`

**Apply command**: `python tools/apply_culture_skills_traits.py --culture dolguldur --apply`
...
**Notes:** Pure archetype application — no canonical Tolkien overrides. Reuses orc archetypes from Mordor.
...
- Coverage check: confirmed all Dol Guldur adult NPCs now have populated `<skills>` blocks.
...
2. Open Encyclopedia → Heroes → filter Culture = Dol Guldur.
