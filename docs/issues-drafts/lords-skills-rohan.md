# feat(lords-skills): Rohan — lore-driven skills + traits for 92 adult NPCs

## Motivation

Following the Gondor Lord Review session (2026-05-26) and Gondor skills+traits pass
(2026-05-27), Rohan adult NPCs in TAOM had empty `<skills/>` blocks or vanilla-default
values — boring, lore-inconsistent stats. This issue covers the Rohan portion of the
...
- All adult Rohan NPCs receive one of the shared TAOM archetype templates (10 base
  archetypes + man-specific variants).
- 19 canonical Tolkien characters receive explicit
  per-NPC overrides on top of the archetype.

**Canonical highlights:**
Théoden (Leadership 295), Éomer (OneHanded 285, Riding 295), Théodred (Polearm 275, killed at Fords of Isen), Éowyn (shieldmaiden archetype, OneHanded 250, Medicine 200 — House of Healing), Erkenbrand (Lord of Westfold, Leadership 265), Grimbold (Westfold marshal, hero of Fords of Isen), Théodwyn + Elfhild (Théoden's deceased sister/queen, matriarch stats), plus Varmund (Aldburg), Marhath (horse-breeder), and 30+ Mark riders
...
**Touched**: 92 NPCs (lords.xml + lords.xslt). Children (age <14) skipped.
...
- `tools/apply_culture_skills_traits.py` — generalized generator with this culture's
  entry under `CULTURES['rohan']`

**Apply command**: `python tools/apply_culture_skills_traits.py --culture rohan --apply`
...
**Notes:** Introduced `rider`, `shieldmaiden`, `horse_breeder` archetypes for Rohirric cavalry emphasis (high Riding/Polearm, low Crossbow). Bards/wives use general `lady`+`matriarch`.
...
- Coverage check: confirmed all Rohan adult NPCs now have populated `<skills>` blocks.
...
2. Open Encyclopedia → Heroes → filter Culture = Rohan.
