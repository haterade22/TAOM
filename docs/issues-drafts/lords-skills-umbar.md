# feat(lords-skills): Umbar (Corsairs) — lore-driven skills + traits for 10 adult NPCs

## Motivation

Following the Gondor Lord Review session (2026-05-26) and Gondor skills+traits pass
(2026-05-27), Umbar (Corsairs) adult NPCs in TAOM had empty `<skills/>` blocks or vanilla-default
values — boring, lore-inconsistent stats. This issue covers the Umbar (Corsairs) portion of the
...
- All adult Umbar (Corsairs) NPCs receive one of the shared TAOM archetype templates (10 base
  archetypes + man-specific variants).
- 10 canonical Tolkien characters receive explicit
  per-NPC overrides on top of the archetype.

**Canonical highlights:**
TAOM Umbar has 10 Black Númenórean corsair lords with authentic Adunaic names (Ar-Gimilkhâd, Gimilzâr, Gimilthân, Pharaz�n, Zimraphel, Azraphel, Belkazar, Pharakhân, Inkaldâr, Zimrathâr). Adunaic `Ar-` prefix = king/highest noble. All use `corsair_lord` / `corsair_captain` / `young_lord` archetypes.
...
**Touched**: 10 NPCs (lords.xml + lords.xslt). Children (age <14) skipped.
...
- `tools/apply_culture_skills_traits.py` — generalized generator with this culture's
  entry under `CULTURES['umbar']`

**Apply command**: `python tools/apply_culture_skills_traits.py --culture umbar --apply`
...
**Notes:** Introduced `corsair_lord` / `corsair_captain` archetypes. Heavy Trade (265), Roguery (265), Charm (240), Tactics (260) — pirate captain skillset. Honor -1, Mercy -1, Authoritarian 1.
...
- Coverage check: confirmed all Umbar (Corsairs) adult NPCs now have populated `<skills>` blocks.
...
2. Open Encyclopedia → Heroes → filter Culture = Umbar (Corsairs).
