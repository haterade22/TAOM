---
name: context-budget
description: Audit context window consumption across TAOM agents, skills, rules, MCP servers, and AGENTS.md. Report token estimates, flag bloat, recommend trims.
argument-hint: [optional: --verbose for per-file breakdown]
---

# Context Budget

Quantifies what TAOM's `.Codex/` setup consumes from the context window before a single user message is processed. Drives every other context optimization decision (e.g., whether two-layer skill injection or three-layer compression are urgent or premature).

Adapted from [affaan-m/everything-Codex](https://github.com/affaan-m/everything-Codex/tree/main/skills/context-budget).
...
```bash
bash .Codex/skills/context-budget/scan.sh
```

This walks every `.Codex/` component, the `.mcp.json`, and `AGENTS.md`, then prints a structured report.

For a per-file breakdown, run with `--verbose`:

```bash
bash .Codex/skills/context-budget/scan.sh --verbose
```
...
Context model: Codex Opus 4.7 (1M)
...
| Is AGENTS.md too long? | >300 lines | Move repeating rules into scoped `.Codex/rules/*.md` files |
...
- **Prose markdown** (skills, agents, rules, AGENTS.md): `words × 1.3`
...
|------|-------------------|
| `AGENTS.md` | Always loaded into every session |
| `.Codex/agents/*.md` | Agent descriptions loaded with every Task tool spawn (full body loaded only when invoked) |
| `.Codex/skills/*/SKILL.md` | Skills (loaded names; bodies on demand if Codex skill cache is two-layer — verify this!) |
| `.Codex/rules/*.md` | Scoped rules; loaded conditionally based on file glob, but counted as worst-case |
| `.Codex/hooks/*.sh` | Hook scripts — not loaded into context, but counted to surface candidates for consolidation |
| `.mcp.json` | MCP server count + estimated tool count overhead |

## What's NOT Counted

- User message history (variable)
- System prompt boilerplate from Codex itself (fixed, ~3-5K tokens)
...
- **Model tiering** (already in AGENTS.md "Model Routing"): Haiku for read-only/search, Sonnet for most coding, Opus for architecture/deep reasoning.
...
- **AGENTS.md substantial.** At ~10 words/line × 1.3, every 100 lines ≈ 1,300 tokens — check `scan.sh` for the current size.
...
If MCP dominates and AGENTS.md is large, the highest-leverage trim is usually MCP server pruning, not skill refactoring.

## Notes

This skill is adapted for TAOM's layout (skills-as-directories, .mcp.json at project root). The token estimates are conservative — actual context cost varies by Codex version, MCP transport overhead, and tokenizer revision. Treat numbers as ordinal (which is biggest) not cardinal (exact byte count).
