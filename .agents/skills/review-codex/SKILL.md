...
Handles the full Codex review lifecycle automatically — Codex dispatches Codex directly via the local `codex` CLI (no terminal hand-off to the user). Detects what needs reviewing from context.
...
5. **Continue with other work or stop**. When the background notification arrives, automatically proceed to Phase 3 by reading the output file. Do NOT re-prompt the user to "run /review-codex again" — Codex continues the lifecycle itself.
...
**Why did Codex miss it during implementation?**
...
- A rule in `.Codex/rules/` if it's a recurring pattern
...
2. If Codex **missed a bug category** that Codex caught, add it to "Bugs Codex typically misses"
3. If Codex did something **particularly well** in this review, add it to "What Codex does well"
4. Update the "Last updated" date

This creates a feedback loop: Codex's findings improve Codex's next review. Over time, Codex's accuracy improves and the gap between what Codex finds and what Codex catches shrinks.
