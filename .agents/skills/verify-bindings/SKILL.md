...
`BANNERLORD_GAME_DIR` (or `BANNERLORD_OVERRIDE_DIR`) must point at the install — the gate loads the SandBox/CustomBattle/StoryMode module DLLs from there. If unset, the gate self-reports `Assert.Inconclusive` (it does not falsely pass). This is an environment fact to report, not fix (see `.Codex/rules/environment-failures.md`).
