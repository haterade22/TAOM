...
- Ending the conversation also clears the boundary (state file lives in `.Codex/tmp/freeze/`, gitignored, but persists across sessions until manually cleared — re-run `/unfreeze` if a stale boundary is still active when you start work).
