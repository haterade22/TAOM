...
Both are reported with actionable error messages if missing (per `.Codex/rules/environment-failures.md`).

## Related

- `.Codex/skills/research/SKILL.md` — broader "decompile and analyze before implementing" workflow. Use that for full analysis sessions; use `taom-src` for one-shot signature lookups.
- `AGENTS.md` "TaleWorlds Research — Lookup Order" — high-level routing.
