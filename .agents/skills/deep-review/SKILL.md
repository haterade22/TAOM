...
2. **Dispatch Codex directly via Bash** -- Codex does this itself, no terminal hand-off:
   - Pre-flight: `codex login status` -- expect `Logged in using ChatGPT`. If not, surface the message and continue WITHOUT the Codex pre-review (don't block the Codex agents).
   ...
   - Run via Bash:
     ...
     timeout: 600000
     ```
   - See `.Codex/skills/review-codex/SKILL.md` "Codex CLI invocation contract" for full dispatch semantics.
3. Continue to Step 1 immediately — do NOT wait for Codex here. The 5 Codex agents run in parallel with the Codex background job.
4. After all 5 Codex agents complete (Step 2), check if the Codex background job has notified. If yes, read `docs/reviews/codex-prereview-{feature}-{date}.md`. If not yet (Codex agents finish faster on this kind of work), Codex result will arrive later -- proceed with Step 3 using just the Codex agent results and append Codex when it arrives.
...
If Codex and any Codex agent disagree on a finding, flag the disagreement explicitly — it is valuable signal.
...
11. **XML Parse Smoke Test (MANDATORY — fires for every modified ModuleData XML file).** Every new or modified XML file under any TAOM-managed module's `ModuleData/` must parse cleanly via PowerShell `[xml]$x = Get-Content -Raw <file>`. XML spec edge cases that pass eyeball review but reject at parse time:
    ...
    - **Why this rule exists:** Bandit Management Codex review (2026-05-27) — `taom_partyTemplates.xml` had `--` inside a comment body. 5 deep-review Codex agents read the XML semantically but none parsed it. Engine would have rejected the file silently at load. Memory: [`feedback_xml_parser_smoke_test_before_commit.md`](C:/Users/mikew/.Codex/projects/c--Users-mikew-source-repos-TAOM/memory/feedback_xml_parser_smoke_test_before_commit.md).
...
- Any Codex agent and Codex disagree → dispatch a second Codex pass focused on the disputed finding
...
If any agent (or Codex pre-review, if Step 0 ran) returned ANY confirmed finding (any severity, including LOW), Phase 3e RCA applies before the closing commit. Per `.Codex/rules/harness-facts.md` and `feedback_root_cause_mandatory.md` — this is not optional, not severity-gated, not "only HIGH." The literal text: *"Do NOT skip this step. The point is not just to fix bugs — it's to make the same category of bug impossible in future features."*
...
4. If the pattern has shipped before (grep `docs/reviews/rca-*.md` + `~/.Codex/projects/.../memory/feedback_*.md` for it), call that out — repeat-offender bugs need stronger preventive action than first-time bugs.
