...
Adapted from [affaan-m/everything-Codex/skills/skill-stocktake](https://github.com/affaan-m/everything-Codex/tree/main/skills/skill-stocktake) — TAOM version drops the multi-subagent batch evaluation and uses a leaner deterministic-scan + LLM-judgment two-pass.
...
- [ ] No `triggers:` field (not consumed by current Codex; per `harness-facts.md`)
...
- [ ] If the skill or its referenced rules state facts about Codex runtime behavior, each fact is labeled DOC-BACKED (with URL) or EMPIRICAL (with observation context). Vague "verified" claims are forbidden.

### Cross-references
- [ ] Every `/skill-name` mentioned in the body resolves to an existing skill at `.Codex/skills/<name>/SKILL.md`
- [ ] Every `feedback_*.md` memory citation exists at the actual memory path
- [ ] Every `.Codex/rules/*.md` reference exists
...
- [ ] No qualifying documented workflow is missing a skill. Per AGENTS.md "Workflow → Skill convention", a process that is **recurring + multi-step + gotcha-bearing** should be a skill. Scan for prose workflows that qualify but aren't skilled, and flag each as `[LOW] un-skilled workflow: <doc> — candidate /<name>`:
  - Numbered/phased authoring guides in `docs/ai-includes/*.md` with no matching `.Codex/skills/<name>/`.
  - "Workflow (MANDATORY)" / "sequence" / "phases" blocks in AGENTS.md that only chain other skills as prose.
...
if [[ "$1" == "full" ]]; then
    SKILLS=$(find .Codex/skills -mindepth 1 -maxdepth 1 -type d -printf '%f\n')
    AGENTS=$(find .Codex/agents -maxdepth 1 -name '*.md' -printf '%f\n' | sed 's/\.md$//')
elif [[ -n "$1" ]]; then
    # Specific skill or agent
    SKILLS=$(find .Codex/skills -mindepth 1 -maxdepth 1 -type d -name "$1" -printf '%f\n')
    AGENTS=$(find .Codex/agents -maxdepth 1 -name "$1.md" -printf '%f\n' | sed 's/\.md$//')
else
    # Quick scan: changed in last 30 days
    SKILLS=$(find .Codex/skills -mindepth 1 -maxdepth 1 -type d -mtime -30 -printf '%f\n')
    AGENTS=$(find .Codex/agents -maxdepth 1 -name '*.md' -mtime -30 -printf '%f\n' | sed 's/\.md$//')
fi
...
- presence of `triggers:` field (flag — undocumented in Codex)
- hook command paths exist and are tracked + executable
- paths to `.Codex/rules/*`, `feedback_*.md`, `docs/`, `ADR-*` resolve
