---
name: localize
description: Propagate new player-facing text through TAOM's 12-language localization pipeline — wrap with {=KEY}, register the string, run the Codex translation tool, validate. Use after adding UI text or XSLT strings.
...
## Tools
- `tools/translate_with_claude.py` — 4-tier fallback (override → cache → Codex LLM → English); cache in `tools/translation_cache/<lang>.json` is git-tracked, so re-runs are free.
