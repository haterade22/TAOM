---
name: security-scan
description: Use when auditing TAOM's Codex config for security issues — committed secrets, over-broad permissions, hook exfiltration, MCP risk, hidden-unicode injection. Runs tools/audit_claude_config.py.
allowed-tools: Bash, Read
---

# Security Scan — audit TAOM's own Codex config

A deterministic, offline security scan of TAOM's Codex configuration surface (`.Codex/`, `.mcp.json`, `settings*.json`, `AGENTS.md`, `AGENTS.md`). Ported as a TAOM-calibrated subset of obra-style "scan your own agent config" tooling (affaan-m/ECC AgentShield), 2026-05-29. See `docs/reviews/` for the review that motivated it.
...
This is **not** a blind port of the upstream ruleset. TAOM *mandates* fail-open hooks (`|| true`, `2>/dev/null`, always `exit 0`) per `.Codex/rules/harness-facts.md`, so that pattern is deliberately NOT flagged here (it is upstream). When porting any future rule, calibrate it to TAOM's conventions first.
...
- **INFO** — injection-style phrases found in `docs/` or `.Codex/rules/` are dampened to INFO because security docs legitimately quote them as examples.
...
- Run from the repo root (it resolves `.Codex/`, `.mcp.json` etc. relative to `--root`, default cwd).
