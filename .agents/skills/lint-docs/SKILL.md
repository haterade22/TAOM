...
4. **Missing feature docs** — `Main/Features/<X>/` directories with no matching `docs/features/<x>.md` (PascalCase→kebab-case + fuzzy match, same algorithm as `.Codex/hooks/detect-docs-gaps.sh`).
...
- If a memory-link path uses `../../C:/Users/mikew/.Codex/projects/...` → mention that these paths bake in a per-user home dir; suggest referencing the memory filename in prose instead of linking.
...
- `.Codex/hooks/detect-docs-gaps.sh` — the SessionStart hook this skill's "missing feature docs" check builds on
