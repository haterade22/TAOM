...
| `results.tsv` (untracked) | `.Codex/state/lint-cleanup-loop/results.tsv` (gitignored) |
...
5. **Initialize results.tsv**: at `.Codex/state/lint-cleanup-loop/results.tsv` with header `commit\tcategory\tcount_before\tcount_after\tstatus\tdescription`. Untracked by git — same convention as `autoresearch/results.tsv` (in `.Codex/state/` which is gitignored).
...
2. **Run lint + pick one finding** to fix:
   - `python tools/lint_docs.py > .Codex/state/lint-cleanup-loop/last-lint.md 2>&1`
...
5. **Re-run lint** to measure delta:
   ```
   python tools/lint_docs.py --summary > .Codex/state/lint-cleanup-loop/post.txt 2>&1
   ```
6. **Extract the metric**: `grep "^total_findings:" .Codex/state/lint-cleanup-loop/post.txt` (or the category-specific line).
...
These live under `.Codex/state/` which is gitignored.
