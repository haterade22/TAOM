...
New `Main/Features/CultureConversion/` module. When a town/castle is captured by a clan of a different culture, it now *gradually* converts to the new owner's culture — after a configurable hold period the settlement (and its bound villages) flips `Settlement.Culture`, recruits the new owner's troops, spawns their militia, takes on their identity, and loses the vanilla "foreign occupier" loyalty penalty. Conquering it back to its original culture reverts the change. Build green; **2998/3000 tests pass (56 new)**.

**Why the naive fix doesn't work in TAOM (two wrinkles):**
- TAOM recruitment never reads `settlement.Culture` — `VolunteerRecruitmentService.GetVolunteerTroopId` keys on per-settlement (~81 hard-coded pools) → per-clan → notable-culture. Changing the culture alone would not flip the troops. So the feature adds a **converted-settlement branch** to the cascade: a converted fief resolves `CultureMap[SettlementCultureId]` before the (stale) settlement/clan pools. Two new `VolunteerContext` fields (`IsConvertedSettlement`, `SettlementCultureId`) are populated by `VolunteerContextAdapter` (queries the conversion store for the settlement *or its bound parent*). New `IVolunteerRecruitmentService.HasCulturePool` gates conversion targets to the 16 recruitable cultures (never convert to a minor/bandit/mercenary culture we can't recruit for).
- `Settlement.Culture` is **not** an engine-saved field (reverts to XML on load), so completed conversions are persisted as our own records and re-applied on `OnGameLoadedEvent`.

**Model** (`SettlementConversionRecord`, one per managed town/castle): captures the *original* culture once (so reconquest-to-original removes the override and restores vanilla same-culture loyalty), an applied-override id, and a pending hold-timer. Conquest starts/cancels the timer (same-culture or rebellion = no-op; minor-culture or player-owned-when-disabled = gated out); the daily sweep completes it once `RequiredHoldDays` elapse (+ optional loyalty floor). On completion the service sets `Settlement.Culture` on town + bound villages and **clears notable `VolunteerTypes`** — required, because the vanilla daily refill only fills empty slots and upgrades existing troops within their own tree, never re-rolling a populated base type, so without the reset the old troops would persist forever.

**Architecture** (ADR-002/007): thin `CultureConversionBehavior` (events + SyncData + new-campaign reset guard, mirrors `MessengerCampaignBehavior`) → pure `CultureConversionService` (all decisions) → `CultureConversionAdapter` (all TaleWorlds access: `Settlement.Find`, `Culture`, `BoundVillages`, `Town.Loyalty`, notable `VolunteerTypes`). State in singleton `CultureConversionStore` (composite-string save format with R-format + NaN/∞ reject, mirrors `PendingMessenger`). Config `culture_conversion/culture_conversion_config.json` (validated, `FiniteFloatValidator`-guarded) + MCM group "Culture Conversion" (Enable / Days To Convert 1–365 / Require Stable Loyalty). No circular IoC dependency (recruitment has no back-dependency on conversion).

Feature doc: [docs/features/culture-conversion.md](docs/features/culture-conversion.md). Tests: service (queue/gate/complete/loyalty/stale-drop/reconquest-restore/re-apply), record (round-trip + NaN drop), store, config provider, recruitment converted-branch.

**Review (deep-review 16-agent workflow + Codex `gpt-5.5 xhigh`).** Deep-review: 10 findings → 6 confirmed (4 refuted incl. a HIGH R6 scenario proven architecturally impossible). Codex: 1 HIGH + 3 LOW, DISPUTED 5 of 7 suspects with decompiled evidence (owner-changed event fires AFTER `Town.OwnerClan` is set; R6 capture safe; `_justLoadedFromSave` guard mirrors Messengers; `ResolveStandardCascade` is the old cascade intact). Fixes: (1) HIGH — `ReapplyConvertedCultures` now purges a stale record when `SetSettlementCulture` fails (culture removed in a later mod version) instead of leaving `IsConverted=true`; (2) HIGH — `HasCulturePool` gate excluded 5 playable cultures (only checked `CultureMap` keys); added `CultureMap["vlandia"]` (Rohan) + `["aserai"]` (Harad), documented Khand/Mirkwood/Umbar as a known gap (no `is_basic_troop` set authored → conversion gated off until pools exist), + enumeration test pinning the full playable-culture domain; (3) LOW — `IsStorableId` guard rejects culture ids containing the `|` save delimiter; (4) docs — castle-refill 4-case caveat, additional `Settlement.Culture` readers (tournament/notable/Citizenship policy), RevoltTuning + CultureMarketplace cross-feature couplings. 3026/3028 tests pass (+23 review tests). RCA: [docs/reviews/rca-culture-conversion-2026-06-02.md](docs/reviews/rca-culture-conversion-2026-06-02.md).

### refactor(career-system): decompose CareerPerkMissionBehavior 302→139 LOC + repurpose 98 dead MaxCharge mutations as CooldownReduction (closes #102, #104)

Two cooperating issues finalized in a single session. Both went through the mandatory completion sequence: build + tests → first Codex pass → 5-dimension Claude deep-review fan-out (23 agents) → second Codex pass on the post-fix diff. Net: build green, 2942/2944 tests passing (28 new), `CareerPerkMissionBehavior.cs` 302 → 139 LOC (under the 150 ADR-002 ceiling).

**#102 — decompose `CareerPerkMissionBehavior`.** Extracted four inline state machines (per-frame ability tick, V-key + ready-state notification + charging-throttle, GauntletUI HUD lifecycle, per-activation effect dispatch) from a 302-line monolith into three Singleton-lifetime controllers + two adapters so the V-key + cooldown logic is unit-testable end-to-end:

- **`IAbilityActivationController`** ([AbilityActivationController.cs](Main/Features/CareerSystem/Abilities/AbilityActivationController.cs)) — pure state machine, 13 unit tests via `IAbilityInputAdapter` (V-key) + `IMissionTimeProvider` (Mission.CurrentTime) injection. Returns a flags-style `AbilityActivationResult { JustBecameReady, Activated, Charging }` struct (post-fix from single-outcome enum — see deep-review MED below) so the host can emit BOTH the green "ready" toast and the yellow "activated" toast on the same frame, preserving legacy UX.
- **`IAbilityHudController`** ([AbilityHudController.cs](Main/Features/CareerSystem/UI/AbilityHudController.cs)) — `GauntletLayer` lifecycle (boundary class). Captures `_attachedScreen` at attach time so `Cleanup()` removes from the screen we attached to, not whichever screen happens to be `ScreenManager.TopScreen` at OnEndMission (battle-result/pause modal can sit above the MissionScreen by then). Engine teardown wrapped in `try / catch / finally` so a throw cannot leave the Singleton with `_hudInitialized=true` (which would silently kill the HUD for every subsequent mission this session).
- **`IAbilityEffectExecutor`** ([AbilityEffectExecutor.cs](Main/Features/CareerSystem/Abilities/AbilityEffectExecutor.cs)) — per-activation pipeline (boundary class): MutateTemplate → ApplyCooldownAdjustment (#104) → context register → executor dispatch → toast + sound + particles.

`CareerPerkMissionBehavior` becomes a 139-line thin entry point that owns only the mission-scoped `_activeContexts` expiration list + the `OnEndMission` per-step `try/catch` teardown sequencing. Each cleanup op (`_hudController.Cleanup`, `_activationController.Reset`, `_abilityService.ClearAll`, `CareerAbilityBuffTracker.ClearAll`) runs in its own `try/catch` so a throw in one (most plausibly the HUD layer's `RemoveLayer` calling `HandleFinalize` unconditionally per vanilla `ScreenBase.RemoveLayer`) does not abort the others, preventing cross-mission state leak in the new singleton lifetime model.

**#104 Option B — repurpose 98 dead MaxCharge mutations as CooldownReduction.** After the #103 cooldown rework, `AbilityTemplateData.MaxCharge` was unread by activation logic; the 98 `<Mutation property="MaxCharge" calculator="flat" value="-20|-30"/>` entries in `taom_career_choices.xml` silently misled designers. Repurposed:

- New `AbilityTemplateData.CooldownReduction` property (default 0, copied in copy-ctor).
- New `GlobalTuning.MinCooldownSeconds` (default 5s, parsed from optional `min_cooldown_seconds` XML attribute with full NaN/Infinity/range validation per `feedback_clamp_nan_infinity_propagates.md` — bug has shipped three times).
- New `CareerAbility.AdjustCooldown(reductionSeconds, minCooldownSeconds)` — no-op for charge-based abilities, NaN/Infinity-safe (guards precede the `<=0` check because IEEE-754 NaN comparisons always return false), negative-reduction-rejected, negative-min-clamped-to-zero. Sets `CooldownRemaining = max(min, CooldownRemaining - reduction)`.
- New `ICareerAbilityService.ApplyCooldownAdjustment` overload. `AbilityEffectExecutor.Execute` calls it after mutating the template, BEFORE running the per-archetype effect executor — the HUD pip on the next frame already shows the adjusted cooldown.
- 98 XML mutations rewritten via [tools/rename_maxcharge_to_cooldownreduction.py](tools/rename_maxcharge_to_cooldownreduction.py) (50× -20 → 6, 48× -30 → 9). Preserves the 1.5× tier ratio (originally -30/-20 = 1.5, now 9/6 = 1.5). With defaults: tier-1 keystone = max(5, 30-6) = 24s; tier-2/3 = max(5, 30-9) = 21s. Stacks of 4 tier-2/3 keystones (sum 36) floor at 5s — prevents stack-up cheese.
- `MaxCharge` property retained on `AbilityTemplateData` for back-compat (the mutation reflection pipeline would warn on a future XML reference rather than silently fail).

**Review highlights.**

- **First Codex pass (1 MED + 1 LOW).** MED: `_attachedScreen` capture in `AbilityHudController` (verified against installed v1.4.5 `TaleWorlds.ScreenSystem.ScreenBase.cs:308-316` — `RemoveLayer` calls `HandleFinalize()` unconditionally with no ownership check). LOW: `dt` semantics — vanilla `Mission.OnTick` scales by `Scene.TimeSpeed` and splits fast-forward into 0.1s sub-ticks, so `dt` is mission-simulation delta NOT wall-clock; comment tightened.
- **Deep-review fan-out (23 agents, 5 dimensions × adversarial verify + completeness critic, 15 confirmed).** 2 HIGH: HUD cleanup screen-mismatch + try/finally (fixed); ADR-008 test coverage gap on boundary classes (refuted — `AbilityHudController` + `AbilityEffectExecutor` are boundary-classed per ADR-008). 4 MED: dual-toast-drop on simultaneous frame → flags-style result struct (fixed); systemic singleton lifetime asymmetry + OnEndMission per-step try/catch (fixed); doc gap (fixed in this CHANGELOG + the feature doc); test gap for ready-and-V-pressed-same-frame → new regression test `Tick_ReadyAndVPressedSameFrame_FlagsBothJustBecameReadyAndActivated`. 9 LOW: hoisted `hasCareer` once at top of OnMissionTick + deduped `IsAbilityReady` lookup (fixed); refuted `MissionAbilityExecutionContext` interface-ification per simplicity-criterion; deferred eager-logger-interpolation per simplicity-criterion; informational `_activeContexts` ordering change (the new order is arguably better — orphan-context defense); sprite-bake coverage gap (tracked at #101).
- **Second Codex pass (post-fix diff): 1 HIGH + 2 LOW.** Caught a HIGH that the first Codex pass + deep-review BOTH missed: the original XML rewrite emitted **negative** values (`-6`/`-9`) on the theory the calculator would subtract them; the `flat` calculator is actually `baseValue + value` (`BuiltInCalculators.cs:7-8`), so the mutated `CooldownReduction` was negative, and `AbilityEffectExecutor` guards on `> 0f` — the entire feature was a dead no-op. Fixed by re-running the rename script with the positive-value variant (50 × 6, 48 × 9) and tightening the script regex to be idempotent on both the legacy `MaxCharge` schema AND any previously-broken `CooldownReduction` schema (so re-application repairs the sign). LOW#1: `AbilityEffectExecutor.Execute` reordered so `ApplyCooldownAdjustment` runs AFTER `executor.Execute(context)` — a throw from the effect executor no longer shortens the cooldown for an activation whose effect never fully applied. LOW#2 (dead `MaxCharge` property on `AbilityTemplateData` still parsed + copied): deferred — removing the property is a back-compat decision tracked as future cleanup, the `CareerAbility.MaxCharge` runtime field remains correct domain shape for the charge-based execution path that the codebase still supports. **Phase 3e RCA:** the first review chain validated `AdjustCooldown` with positive direct values but did not trace XML value → flat calculator → mutated template → executor guard. Lesson: when a feature gates on a sign or comparison, ALWAYS write an end-to-end test that drives the full XML → mutation → guard path with a real choice from the shipped XML, not just a synthetic unit test of each layer.

**Files (#102):** [CareerPerkMissionBehavior.cs](Main/Features/CareerSystem/CareerPerkMissionBehavior.cs), [CareerSystemIoC.cs](Main/Features/CareerSystem/CareerSystemIoC.cs), [SubModule.cs](Main/SubModule.cs), + 11 new files in `Main/Features/CareerSystem/Abilities/` + `Main/Features/CareerSystem/UI/`, + [AbilityActivationControllerTests.cs](TAOM.Tests/Features/CareerSystem/AbilityActivationControllerTests.cs).

**Files (#104):** [AbilityTemplateData.cs](Main/Features/CareerSystem/Domain/AbilityTemplateData.cs), [AbilityTuningConfig.cs](Main/Features/CareerSystem/Domain/AbilityTuningConfig.cs), [CareerConfigProvider.cs](Main/Features/CareerSystem/CareerConfigProvider.cs), [CareerAbility.cs](Main/Features/CareerSystem/Abilities/CareerAbility.cs), [ICareerAbilityService.cs](Main/Features/CareerSystem/Abilities/ICareerAbilityService.cs), [CareerAbilityService.cs](Main/Features/CareerSystem/Abilities/CareerAbilityService.cs), [AbilityEffectExecutor.cs](Main/Features/CareerSystem/Abilities/AbilityEffectExecutor.cs), [taom_career_choices.xml](Main/_Module/ModuleData/career_system/taom_career_choices.xml), [tools/rename_maxcharge_to_cooldownreduction.py](tools/rename_maxcharge_to_cooldownreduction.py), [CooldownReductionTests.cs](TAOM.Tests/Features/CareerSystem/CooldownReductionTests.cs).

**Lessons.** New memory entry `feedback_singleton_controller_per_mission_behavior_lifetime_asymmetry.md` codifies the systemic deep-review finding: when an entry point is per-instance (`mission.AddMissionBehavior(new XBehavior(...))`) but its delegated controllers are `Reuse.Singleton`, every controller field becomes cross-mission state. The mitigation is per-step `try/catch` in the entry point's `OnEndMission` so a throw in one cleanup op cannot leak state into the next mission via skipped clears. Applies to any future TAOM feature with the same shape — Messengers, QuickActions, SmartCavalryAI have similar profiles and should be audited if symptoms surface.
...
Two interleaved UX changes to the CareerSystem. Plan: [for-our-career-system-structured-stroustrup.md](C:/Users/mikew/.claude/plans/for-our-career-system-structured-stroustrup.md).

**Concern 1 — Effect-scope clarity in choice tree.** Career-screen choice groups (e.g. "Hammer of the Deeps") show 5 bullets that mix always-active passives (+8% melee damage, +30 max health, etc.) with ability-bound mutations (Mithril Bastion radius, which only fires while V-key career ability is active). Players had no way to tell which was which. `CareerChoiceObjectVM` gained a new `EffectScopeBadge` `[DataSourceProperty]` that renders "While active" next to keystone bullets (gated on `@IsKeystone` in the prefab); passive bullets render with no badge so the asymmetry telegraphs the always-on default. Keystones now have a small text widget to the right of the description in all three tier item-templates.

**Concern 2 — Dialogue-driven career-switch picker.** "I wish to discuss my career path" (on any companion under `hero_main_options`) previously silently auto-switched the player to whatever career was first in registry iteration order — no choice presented. Replaced with a picker: the dialogue option opens `GauntletCareerScreen` in a new "switch mode" that lists eligible alternatives. Each card shows portrait + name + description + ability name + green "Choose" button.

- New `ICareerRegistry.GetEligibleSwitchTargets(currentCareerId, hero)` — filters by `IsEligible` (culture + clan tier) AND excludes the hero's current career id (case-insensitive).
- `CareerSwitchService.CanSwitch` now reads `hero.StringId` to look up the current career and rejects same-career switches at the boundary (defense in depth — the picker filters this out via `GetEligibleSwitchTargets`, but a future caller invoking `SwitchCareer` directly must not silently "succeed" into a no-op).
- `CareerSwitchDialogueBehavior` condition uses `GetEligibleSwitchTargets` to hide the dialogue option entirely when no alternatives exist (a culture with one career path produces no entry). Consequence opens the screen in switch mode.
- New `CareerSwitchTargetVM` per eligible-career card; `CareerScreenVM` gained `IsSwitchMode` / `IsNormalMode` / `IsBrowsingTargets` / `HasNoSwitchTargets` / `EligibleSwitchTargets` / `SwitchModeSubtitle` / `NoTargetsMessage` `[DataSourceProperty]`s. Normal mode (perk tree) and switch mode (picker) share the same screen via prefab gates `@IsNormalMode` and `@IsSwitchMode`.
- `CareerScreenGameState.IsSwitchMode` flag; `GauntletCareerScreen.OpenCareerScreen(bool switchMode = false)` plumbs through; `OnChooseSwitchTarget` callback wires the picker click to `_switchService.SwitchCareer` then closes.

**Codex Review #46 fixes (caught in adversarial pass on the work above).** Both reviews ran via the Workflow tool (`/deep-review` = 49 agents in adversarial-verify-plus-completeness-critic; `/review-codex` = gpt-5.5 xhigh single agent + vanilla decompilation). Codex caught a BLOCKING bug deep-review's 49 agents missed:

- **HIGH (blocking) — GameState lifecycle race.** `GauntletCareerScreen` ctor read `state.IsSwitchMode` immediately, but vanilla `GameStateManager.CreateState<T>()` invokes the ctor synchronously via `HandleCreateState → IGameStateManagerListener.OnCreateState → GameStateScreenManager.CreateScreen → Activator.CreateInstance(type, state)` BEFORE `OpenCareerScreen` can set `state.IsSwitchMode = true`. Result: the in-flight dialogue path opened the screen in NORMAL mode, never the picker — the entire Concern 2 feature was non-functional. Fix: moved `state.IsSwitchMode` read + heroAdapter creation from ctor to `OnInitialize` (which fires after `PushState`, by which point `OpenCareerScreen` has set the flag).
- **HIGH — `Popup.GreenButton` brushes don't exist in vanilla 1.4.5.** The picker Choose button referenced `Popup.GreenButton` and `Popup.GreenButton.Text` which don't exist in any `Native` / `SandBox` / `SandBoxCore` brush XML. Fix: replaced with `Popup.Done.Button.NineGrid` + `Popup.Button.Text` (the canonical "green done" pair vanilla uses).
- **MEDIUM — Empty-state UX + missing `OnPropertyChanged` notification.** The prefab's outer picker panel gated on `@IsSwitchMode` not `@IsBrowsingTargets` (the VM comment explicitly stated the intent was the latter); plus `IsBrowsingTargets` is a computed expression-body property that never fires `OnPropertyChanged` after `_eligibleSwitchTargets.Clear() + Add()` mutates the list. Both: empty target list rendered a 760px blank scroll canvas with no explanation. Fix: kept outer gate on `@IsSwitchMode` (header still shows in switch mode), gated `ScrollablePanel` on `@IsBrowsingTargets`, added new `HasNoSwitchTargets` VM property + `@NoTargetsMessage`-bound empty-state TextWidget gated on `@HasNoSwitchTargets`, and added explicit `OnPropertyChanged(nameof(IsBrowsingTargets))` + `nameof(HasNoSwitchTargets)` calls after `RebuildEligibleSwitchTargets` mutates the list.
- **LOW cleanup** — Deleted dead `[DataSourceProperty]`s never bound in the prefab: `EffectScopeTooltip` on `CareerChoiceObjectVM` (the "While active" badge alone is enough UX signal; passives need no explicit label), `SwitchModeTitle` on `CareerScreenVM` (redundant with `ScreenTitle` which is already set to "Choose Your Path" in switch mode), `AbilitySpriteName` on `CareerSwitchTargetVM` (the ability *name* label is enough on the card). Plus 2 dead loc keys (`_passive_tooltip`, `_keystone_tooltip`). Per `simplicity-criterion.md`: tiny gain + added complexity = reject.
- **LOW cleanup** — Removed discarded `switchService` ctor param from `CareerSwitchDialogueBehavior` (the switch action moved to `GauntletCareerScreen.OnChooseSwitchTarget`); updated `SubModule.cs` ctor call from 5 args to 4. Plus `LogWarning` in `RebuildEligibleSwitchTargets` when the eligible list is empty (debug triage for blank-picker reports).

**Tests:** 14 new across `CareerRegistryTests` (6 — `GetEligibleSwitchTargets` matrix + null hero + null currentCareerId boundary), `CareerSwitchServiceTests` (3 — same-career rejection, empty StringId fallthrough), `CareerScreenVMTests` (5 — switch-mode population, choice-groups skipped, mode-flag bindings, no-eligible empty-list, ExecuteChoose callback). CareerSystem suite **311/311**. Full suite **2896 passed / 2 skipped (pre-existing Warg) / 0 failed**.

**Meta-RCA written into REVIEW-LOG.md + AGENTS.md.** SECOND consecutive shipped feature where I stopped at build-green and skipped `/deep-review` + `/review-codex` (also #31 cooldown rework). The CLAUDE.md rule isn't sticking; mechanization candidate: session-stop hook checking `docs/reviews/` for recent artifacts. AGENTS.md gained a new "What Codex does well" lesson: *targeted-prompt + full-vanilla-decompilation beats broad-prompt + adversarial-verification when verifiers refute at the framing layer rather than reading the called code path* (Codex's single-agent-with-targeted-prompt caught the GameState race that deep-review's verifier-pair refuted as "Suspect 6 DISPUTED" by focusing on the wrong sentinel value).

**Loc:** 7 new strings added to `taom_module_strings.xml` and propagated via `tools/translate_with_claude.py` + `tools/rebuild_translation_files.py` to 10 of 11 AI languages (BR, CNt, DE, FR, IT, JP, KO, RU, SP, TR — all rendered with native translations). CNs (Simplified Chinese) failed consistently on this run and falls back to the engine's `{=key}default` English text; it will pick up the next translation pass. PL is hand-curated by community translator. Issue #265.

Save-compat: no persistent state changed (career switching writes through the existing `ICareerSwitchService.SwitchCareer` path which has shipped since the original CareerSystem release). Safe on any save.

Research: `GameStateManager.CreateState<T>()` / `HandleCreateState` / `OnCreateState` listener fanout (vanilla `Core/TaleWorlds.Core/GameStateManager.cs:163-183`); `GameStateScreenManager.CreateScreen` / `Activator.CreateInstance(type, state)` (installed `TaleWorlds.MountAndBlade.View.dll`); `CampaignGameStarter.AddPlayerLine` / `AddDialogLine` 7-arg overload signatures; vanilla `hero_main_options` branch enumeration; vanilla brush registry (`Native/GUI/Brushes/Popup.xml`).

Not-tested: in-game V-key activation, dialogue option visibility through `hero_main_options`, switch screen rendering, empty-state TextWidget visibility, Choose button click → `SwitchCareer` → close flow. Verified through vanilla decompilation traces but require live battle/dialogue to confirm visual + audio + click routing.
