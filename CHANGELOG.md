...
- **Scene + item audits clean.** `audit_scene_names.py` / `audit_battle_scenes.py`: every TAOM battle scene id on disk, all 256 map_indices covered. `audit_item_refs.py` (weapon/equipment cross-check requested this session): **12,476 defined item IDs, 2,665 TAOM references, 0 missing** — the hotfix renamed/removed no item TAOM uses. Spot-checked the tournament horses + practice weapons directly: all defined.
