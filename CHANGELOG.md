...
### feat(music): culture-specific music integration across 6 contexts — 17 cultures, 476 tracks

Adds a TAOM-owned music playback engine that replaces vanilla's single shared track pool. Each of TAOM's 17 Middle-earth cultures now plays distinct OGG tracks across six contexts: world map, town wandering, tavern, battle, siege, and character creation. Vanilla tavern musician groups are suppressed; the CC ambient track is suppressed once a culture-specific CC track is confirmed playing.

**Architecture:** `MusicPlaybackService` manages one active engine channel (allocate → load → play → rotate → stop). `MusicTransitionResolver` merges mission context (Battle/Siege) and campaign context (World/Town/Tavern/CC) — mission wins. `MusicTrackIndex` is loaded at startup from `taom_music_module_sounds.xml` (476 entries, auto-generated by `tools/replace_taom_music_assets.ps1`). `NoRepeatShufflePicker` prevents back-to-back repeats (history 8, round-robin fallback). Track rotation: 180 s per-bucket default. Lost-channel back-off: 1 s (campaign context only; mission/battle restarts immediately).

**Entry points:** `MusicCampaignBehavior` (campaign tick), `MusicMissionBehavior` (mission tick + end), `CharacterCreationScreen_OnFrameTick_MusicPatch` (CC ambient suppression), `MusicianGroup_SetPlayList_Patch` (tavern suppression + exit signal), `CharacterCreationCultureVM_ExecuteSelectCulture_MusicPatch` + `CharacterCreationContent_SetSelectedCulture_Patch` (CC culture signals), `CustomBattleSideVM_OnCultureSelection_Patch` (custom battle culture context).

**Also modified:** `FactionSelectionVM` signals culture selection to CC music context; `CustomBattlesIoC` wires `ICustomBattleMusicContextService`; `CharacterCreationContent_SetSelectedCulture_Patch` gains a `ConfirmCulture` signal. 12 new adapter interfaces + implementations in `Main/Adapters/`. 123 Music tests; 3154/3156 total suite passing.

**Known limitations (tracked):** MCM not yet wired (settings return compile-time defaults); no fade-in/out; CC bucket uses TownEnabled/TownVolume. See `docs/features/music.md`.
