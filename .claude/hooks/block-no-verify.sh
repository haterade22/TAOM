...
# Prefilter: the block below needs `--no-verify` in the command, and Claude Code never escapes
# an ASCII letter, so a raw payload without the text `no-verify` cannot concern this gate.
