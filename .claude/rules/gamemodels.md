...
7. **Register in SubModule.cs:** GameModel overrides are registered with `campaignStarter.AddModel<TBase>(new TaomXxxModel(...))` inside `SubModule.OnGameStart` (there is no `CreateGameModels()`); resolution is last-registered-wins, so TAOM's call must follow the engine's. `GameModelOverrideBindingTests` looks for the literal `new TaomXxxModel(` in `SubModule.cs`, so register with the unqualified type name and a `using`, not a namespace-qualified `new`.
