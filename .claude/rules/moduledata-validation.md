...
| `RANGED_LADDER_INVERSION` (warn) | a troop that outranges one it must not: inside a LINE a lower tier faster than a higher tier, or inside a BAND a worse-ranked kingdom line faster than a better-ranked one, per launcher class. Reach is the launcher's `missile_speed` and nothing else (skill feeds accuracy, cadence and AI error; the decompile cites are in `tools/ranged_ladder.py`), so the whole order lives in `tools/ranged_ladders.json` (bands by engine tier, 18 lines in kingdom rank order, `speed = band_base + rank_step * (n - rank)`), and the gate, the report and the roster tool share one pure function (`ranged_ladder.inversions`). 1,741 inverted pairs shipped on 2026-09-12 (#582). Launcher speeds come from the install, so the check is skipped, never faked, without it; a missing or self-contradicting spec and a ranged troop no line claims are findings, because a gate that quietly checks nothing reads like a clean run. Repair: `python tools/generate_ranged_ladder_items.py --apply` then `python tools/rebalance_ranged_ladders.py --apply`. Exemptions go in `_RANGED_LADDER_EXEMPT` with a reason each. A troop file that does not parse is a `(file)` finding under this code (not checked is not clean), and a higher troop is judged by its SLOWEST set |
| `RANGED_MOUNT_USAGE` (warn) | a mounted troop (`default_group` HorseArcher or Cavalry, or a Horse slot in any battle set) holding, in any battle set, a bow or crossbow whose `item_usage` set carries `requires_no_mount` in the install's `item_usage_sets.xml` (Native: `long_bow`, base_set `bow` plus that flag and `requires_no_shield`). The engine tooltip says "Can't use on horseback"; a mounted AI archer spawns with the bow on its back and never draws it. Seven horse archers shipped that way with #582 on 2026-09-12 (Harad R/V, Rohan E, Dunland R/V), each cloned from a `long_bow` donor. Fix in `tools/ranged_ladders.json`: the line's `"usage": {"Bow": "bow"}` override (the Armory's own `wm_mirkwood_bow_a02` "- Horse" pattern, same mesh) or a mounted-usage donor; `planned_edits(barred=)` refuses to roster such a troop and `--verify` checks the usage. The barred set is read from the install (`ranged_ladder.mount_barred_usages`); skipped, and said so, without it |
