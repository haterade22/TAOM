...
| `RANGED_DAMAGE_CEILING` (warn) | a bow or crossbow a Lord, Wanderer or `is_hero` character can carry (its own kit, its battle `EquipmentSet` rosters, or a template `lords.xslt` hands a retagged vanilla lord) that hits above the spec's `hero_ceiling` (Bow 90, Crossbow 105). The Armory's own launchers are what heroes and shops hand out, and they sat at 97 to 130 with accuracy 100 (#617). Repair: the item's row in the spec's `donor_stats`, then `python tools/restat_ranged_donors.py --apply` |
