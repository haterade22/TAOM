...
| the hill troll and dwarf rig FBX in the live Armory `AssetSources` (the gate's `SPEC`: head, eye, mouth, hands or arms); a new race rig is unchecked until its meshes are added to `SPEC` | the tool's docstring | `python tools/check_race_morph_channels.py` (exact counts: head, eye and mouth 101, hands or arms 26; SKIPPED has not passed) |
...
| language files, `{=KEY}` text | `docs/reference/localization-map.md` | `python tools/check_external_loc_coverage.py`; `dotnet test ... --filter FullyQualifiedName~Infrastructure.Localization` (`LanguageDataXmlTests`, `LocalizationKeyConsistencyTests`: one text per id and per shared key, inline defaults equal the registration; `LanguageTextIntegrityTests`: no wrong-script word in the rows or the translation cache) |
