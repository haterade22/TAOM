...
| **An `ask` prompts even in bypass mode.** A PreToolUse `ask` shows the user an approval prompt although the session runs with `bypassPermissions`, and the tool call waits until someone answers. A headless `claude -p` run did not wait: the agent saw the ask's reason and the command did not run. | EMPIRICAL 2026-09-26: the maintainer saw a confirm-gate prompt from a plan 027 review agent in a bypass session (a reviewer the night before stalled for nine hours, most likely the same way); headless probe of `GIT add --dry-run -A` from the PowerShell tool | The confirm gates match dangerous git text anywhere in a command, quoted text included, so an unattended agent keeps such text out of its shell commands: payloads go in files, scripts run by path. |
