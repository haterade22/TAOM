...
**Append the durable lesson to the master record.** For each systemic finding, also add an entry to `docs/reviews/LESSONS-LEARNED.md` in the matching subsystem category, in the house shape (`### <imperative rule>` → `**Why missed:**` → `**Prevent:**` → `**Source:**`). The per-feature `rca-*.md` is the incident report; the `LESSONS-LEARNED.md` entry is the cross-feature rule that stops the *category* from recurring — it is the canonical, always-consulted record (indexed from the harness `MEMORY.md`). Read the relevant category BEFORE the next review of that subsystem. Only touch `MEMORY.md` if a new feature/topic memory file is involved.
