...
2. **Stale version refs** — a version string **presented as the current target** when the pin says otherwise. Not a mention: "Ported for TAOM v1.3.15" and "v1.3.15 reference RVA, informational only" are true statements about history and are correct as written. Current target is **v1.4.8** (the pin at `.claude/pinned-game-version.txt`).
