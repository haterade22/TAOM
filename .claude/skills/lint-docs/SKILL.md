...
2. **Stale version refs**: a version string **presented as the current target** when the pin says otherwise. Not a mention: "Ported for TAOM v1.3.15" and "v1.3.15 reference RVA, informational only" are true statements about history and are correct as written. Current target is **v1.5.2** (the pin at `.claude/pinned-game-version.txt`).
...
6. **Version mismatch**: the "Target: Bannerlord X" line in AGENTS.md or CLAUDE.md (or no such line at all), or an API-snapshot header that disagrees with `.claude/pinned-game-version.txt`. Catches "pin bumped but a doc/snapshot left stale" (run `/engine-bump` if the pin itself is behind the installed game). Also gated by the same hook.
7. **Context budget** (ADR-011): everything CLAUDE.md loads at launch (CLAUDE.md and its `@`-imports, read from CLAUDE.md itself) and every rule without `paths:` load into every session and every custom or general-purpose subagent. Caps: the three entry docs together, 200 lines each, table rows and prose lines, the trap index (row count and row length, and that it is still imported with its heading), a missing import, and the unscoped rules together; the constants at the top of `tools/lint_docs.py` hold the numbers. An oversized path-scoped rule is report-only until the path-rule diet lands. AGENTS.md alone is also capped at 8,192 bytes by `tools/reviewctl.py`. Gated by the same hook and by CI, except `size-warn`, which is report-only.
