...
Adapted from [affaan-m/everything-claude-code/skills/agent-introspection-debugging](https://github.com/affaan-m/everything-claude-code/tree/main/skills/agent-introspection-debugging).
...
| **Permission prompt loop** | User keeps being asked the same permission | Add the action to `.claude/settings.local.json` allowlist via `/update-config` |
...
- `.claude/rules/harness-facts.md` (if it's a load-semantic / lifecycle finding)
