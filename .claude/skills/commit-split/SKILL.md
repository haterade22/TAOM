---
name: commit-split
description: Inspect all changed files, group by logical concern, and guide atomic per-concern commits following TAOM's versioned-subject rule and commit trailer convention.
---
...
Every subject carries the current module version after the type: `<type>[(scope)]: vX.Y.Z - <description>`,
where `vX.Y.Z` is the `<Version>` in `Main/_Module/SubModule.xml` (read it; do not guess). The
`check-commit-subject-version.sh` hook refuses a subject without it. The examples below use v2.0.28.

| Files | Commit type | Example message |
|-------|------------|-----------------|
| `Main/Features/<Name>/**/*.cs` | `feat:` or `fix:` | `feat: v2.0.28 - add alignment-aware execution penalties` |
...
| Bug fixes spanning multiple files | `fix:` | `fix: v2.0.28 - resolve null ref in WargAttackService` |
...
1. feat: vX.Y.Z - [message] ([files])
2. test: vX.Y.Z - [message] ([files])
3. data: vX.Y.Z - [message] ([files])
4. docs: vX.Y.Z - [message] ([files])
5. chore: vX.Y.Z - [message] ([files])
```
...
4. **Write commit** (subject at most 72 chars including the version label, 72-char body wrap, no AI attribution):
   ```bash
