...
- Known baseline failures you must NOT chase: on a base older than a39a9c86 exactly two tests fail because another session was editing the live Armory: TheElkItem_DeclaresTheScaleTheReachIsTunedFor and AnimaliaActionSets_BindOnlyHorseActions_ToClipsThatExist. On a39a9c86 or later they pass (10318+ passed, 0 failed), so there NO failure is acceptable. Run the full suite once before your first edit to record the base's own totals, and match them. Two [Ignore] tests are expected.
...
- When the plan's done criteria all hold: do NOT edit CHANGELOG.md: since plan 020 it is generated at /release from commit subjects and bodies, and a hand-written section makes the next release refuse; the commit body IS the changelog entry, so write it for a reader of the release note (what changed and why). Then stage EXPLICIT paths only (git add <path> ...; never -A, never .), and commit with subject '<type>(<scope>): v2.0.30 - <description>' (72 characters max, version from Main/_Module/SubModule.xml), a body wrapped at 72 that says what changed and why and names plan ${it.num}, and NO Co-Authored-By or other AI attribution trailer. Add a 'Not-tested:' trailer for anything structurally untestable (live Harmony, in-game behaviour). If a commit hook denies the commit, read its reason, fix the cause if it is yours, and never bypass hooks (--no-verify is forbidden); if the hook is confused by the worktree, report it as BLOCKED with the hook text.
