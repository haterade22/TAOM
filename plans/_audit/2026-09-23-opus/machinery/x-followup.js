...
HOUSE RULES: TDD for every behaviour change (write the failing test, SEE it fail and quote the failure, then implement, then see it pass). Read the rules that apply from ${it.wt}\\.claude\\rules\\ yourself (path rules do not auto-load outside E:\\repos\\TAOM). Build and test only in the worktree: dotnet build Main/TAOM.csproj -p:DisableModuleCopy=true -p:ModuleId= and dotnet test TAOM.Tests -p:DisableModuleCopy=true -p:ModuleId= (never ./build.ps1); the only allowed full-suite failures are TheElkItem_DeclaresTheScaleTheReachIsTunedFor and AnimaliaActionSets_BindOnlyHorseActions_ToClipsThatExist. For hook edits also run bash tools/test_hooks.sh. ADR-002/ADR-007; no #region, no [Obsolete]; no em or en dashes in prose. Update the branch's docs and its review record (append a '## Maintainer decisions applied (2026-09-24)' section to the deep-review report listing each decision and its commit) and add a line to the branch's CHANGELOG entry if behaviour changed. Do NOT edit plans/README.md; no GitHub issues or comments; never push, merge, rebase, stash, reset or switch branches.
