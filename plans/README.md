...
| [011](011-stop-reminders-and-trunk-guard.md) | Make the four Stop reminders reach Claude and guard the live trunk against force pushes | P2 | S | dx | none | IN PROGRESS on `improve/011-stop-reminders-and-trunk-guard` (from `bec0389d`): Step 0 done by the orchestrator (with D42); executor running |
...
| [016](016-repo-hygiene-pins-readme.md) | Untrack personal and generated files, pin the MCP servers, and correct the README | P2 | S | security | none | IN PROGRESS on `improve/016-repo-hygiene-pins-readme` (from `bec0389d`): Step 0 done; executor running |
...
| [021](021-architecture-rule-amendments.md) | Amend three architecture rules to match the code: virtual boundary seams, when a service gets an interface, when a patch gets a hook (decision 19) | P2 | S | tech-debt | none | IN PROGRESS on `improve/021-architecture-rule-amendments`: Step 0 ADR commit `1cdf8eb0` (looser seam wording, D49); executor queued |
...
| [005](005-security-hygiene.md) | Scrub vendored credential + pin MCP servers + close faction_map subprocess injection | P2 | S | security | none | MERGED into `bannerlord-1.5.x` (`e9b28e2a`, 2026-09-25); issue held for Mike (vendored credential still in three tarballs) |
