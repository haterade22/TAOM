...
|------|-------|----------|--------|----------|------------|--------|
| [008](008-binding-gate-no-silent-skips.md) | Make the binding gate fail loudly instead of passing by skipping | P2 | S | tests | none | TODO |
| [010](010-ci-on-hosted-windows.md) | Compile and test C# on GitHub-hosted Windows runners against BUTR reference assemblies | P2 | M | dx | 008 | TODO |
| [009](009-guarded-patch-category-apply.md) | Apply every Harmony patch category through one guarded helper | P2 | S | bug | none | TODO |
| [011](011-stop-reminders-and-trunk-guard.md) | Make the four Stop reminders reach Claude and guard the live trunk against force pushes | P2 | S | dx | none | TODO |
| [012](012-loading-window-trace-per-frame.md) | Stop the loading-window trace walking the stack and flushing a log line every frame | P2 | S | perf | none | TODO |
| [006](006-crash-capture-boot-cost.md) | Stop the crash-capture sweep costing 30 s of every boot, and delete the four finalizers that can never fire | P2 | S | perf | none | TODO |
| [007](007-patchshield-skip-callback-shims.md) | Stop PatchShield re-shielding the 247 callback shims at the first game start | P2 | S | perf | none (complements 006) | TODO |
| [014](014-enlistment-session-scope.md) | Reset Enlistment's per-session clocks and caches on load and on a new campaign | P2 | S to M | bug | none | TODO |
| [016](016-repo-hygiene-pins-readme.md) | Untrack personal and generated files, pin the MCP servers, and correct the README | P2 | S | security | none | TODO |
| [013](013-bash-hook-prefilter.md) | Let non-git Bash calls skip the Python start-up in every Bash hook | P3 | S | dx | none | TODO |
| [015](015-warg-tick-costs.md) | Cut the warg behaviour tree's per-tick resolves, allocations and native wrapper churn | P3 | S | perf | none | TODO |
| [017](017-build-identity-dirty-flag.md) | Stamp a dirty-tree flag into every build and a structured build field into every crash bundle | P3 | S | dx | none | TODO |
| [018](018-composition-root-first-steps.md) | Start the feature-module composition root: shared source reader, module contract, empty list, one pilot feature | P3 | M | tech-debt | 009 | TODO |
| [019](019-nullable-ratchet.md) | Stop discarding the nullable warnings and graduate the first folder to errors | P3 | S | tech-debt | none | TODO |
| [002](002-nan-infinity-config-guards.md) | Reject NaN/Infinity in TroopWeight + Career-mutation config loaders | P1 | S | bug | none | PARTIAL (2026-09-23): TroopWeight guard landed in `bee07b48` (`TroopWeightXmlLoader.cs:86`); `MutationParams.GetFloat` still unguarded (`MutationParams.cs:15-20`). `impl-002`: cherry-pick `cfc47206` only, trunk side on conflicts |
| [001](001-cross-campaign-singleton-resets.md) | Reset feature singletons on new-campaign boundary (Career + SpecialResources) | P1 | S to M | bug / save-integrity | none | PARTIAL (2026-09-23): Career reset landed in `f4273639` (`ResetForNewCampaign`); SpecialResources storage still leaks (`SpecialResourcesBehavior.cs:118-132` resets session state only; live dict passed on load at `:107-109`). `impl-001`: rewrite the SpecialResources half fresh; its test targets a constructor that no longer exists |
| [005](005-security-hygiene.md) | Scrub vendored credential + pin MCP servers + close faction_map subprocess injection | P2 | S | security | none | TODO (2026-09-23): nothing landed. Item B (MCP pins) is superseded by plan 016. `process_faction_map.py:260,383` unchanged; the credential now sits only in three gitignored, untracked BUTR tarballs under `Dependencies/.vendor-source/`. `impl-005`: cherry-pick `4310aa6e` and `4bc520a1`, drop `1d566be6` (stale pins) |
| [003](003-hot-path-resolve-and-grid-caching.md) | Cache hot-path IoC/MCM resolves + fix 2s SpatialGrid staleness | P3 | S each | perf | none | TODO (2026-09-23): nothing landed. PERF-03 STALE (`bee07b48` deleted 6 of its 8 files); PERF-07 superseded by plan 015; PERF-04 still open (`impl-003` `6eb5955c` applies clean); PERF-01 grid still 2 s (`AdvancedCombatBehavior.cs:16`), needs a gate decision |
