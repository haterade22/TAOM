...
| 026 (plan file on its branch) | Move TAOM decision logic out of four protected-virtual seams into their services: `SupplyOrderService.ChargePlayer`, `RefugeService.FindNearestHostile`, `WardenService.CompanionsInMainParty`, `WardenService.MintCompanionFromTroop` (decision 56) | P3 | M | tech-debt / testability | none (satisfies ADR-007 as amended by 021) | IN PROGRESS on `improve/026-seam-decision-logic` (worktree `wt-026`, from `31cc629f`): plan being written |
