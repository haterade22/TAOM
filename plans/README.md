...
| 027 (plan file on its branch) | Make the eight Bash-only PreToolUse gates cover the PowerShell tool, parse PowerShell syntax, and close validate-push's remaining bypass shapes (decision 61) | P2 | M | dx / safety gates | none | IN PROGRESS on `improve/027-powershell-gate-coverage` (worktree `wt-027`, from `96afb6fb`): plan being written |
