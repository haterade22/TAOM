...
internal static class SettlementPlatePresenter
{
    ...
    /// the tracked ring, the party icons and the event icons. Those last three keep vanilla's
    /// full alpha at close range (text alpha is 1 there) and only follow the plate down under the
    /// distance fade, otherwise a far tracked settlement would leave an opaque ring floating over
    /// the map after its plate faded (Codex review, #591). Every write repeats whenever the
    /// destination disagrees, because vanilla rewrites the text, grid and event alphas each
    /// parallel update from its own lerp toward 1. A non-finite plate alpha leaves bar and frame
    /// at their last good value (the policy refuses it) and restores full text alpha.
    /// </summary>
        ...
        ref float lastAlpha, ref float lastColorFactor)
    {
        ...
        var textAlpha = PlateAlphaPolicy.TextAlphaFor(alpha);
        ...
        var events = (item.ParentWidget as SettlementNameplateWidget)?.EventsListPanel;
