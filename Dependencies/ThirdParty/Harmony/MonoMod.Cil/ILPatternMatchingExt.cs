// Decompiled MonoMod — ref-assign pattern is valid IL but can't be expressed in safe C#
#pragma warning disable 8374
using System;
...
internal static class ILPatternMatchingExt
{
	...
	public static unsafe bool Match<T>(this Instruction instr, OpCode opcode, T value)
	{
		if (instr.Match<T>(opcode, out var value2))
		{
			...
			{
				val = reference;
				unsafe { reference = ref Unsafe.AsRef<T>(Unsafe.AsPointer(ref val)); }
				if (val == null)
				{
					return value == null;
				}
	...
	public static bool Match<T>(this Instruction instr, OpCode opcode, [MaybeNullWhen(false)] out T value)
	{
		...
		if (instr.OpCode == opcode && instr.Operand is T val)
		{
			value = val;
			return true;
		}
		...
		if (obj != null)
		{
			if (!(obj is int result))
			{
				if (!(obj is short result2))
				{
					...
					{
						if (!(obj is ushort result4))
						{
							if (!(obj is byte result5))
							{
								if (!(obj is sbyte result6))
								{
									if (!(obj is ParameterReference { Index: var index }))
									{
					...
					if (!(obj is uint result3))
					{
						if (!(obj is ushort result4))
						{
							if (!(obj is byte result5))
							{
								if (!(obj is sbyte result6))
								{
									if (!(obj is VariableReference { Index: var index }))
									{
										throw new InvalidCastException();
									}
									return index;
	...
	{
		Helpers.ThrowIfArgumentNull(instr, "instr");
		if ((instr.OpCode == OpCodes.Call || instr.OpCode == OpCodes.Callvirt) && instr.Operand is MethodReference methodReference)
		{
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Beq || instr.OpCode == OpCodes.Beq_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Bge || instr.OpCode == OpCodes.Bge_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Bge_Un || instr.OpCode == OpCodes.Bge_Un_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Bgt || instr.OpCode == OpCodes.Bgt_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Bgt_Un || instr.OpCode == OpCodes.Bgt_Un_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ble || instr.OpCode == OpCodes.Ble_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ble_Un || instr.OpCode == OpCodes.Ble_Un_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Blt || instr.OpCode == OpCodes.Blt_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Blt_Un || instr.OpCode == OpCodes.Blt_Un_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Bne_Un || instr.OpCode == OpCodes.Bne_Un_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Box && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Br || instr.OpCode == OpCodes.Br_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Brfalse || instr.OpCode == OpCodes.Brfalse_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Brtrue || instr.OpCode == OpCodes.Brtrue_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Call && instr.Operand is MethodReference methodReference)
		{
			value = methodReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Calli && instr.Operand is IMethodSignature methodSignature)
		{
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Callvirt && instr.Operand is MethodReference methodReference)
		{
			value = methodReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Castclass && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Constrained && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Cpobj && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Initobj && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Isinst && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Jmp && instr.Operand is MethodReference methodReference)
		{
			value = methodReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldc_I8 && instr.Operand is long num)
		{
			value = num;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldc_R4 && instr.Operand is float num)
		{
			value = num;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldc_R8 && instr.Operand is double num)
		{
			value = num;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldelem_Any && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldelema && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldfld && instr.Operand is FieldReference fieldReference)
		{
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldflda && instr.Operand is FieldReference fieldReference)
		{
			value = fieldReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldftn && instr.Operand is MethodReference methodReference)
		{
			value = methodReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldobj && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldsfld && instr.Operand is FieldReference fieldReference)
		{
			value = fieldReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldsflda && instr.Operand is FieldReference fieldReference)
		{
			value = fieldReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldstr && instr.Operand is string text)
		{
			value = text;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldtoken && instr.Operand is IMetadataTokenProvider metadataTokenProvider)
		{
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Ldvirtftn && instr.Operand is MethodReference methodReference)
		{
			value = methodReference;
			return true;
		}
	...
	{
		if ((Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Leave || instr.OpCode == OpCodes.Leave_S) && instr.Operand is ILLabel iLLabel)
		{
			value = iLLabel;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Mkrefany && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Newarr && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Newobj && instr.Operand is MethodReference methodReference)
		{
			value = methodReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Refanyval && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Sizeof && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Stelem_Any && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Stfld && instr.Operand is FieldReference fieldReference)
		{
			value = fieldReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Stobj && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Stsfld && instr.Operand is FieldReference fieldReference)
		{
			value = fieldReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Switch && instr.Operand is ILLabel[] array)
		{
			value = array;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Unaligned && instr.Operand is byte b)
		{
			value = b;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Unbox && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
	...
	{
		if (Helpers.ThrowIfNull(instr, "instr").OpCode == OpCodes.Unbox_Any && instr.Operand is TypeReference typeReference)
		{
			value = typeReference;
			return true;
		}
