...
internal class Core21Runtime : CoreBaseRuntime
{
	private sealed class JitHookDelegateHolder
	{
		...
		public unsafe CoreCLR.CorJitResult CompileMethodHook(IntPtr jit, IntPtr corJitInfo, CoreCLR.V21.CORINFO_METHOD_INFO* methodInfo, uint flags, byte** pNativeEntry, uint* pNativeSizeOfCode)
		{
			...
			*pNativeSizeOfCode = 0u;
			...
			try
			{
				CoreCLR.CorJitResult result = InvokeCompileMethodPtr.InvokeCompileMethod(CompileMethodPtr, jit, corJitInfo, methodInfo, flags, pNativeEntry, pNativeSizeOfCode);
				if (ptr != null && (num = *ptr) != 0)
				{
					bool isEnabled;
					_003C24b3ba8a_002D00b7_002D40fc_002Da603_002D2711fa115297_003EMMDbgLog.DebugLogWarningStringHandler message = new _003C24b3ba8a_002D00b7_002D40fc_002Da603_002D2711fa115297_003EMMDbgLog.DebugLogWarningStringHandler(59, 1, out isEnabled);
					if (isEnabled)
					{
						message.AppendLiteral("Native exception caught in JIT by exception helper (ex: 0x");
				...
				if (hookEntrancy == 1)
				{
					try
					{
						...
						if (methodInfo->args.sigInst.classInst != null)
						{
							...
							{
								array[i] = JitHookHelpers.GetTypeFromNativeHandle(methodInfo->args.sigInst.classInst[i]).TypeHandle;
							}
						}
						if (methodInfo->args.sigInst.methInst != null)
						{
							...
							{
								array2[j] = JitHookHelpers.GetTypeFromNativeHandle(methodInfo->args.sigInst.methInst[j]).TypeHandle;
							}
						}
						RuntimeTypeHandle typeHandle = JitHookHelpers.GetDeclaringTypeOfMethodHandle(methodInfo->ftn).TypeHandle;
						RuntimeMethodHandle methodHandle = JitHookHelpers.CreateHandleForHandlePointer(methodInfo->ftn);
						Runtime.OnMethodCompiledCore(typeHandle, methodHandle, array, array2, (IntPtr)(*pNativeEntry), (IntPtr)(*pNativeEntry), *pNativeSizeOfCode);
					}
					catch
					{
					}
	...
	protected sealed class JitHookHelpersHolder
	{
		...
		public JitHookHelpersHolder(Core21Runtime runtime)
		{
			...
			CreateRuntimeMethodInfoStub = runtime.MakeCreateRuntimeMethodInfoStub(type).CreateDelegate<CreateRuntimeMethodInfoStubD>();
			...
			MethodInfo method5;
			using (DynamicMethodDefinition dynamicMethodDefinition3 = new DynamicMethodDefinition("new RuntimeMethodHandle", typeof(RuntimeMethodHandle), new Type[1] { typeof(object) }))
			{
			...
			CreateRuntimeMethodHandle = method5.CreateDelegate<CreateRuntimeMethodHandleD>();
		}
	}
	...
	protected JitHookHelpersHolder JitHookHelpers => Helpers.GetOrInitWithLock(ref lazyJitHookHelpers, sync, createJitHookHelpersFunc, this);
	...
	protected unsafe virtual void InvokeCompileMethodToPrepare(IntPtr method)
	{
		...
		byte* ptr = default(byte*);
		uint num = default(uint);
		InvokeCompileMethodPtr.InvokeCompileMethod(method, IntPtr.Zero, IntPtr.Zero, &cORINFO_METHOD_INFO, 0u, &ptr, &num);
	}
