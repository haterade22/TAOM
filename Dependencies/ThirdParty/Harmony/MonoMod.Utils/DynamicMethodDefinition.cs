...
internal sealed class DynamicMethodDefinition : IDisposable
{
	...
	public DynamicMethodDefinition(DynamicMethodDefinition method)
	{
		...
		Debug = GetDefaultDebugValue();
		Name = method.Name;
		CreateFromDmd(method, out ModuleDefinition Module, out MethodDefinition Definition);
		this.Module = Module;
	...
	{
		ModuleDefinition moduleDefinition = (Module = ModuleDefinition.CreateModule($"DMD:DynModule<{src.Name}>?{GetHashCode()}", new ModuleParameters
		{
			Kind = ModuleKind.Dll,
			ReflectionImporterProvider = MMReflectionImporter.ProviderNoDefault
		}));
		TypeDefinition typeDefinition = new TypeDefinition("", $"DMD<{src.Name}>?{GetHashCode()}", Mono.Cecil.TypeAttributes.Public);
		moduleDefinition.Types.Add(typeDefinition);
		MethodDefinition methodDefinition = new MethodDefinition(src.Name, Mono.Cecil.MethodAttributes.Public | Mono.Cecil.MethodAttributes.Static | Mono.Cecil.MethodAttributes.HideBySig, moduleDefinition.ImportReference(src.Definition.ReturnType));
		typeDefinition.Methods.Add(methodDefinition);
		(Definition = src.Definition.Clone(methodDefinition)).DeclaringType = typeDefinition;
	}
	...
	public MethodInfo Generate(object? context)
	{
		...
		if (Definition.Body.ExceptionHandlers.Any(delegate(Mono.Cecil.Cil.ExceptionHandler eh)
		{
			ExceptionHandlerType handlerType = eh.HandlerType;
			return (handlerType == ExceptionHandlerType.Filter || handlerType == ExceptionHandlerType.Fault) ? true : false;
		}))
		{
			return DMDGenerator<DMDEmitMethodBuilderGenerator>.Generate(this, context);
		}
