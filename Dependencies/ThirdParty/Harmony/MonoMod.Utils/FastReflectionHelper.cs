...
internal static class FastReflectionHelper
{
	...
	private static Exception BadArgException(int arg, RuntimeTypeHandle expectType, object? target, object? result, object?[] args)
	{
		...
		default:
		{
			FormatInterpolatedStringHandler handler = new FormatInterpolatedStringHandler(6, 1);
		...
		case -1:
		{
			FormatInterpolatedStringHandler handler4 = new FormatInterpolatedStringHandler(48, 2);
			handler4.AppendLiteral("Target object is the wrong type; expected ");
			handler4.AppendFormatted(typeFromHandle);
			handler4.AppendLiteral(", got ");
			handler4.AppendFormatted(type);
			text = DebugFormatter.Format(ref handler4);
			break;
		}
		case -2:
		{
			FormatInterpolatedStringHandler handler3 = new FormatInterpolatedStringHandler(48, 2);
			handler3.AppendLiteral("Result object is the wrong type; expected ");
			handler3.AppendFormatted(typeFromHandle);
			handler3.AppendLiteral(", got ");
			handler3.AppendFormatted(type);
			text = DebugFormatter.Format(ref handler3);
			break;
		}
		default:
		{
			FormatInterpolatedStringHandler handler2 = new FormatInterpolatedStringHandler(44, 3);
			handler2.AppendLiteral("Argument ");
			handler2.AppendFormatted(arg);
			handler2.AppendLiteral(" is the wrong type; expected ");
			handler2.AppendFormatted(typeFromHandle);
			handler2.AppendLiteral(", got ");
			handler2.AppendFormatted(type);
			text = DebugFormatter.Format(ref handler2);
			break;
		}
		}
