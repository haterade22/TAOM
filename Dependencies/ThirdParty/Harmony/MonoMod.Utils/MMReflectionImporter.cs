...
internal sealed class MMReflectionImporter : IReflectionImporter
{
	...
	public AssemblyNameReference ImportReference(Assembly asm)
	{
		...
		value = ImportReference(asm.GetName());
		...
		bool TryGetAssemblyNameReference(AssemblyName name, [NotNullWhen(true)] out AssemblyNameReference? assembly_reference)
		{
			...
			{
				AssemblyNameReference assemblyNameReference2 = assemblyReferences[i];
				if (!(name.FullName != assemblyNameReference2.FullName) && assemblyNameReference2.HashIs(asm))
				{
					assembly_reference = assemblyNameReference2;
					return true;
				}
			}
