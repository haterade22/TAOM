			...
			while (targetType != null && targetType != targetType.BaseType)
			{
				...
				foreach (FieldInfo fieldInfo in fields)
				{
					...
					try
					{
						...
						Getters[name] = (object? target) => fastInvoker(target);
						Setters[name] = delegate(object? target, object? value)
						{
							fastInvoker(target, value);
						};
					}
					catch
					{
				...
				foreach (PropertyInfo propertyInfo in properties)
				{
					...
					if (get != null && !Getters.ContainsKey(name2))
					{
						try
						{
							...
							Getters[name2] = (object? target) => fastInvoker2(target);
						}
						catch
						{
							Getters[name2] = (object? obj5) => get.Invoke(obj5, _NoArgs);
						}
					}
					...
					try
					{
						...
						Setters[name2] = delegate(object? target, object? value)
						{
							fastInvoker3(target, value);
						};
					}
					catch
					{
						Setters[name2] = delegate(object? obj5, object? value)
						{
							set.Invoke(obj5, new object[1] { value });
						};
					}
				}
