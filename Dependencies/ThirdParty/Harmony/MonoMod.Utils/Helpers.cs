...
internal static class Helpers
{
	...
	public static T GetOrInit<T>(ref T? location, Func<T> init) where T : class
	{
		...
		return InitializeValue<T, Func<T>>(ref location, FuncInvokeHolder<T>.InvokeFunc, init);
	}
	...
	public static T GetOrInitWithLock<T>(ref T? location, object @lock, Func<T> init) where T : class
	{
		...
		return InitializeValueWithLock<T, Func<T>>(ref location, @lock, FuncInvokeHolder<T>.InvokeFunc, init);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static T GetOrInit<TParam, T>(ref T? location, Func<TParam, T> init, TParam param) where T : class
	{
		ThrowIfArgumentNull(init, "init");
		if (location != null)
		{
			return location;
		}
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	private static T InitializeValue<T, TParam>(ref T? location, Func<TParam, T> init, TParam obj) where T : class
	{
		Interlocked.CompareExchange(ref location, init(obj), null);
		return location;
	}
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	private static T InitializeValueWithLock<T, TParam>(ref T? location, object @lock, Func<TParam, T> init, TParam obj) where T : class
	{
		lock (@lock)
		{
			if (location != null)
