...
internal sealed class MethodSignature : IEquatable<MethodSignature>, IDebugFormattable
{
	...
	bool IDebugFormattable.TryFormatInto(Span<char> span, out int wrote)
	{
		...
		for (int i = 0; i < parameters.Length; i++)
		{
			...
			bool enabled2;
			FormatIntoInterpolatedStringHandler handler2 = new FormatIntoInterpolatedStringHandler(0, 1, span2, out enabled2);
			if (enabled2)
			{
				handler2.AppendFormatted(parameters[i]);
			}
			else
				_ = 0;
			if (!DebugFormatter.Into(span4, out wrote2, ref handler2))
			{
				return false;
			}
			wrote += wrote2;
