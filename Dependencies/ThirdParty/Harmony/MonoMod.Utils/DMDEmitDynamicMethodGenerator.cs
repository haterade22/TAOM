...
internal sealed class DMDEmitDynamicMethodGenerator : DMDGenerator<DMDEmitDynamicMethodGenerator>
{
	...
	protected override MethodInfo GenerateCore(DynamicMethodDefinition dmd, object? context)
	{
		...
		if (originalMethod != null)
		{
			MMDbgLog.DebugLogTraceStringHandler message2 = new MMDbgLog.DebugLogTraceStringHandler(6, 1, out isEnabled);
			if (isEnabled)
			{
				message2.AppendLiteral("orig: ");
				message2.AppendFormatted(originalMethod);
			}
			MMDbgLog.Trace(ref message2);
		}
		MMDbgLog.DebugLogTraceStringHandler message3 = new MMDbgLog.DebugLogTraceStringHandler(9, 3, out isEnabled);
		if (isEnabled)
		{
			message3.AppendLiteral("mdef: ");
			message3.AppendFormatted(methodDefinition.ReturnType?.ToString() ?? "NULL");
			message3.AppendLiteral(" ");
			message3.AppendFormatted(text2);
			message3.AppendLiteral("(");
			message3.AppendFormatted(string.Join(",", methodDefinition.Parameters.Select((ParameterDefinition arg) => arg?.ParameterType?.ToString() ?? "NULL").ToArray()));
			message3.AppendLiteral(")");
		}
