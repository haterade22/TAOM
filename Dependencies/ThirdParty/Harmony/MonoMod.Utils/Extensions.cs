...
internal static class Extensions
{
	...
	public static IMetadataTokenProvider ImportReference(this ModuleDefinition mod, IMetadataTokenProvider mtp)
	{
		...
		if (mtp is Mono.Cecil.CallSite callsite)
		{
	...
	{
		Helpers.ThrowIfArgumentNull(mod, "mod");
		Helpers.ThrowIfArgumentNull(callsite, "callsite");
		...
		{
			ParameterDefinition item = new ParameterDefinition(mod.ImportReference(parameter.ParameterType))
			{
				Name = parameter.Name,
	...
	{
		if (!Helpers.ThrowIfNull(t, "t").IsByRef && !t.IsPointer)
		{
	...
	{
		using DynamicMethodDefinition dynamicMethodDefinition = new DynamicMethodDefinition($"SizeOf<{t}>", typeof(int), new Type[0]);
		...
		return (int)dynamicMethodDefinition.Generate().Invoke(null, null);
	}
