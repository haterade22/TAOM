...
internal static class ReflectionHelper
{
	...
	public static bool HashIs(this AssemblyNameReference asmRef, Assembly asm, bool defaultIfNoHash = true)
	{
		...
		if (asmRef.Hash?.Length == AssemblyHashPrefix.Length + 4)
		{
			...
			for (int j = 0; j < 4; j++)
			{
				if (hash[AssemblyHashPrefix.Length + j] != bytes[j])
				{
					return false;
				}
			}
	...
	private static MemberInfo? _ResolveReflection(MemberReference? mref, Module[]? modules)
	{
		...
		{
			IEnumerable<string> source = GetGenericArgumentsRecursive(mref).Select(delegate(MemberReference x)
			{
				var (asmName2, moduleName2) = GetScope(x);
				return ToCacheKeyPart(asmName2, moduleName2);
			});
			text = string.Concat(text, string.Concat(source.ToArray()));
		}
		else
		{
			text += ToCacheKeyPart(asmName, moduleName);
		}
		lock (ResolveReflectionCache)
		{
			if (ResolveReflectionCache.TryGetValue(text, out WeakReference value) && value != null && value.SafeGetTarget() is MemberInfo result)
			{
				return result;
			}
		}
		...
		while (true)
		{
			...
			if (modules == null)
			{
				...
				if (flag)
				{
					lock (AssemblyCache)
					{
						if (AssemblyCache.TryGetValue(asmName, out WeakReference value2) && value2.SafeGetTarget() is Assembly assembly)
						{
				...
				if (array == null && !flag2)
				{
					...
					{
						if (AssembliesCache.TryGetValue(asmName, out WeakReference[] value3))
						{
			...
			if (mref is TypeReference typeReference2)
			{
				...
				if (mref is TypeSpecification typeSpecification)
				{
					...
					{
						return _Cache(text, ((ArrayType)typeSpecification).IsVector ? type.MakeArrayType() : type.MakeArrayType(((ArrayType)typeSpecification).Dimensions.Count));
					}
					if (typeSpecification.IsGenericInstance)
					{
						return _Cache(text, type.MakeGenericType(((GenericInstanceType)typeSpecification).GenericArguments.Select((TypeReference arg) => _ResolveReflection(arg, null) as Type).ToArray()));
					}
				}
				else
				{
		...
		static IEnumerable<MemberReference> GetGenericArgumentsRecursive(MemberReference memberReference)
		{
			yield return memberReference;
			if (memberReference.DeclaringType is IGenericInstance genericInstance)
			{
				foreach (TypeReference genericArgument in genericInstance.GenericArguments)
				{
					foreach (MemberReference item in GetGenericArgumentsRecursive(genericArgument))
					{
						yield return item;
					}
				}
			}
			if (memberReference is IGenericInstance genericInstance2)
			{
				foreach (TypeReference genericArgument2 in genericInstance2.GenericArguments)
				{
					foreach (MemberReference item2 in GetGenericArgumentsRecursive(genericArgument2))
					{
						yield return item2;
					}
				}
		...
		static (string?, string?) GetScope(MemberReference memberReference)
		{
			TypeReference typeReference3 = memberReference.DeclaringType ?? (memberReference as TypeReference) ?? null;
			IMetadataScope metadataScope = typeReference3?.Scope;
			if (metadataScope is AssemblyNameReference asm)
			{
				return (asm.GetRuntimeHashedFullName(), null);
			}
			if (metadataScope is ModuleDefinition moduleDefinition)
			{
				return (moduleDefinition.Assembly.Name.GetRuntimeHashedFullName(), moduleDefinition.Name);
			}
			if (metadataScope is ModuleReference)
			{
				return (typeReference3.Module.Assembly.Name.GetRuntimeHashedFullName(), typeReference3.Module.Name);
			}
			return (null, null);
		}
		static string ToCacheKeyPart(string? text2, string? text3)
		{
			return " | " + (text2 ?? "NOASSEMBLY") + ", " + (text3 ?? "NOMODULE");
		}
	}
	...
	static ReflectionHelper()
	{
		...
		AssemblyHashNameTag = "@#";
		GetUnmanagedSigHelperMethod = typeof(SignatureHelper).GetMethod("GetMethodSigHelper", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, null, new Type[3]
		{
			typeof(Module),
			typeof(CallingConvention),
			typeof(Type)
		}, null);
		GetUnmanagedSigHelper = GetUnmanagedSigHelperMethod?.TryCreateDelegate<GetUnmanagedSigHelperDelegate>() ?? ((GetUnmanagedSigHelperDelegate)delegate
		{
