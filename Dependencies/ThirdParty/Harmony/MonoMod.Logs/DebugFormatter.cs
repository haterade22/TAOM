	...
	private static bool TryFormatException(Exception e, string? eStr, Span<char> into, out int wrote)
	{
		...
		if (e is ReflectionTypeLoadException ex)
		{
			for (int i = 0; i < 4 && i < ex.Types.Length; i++)
			{
				...
				bool enabled;
				FormatIntoInterpolatedStringHandler handler = new FormatIntoInterpolatedStringHandler(56, 3, span, out enabled);
			...
			if (ex.Types.Length >= 4)
			{
				...
				bool enabled2;
				FormatIntoInterpolatedStringHandler handler2 = new FormatIntoInterpolatedStringHandler(62, 1, span, out enabled2);
				if (enabled2 && handler2.AppendFormatted(newLine))
				{
					handler2.AppendLiteral("System.Reflection.ReflectionTypeLoadException.Types[...] = ...");
				}
				else
					_ = 0;
				if (!Into(span3, out wrote2, ref handler2))
				{
					return false;
				}
				wrote += wrote2;
		...
		if (e is TypeLoadException ex3)
		{
			...
			bool enabled3;
			FormatIntoInterpolatedStringHandler handler3 = new FormatIntoInterpolatedStringHandler(36, 2, span, out enabled3);
			if (enabled3 && handler3.AppendFormatted(newLine) && handler3.AppendLiteral("System.TypeLoadException.TypeName = "))
			{
				handler3.AppendFormatted(ex3.TypeName);
			}
			else
				_ = 0;
			if (!Into(span4, out wrote2, ref handler3))
			{
				return false;
			}
			wrote += wrote2;
		...
		if (e is BadImageFormatException ex4)
		{
			...
			bool enabled4;
			FormatIntoInterpolatedStringHandler handler4 = new FormatIntoInterpolatedStringHandler(42, 2, span, out enabled4);
			if (enabled4 && handler4.AppendFormatted(newLine) && handler4.AppendLiteral("System.BadImageFormatException.FileName = "))
			{
				handler4.AppendFormatted(ex4.FileName);
			}
			else
				_ = 0;
			if (!Into(span5, out wrote2, ref handler4))
			{
				return false;
			}
			wrote += wrote2;
