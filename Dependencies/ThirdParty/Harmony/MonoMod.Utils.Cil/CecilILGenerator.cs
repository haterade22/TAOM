...
internal sealed class CecilILGenerator : ILGeneratorShim
{
	...
	{
		t_LocalBuilder = Type.GetType("System.Reflection.Emit.RuntimeLocalBuilder") ?? typeof(LocalBuilder);
		c_LocalBuilder = (from c in t_LocalBuilder.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
			orderby c.GetParameters().Length descending
			select c).First();
		f_LocalBuilder_position = t_LocalBuilder.GetField("position", BindingFlags.Instance | BindingFlags.NonPublic);
		f_LocalBuilder_is_pinned = t_LocalBuilder.GetField("is_pinned", BindingFlags.Instance | BindingFlags.NonPublic);
