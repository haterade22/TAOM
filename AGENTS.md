...
### Lessons From Prior Reviews (75 reviews, 150+ bugs found)
...
- **Semantic correctness of a SAMPLE taken from an engine collection.** `GetFirstUnit()` / `[0]` / `.First()` on an engine collection is a SAMPLE, not a representative. Code that treats it as the owner of a property (culture, faction, tier) is correct only while the collection is homogeneous. Ask "what happens when this collection is MIXED?" -- signature-correctness review structurally never asks this. (BannerBearers 2026-07-16 C1.)
- **Cross-feature interactions where two TAOM patches contend for one engine decision.** A blanket `return false` Prefix (suppressing vanilla) is a silent monopoly on that decision: every later feature relying on the vanilla path breaks against it with no error. Per-feature review scopes structurally exclude this. When a feature starts producing a new KIND of entity the engine positions specially, grep every TAOM Prefix on the relevant engine method. (BannerBearers 2026-07-16 C2 -- MixedFormations' Patch30 vs banner-bearer slotting.)
