...
### Lessons From Prior Reviews (44 reviews, 131 bugs found)

Last updated: 2026-05-31 (cultural-feats 3-pack review 44 — 0 CRITICAL / 0 HIGH / 1 MED + 1 LOW; both confirmed and fixed. C1 (MED): `TaomPartyTroopUpgradeModel` still used the old narrower `Owner?.Culture ?? party.Culture` resolver — same systemic pattern Codex 43 fixed for speed model. Author's broader audit triggered by C1 found 3 ADDITIONAL sibling models with the same gap (`TaomFoodConsumptionModel`, `TaomPartyMoraleModel`, `TaomPartyHealingModel`); all 4 fixed by adopting the shared `CultureFeatAdapter.FromOrNull(PartyBase?)` + new `CultureFeatAdapter.ResolvePartyCulture(PartyBase?)` raw-CultureObject overload. C2 (LOW): doc-table drift in `docs/features/cultural-feats.md` — stale pre-retune party-size values; fixed. All 10 Known Suspects came back no-bug or disputed-with-evidence (notably S2 vanilla `hero.CurrentSettlement` NRE — cleanly DISPUTED with the right rebuttal, avoiding a false-positive guard). Codex correctly limited itself to the most leveraged findings and did not overclaim. RCA: docs/reviews/rca-cultural-feats-3pack-2026-05-31.md "Codex follow-up" section.
