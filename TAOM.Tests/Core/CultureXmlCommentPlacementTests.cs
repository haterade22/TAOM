...
/// same file, does not exist. Shipped in v2.1.0 and fixed in v2.1.1 on the archived v1.5.0 line.
/// </para>
///
/// <para>
...
public class CultureXmlCommentPlacementTests
{
    ...
    // The archived v1.5.0 branch also asserted that its *_f female templates survived the comment
    // removal. Trunk grew its own female notables under other ids, so the gender half of that check
    // lives in NotableTemplateGenderTests, which asserts on the is_female flag rather than a suffix.
}
