...
public class MapLoadHeartbeatServiceTests
{
    ...
    [TestMethod]
    public void ResetForNewSession_ForgetsTheBaseline_SoASecondCampaignStartsAtZero()
    {
        // The service is a process-wide singleton; a second campaign in one process must not read
        // its first heartbeat against the first campaign's clock and counts.
        var sut = new MapLoadHeartbeatService();
        FirstLine(sut, Sample(parties: 2000), T0);
        sut.ShouldEmit(T0.AddSeconds(5), 10d);
        sut.BuildLine(T0.AddSeconds(5), Sample(parties: 2113));

        sut.ResetForNewSession();

        var t1 = T0.AddMinutes(30);
        Assert.IsTrue(sut.ShouldEmit(t1, 10d), "The first tick of a new session emits immediately, as a fresh service does.");
        var line = sut.BuildLine(t1, Sample(parties: 500));
        StringAssert.Contains(line, "t=+0s", "The offset restarts; 30 minutes of the old campaign must not show.");
        StringAssert.Contains(line, "parties=500(+500)", "The delta is against nothing, not against the old campaign's 2113.");
    }
}
