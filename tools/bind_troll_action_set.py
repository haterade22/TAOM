...
Binding rules (clip names from tools/blender/fab_cave_troll_clip_names.json):
  ...
  the _adder codes (additive overlays), turns, strafes, backward walks, attacks, everything else: inherited from as_human_warrior. The
  anim_troll_attack* clips are not bound to melee codes: a release or blocked code needs a clip with a row in the engine's melee attack
  table (docs/features/troll-race.md, "The swing CTD"; the older reason, engine pose-blend, is refuted).
  TAOM's own actions (EXTRA_BINDINGS, declared in the Armory's action_types.xml, not in as_human_warrior) are appended
  after them: act_troll_brute_force, the Brute Force smash the troll's behaviour tree plays (#649).
...
# TAOM-only actions: not in as_human_warrior, so build_body appends them after the vanilla-derived overrides.
# act_troll_brute_force plays the Fab heavy attack for the #649 prototype; the refined smash clip replaces it.
EXTRA_BINDINGS = (("act_troll_brute_force", "anim_troll_attack1"),)
...
def build_body(actions, indent="\t\t"):
    ...
    for code, clip in EXTRA_BINDINGS:
        if code in seen:
            raise SystemExit("%s is already bound from as_human_warrior; an EXTRA_BINDINGS entry must be TAOM's own" % code)
        seen.add(code)
...
def main(argv=None) -> int:
    ...
    header = ("%s\t\t<!-- Fab Cave Troll Lightweight clips retargeted onto human_skeleton (2026-09-17/18); GENERATED by"
              ...
              "%s\t\t     backward walks and attacks inherit as_human_warrior (release and blocked codes need a clip keyed in the engine's melee attack table); the last line"
              "%s\t\t     is TAOM's own act_troll_brute_force, the behaviour-tree smash (#649). -->" % (nl, nl, nl, nl))
