...
- Lake-Town levy line (T2-T7): Lake-Town Peasant → Lake-Town Militia branches
  into two parallel infantry lines:
    Watch line — 2H halberds/polearms (sturgia_2haxe_*, sturgia_polearm_1_t5,
      billhook_polearm_t2) + 1H sword sidearm, no shield, shock infantry role:
      Watchman → Veteran Watchman → Officer of the Watch.
    Pikeman line — vanilla pikes (fine_pike_t4, military_fork_pike_t3,
      vlandia_pike_1_t5, thamaskene_pike_t4) + 1H sword sidearm, no shield,
      anti-cavalry role: Patrolman → Pikeman → Veteran Pikeman → Hearthguard.
...
- vanilla pikes (fine_pike_t4, vlandia_pike_1_t5, etc.) for the Pikeman line
...
# ---------- Lake-Town Watch line (2H halberds/polearms + 1H sword sidearm, no shield) ----------
# Polearm-primary (2H polearm). OneHanded sidearm. Mild TwoHanded for overhead
# halberd/axe swings (sturgia_2haxe_* items use the TwoHanded skill).
def s_lake_skirmisher_t4() -> Skills:  # Lake-Town Watchman
    return Skills(Athletics=75, OneHanded=70, TwoHanded=30, Polearm=95, Throwing=25)

def s_lake_mariner_t5() -> Skills:  # Lake-Town Veteran Watchman
    return Skills(Athletics=95, OneHanded=100, TwoHanded=50, Polearm=125, Throwing=30)

def s_lake_veteran_t6() -> Skills:  # Lake-Town Officer of the Watch
    return Skills(Athletics=115, OneHanded=130, TwoHanded=70, Polearm=160, Throwing=35)

# ---------- Lake-Town Pikeman line (vanilla pikes + 1H sword sidearm, no shield) ----------
# Polearm-primary (pikes). OneHanded as sidearm. No TwoHanded — pikes thrust
# and swing exclusively via Polearm skill.
def s_footman_t4() -> Skills:  # Lake-Town Patrolman
    return Skills(Athletics=80, OneHanded=70, Polearm=100, Bow=20)

def s_spearman_t5() -> Skills:  # Lake-Town Pikeman
    return Skills(Athletics=100, OneHanded=85, Polearm=135, Bow=25)

def s_veteran_spearman_t6() -> Skills:  # Lake-Town Veteran Pikeman
    return Skills(Athletics=120, OneHanded=100, Polearm=170, Bow=30)
...
# ---------- Royal Crossbow branch (parallel to Bow line, silver chivlary armor) ----------
# Crossbow-primary; armor uses archer b01..b04 silver mesh per user spec. Crossbow is
# a 2H weapon — no shield — so OneHanded for melee sidearm and Crossbow as primary.
def s_crossbowman_t4() -> Skills:
    return Skills(Athletics=75, OneHanded=55, Crossbow=90, Bow=15)

def s_veteran_crossbowman_t5() -> Skills:
    return Skills(Athletics=95, OneHanded=70, Crossbow=125, Bow=20)

def s_royal_crossbowman_t6() -> Skills:
    return Skills(Athletics=115, OneHanded=85, Crossbow=160, Bow=25)

def s_master_crossbowman_t7() -> Skills:
    return Skills(Athletics=135, OneHanded=100, Crossbow=195, Bow=30)
...
# Royal-tier pike-wielding anti-cavalry shock infantry. Polearm-primary; pikes
# use Polearm exclusively, no TwoHanded needed.
def s_hearthguard_t7() -> Skills:
    return Skills(Athletics=145, OneHanded=150, Polearm=235, Bow=35)
...
def chivalry_armor_explicit(suffix: str) -> dict[str, str]:
    """Chivlary/chivalry cavalry armor at an EXPLICIT suffix (a01..b04). Used for the
    cavalry Light/Heavy split: Light line uses a01..a04 (bronze), Heavy line uses
    b01..b04 (silver). Note Solus's chest mesh uses 'chivalry' spelling; the other
    4 slots use the 'chivlary' typo."""
    return {
        "Head":   f"sk_dale_helmet_chivlary_{suffix}",
        "Body":   f"sk_dale_chest_chivalry_{suffix}",
        "Gloves": f"sk_dale_gauntlet_chivlary_{suffix}",
        "Leg":    f"sk_dale_boots_chivlary_{suffix}",
        "Cape":   f"sk_dale_shoulder_chivlary_{suffix}",
    }
...
def infantry_armor_explicit(suffix: str) -> dict[str, str]:
    """Infrantry (Solus's typo for 'infantry') royal-infantry armor at an EXPLICIT
    suffix. Used by the Great Infantry line which pairs one bronze (a) + one silver
    (b) variant per tier — 'variation per level' per user spec."""
    return {
        "Head":   f"sk_dale_helmet_infrantry_{suffix}",
        "Body":   f"sk_dale_chest_infrantry_{suffix}",
        "Gloves": f"sk_dale_gauntlet_infrantry_{suffix}",
...
def archer_armor_explicit(suffix: str) -> dict[str, str]:
    """Archer armor at an EXPLICIT suffix (a01..b04). Used by the Bow line (bronze
    a01..a04) and the new Crossbow line (silver b01..b04). Solus's archer shoulder
    mesh is missing a02/a04/b02/b04 — falls back to next-lower available shoulder."""
    shoulder_fallback = {
        "a01": "a01", "a02": "a01", "a03": "a03", "a04": "a04",
        "b01": "b01", "b02": "b01", "b03": "b03", "b04": "b04",
    }
    return {
        "Head":   f"sk_dale_helmet_archer_{suffix}",
        "Body":   f"sk_dale_chest_archer_{suffix}",
        "Gloves": f"sk_dale_gauntlet_archer_{suffix}",
        "Leg":    f"sk_dale_boots_archer_{suffix}",
        "Cape":   f"sk_dale_shoulder_archer_{shoulder_fallback[suffix]}",
    }


def lake_town_armor_explicit(
    suffix: str,
    *,
    no_helmet: bool = False,
    no_shoulder: bool = False,
    no_bracers: bool = False,
) -> dict[str, str]:
    ...
    levy lines per user spec — each tier gets a specific suffix:
        Peasant a01 (chest+boots only — no helmet/shoulder/bracers)
        → Militia a01 (full 5 slots) → Watchman a02 → Veteran Watchman a03
        → Officer of the Watch a04;
...
def build_troops() -> list[Troop]:
    ...
    troops.append(Troop(
        ...
        rosters=[
            EquipmentRoster({
                ...
                **lake_town_armor_explicit("a01", no_helmet=True, no_shoulder=True, no_bracers=True),
            }),
            EquipmentRoster({
    ...
    # ----- Lake-Town Watch line (T4-T6) — 2H halberds/polearms + 1H sword sidearm, no shield -----
    # Armor: Watchman a02, Veteran Watchman a03, Officer of the Watch a04 (full 5-slot mariner).
    # Weapons: sturgia_2haxe_* (2H axes — halberd-class) + sturgia_polearm_1_t5 + billhook_polearm_t2.
    troops.append(Troop(
    ...
    # ----- Lake-Town Pikeman line (T4-T7) — vanilla pikes + 1H sword sidearm, no shield -----
    # Armor: Patrolman b01, Pikeman b02, Veteran Pikeman b03, Hearthguard b04.
    # Weapons: fine_pike_t4 / military_fork_pike_t3 / vlandia_pike_1_t5 / thamaskene_pike_t4.
    troops.append(Troop(
        id="dale_footman",
    ...
    troops.append(Troop(
        ...
        is_basic_troop=True,
        skills=s_militia(),
        upgrades=["dale_riverman", "dale_man_at_arms", "dale_bowman", "dale_crossbowman", "dale_outrider"],
        rosters=[
            EquipmentRoster({
                "Item0": "sturgia_sword_2_t3",
                "Item1": "sturgia_infantry_shield_a",
    ...
    troops.append(Troop(
        ...
        tier=4, default_group="Ranged",
        skills=s_crossbowman_t4(),
        upgrades=["dale_veteran_crossbowman"],
        rosters=[
            EquipmentRoster({
                "Item0": "crossbow_c",
                "Item1": "bolt_a",
    ...
    troops.append(Troop(
        id="dale_veteran_crossbowman",
        display_name="Dalian Veteran Crossbowman",
        tier=5, default_group="Ranged",
        skills=s_veteran_crossbowman_t5(),
        upgrades=["dale_royal_crossbowman"],
        rosters=[
            EquipmentRoster({
                "Item0": "crossbow_d",
                "Item1": "bolt_b",
    ...
    # ----- Royal Infantry branch (T4-T7, "Great Infantry") -----
    # Armor: one bronze (a) + one silver (b) variant per tier — "variation per level".
    troops.append(Troop(
        id="dale_man_at_arms",
    ...
    troops.append(Troop(
        ...
        rosters=[
            EquipmentRoster({
                ...
                "Item1": "sturgia_infantry_shield_a",
                "Item2": "sturgia_polearm_1_t5",
                **infantry_armor_explicit("a04"),
            }),
            EquipmentRoster({
                "Item0": "sturgia_2haxe_2_t5",
                "Item2": "sturgia_noble_sword_4_t5",
                **infantry_armor_explicit("b04"),
            }),
        ],
    ))
