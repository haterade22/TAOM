...
|--------|---------|--------|
| `register_dale_spear_descriptions.py` | Registers the four Dale spears' blades **and** handles under `OneHandedPolearm` in the Armory's `weapon_descriptions.xslt`, so troops carrying a shield beside one will actually fight with it. The **item** list is the hardcoded input (which weapons should be one-handed is a design call); the piece ids are derived from those items' own `<CraftedItem>` blocks, so a re-modelled spear moves its registration instead of silently un-fixing itself. Refuses to write a piece id with no `<CraftingPiece>` definition. Idempotent — the block sits inside `<!-- TAOM-DALE-1H:START/END -->` markers; `--revert` removes it, backup `weapon_descriptions.xslt.bak-dale1h`. **Re-run condition: any LOTRLOME_Armory update**, which overwrites the live file. Gate: `audit_polearm_shield_parity.py`. Record: `docs/reference/lotrlome-armory-snapshot/README.md`. | `<game>/Modules/LOTRLOME_Armory/ModuleData/weapon_descriptions.xslt` (external) |
