...
| `analyze_war_theaters.py` | **Read-only** war-theater and march-radius report for `army_targeting.json`. Resolves all 988 LIVE TAOM_Map settlements through owner-clan to kingdom (villages inherit via the COMPONENT `bound=`, not the settlement element), computes G (the engine's average gap between the closest two towns) and every `Hostile` pair's **minimum fortification-to-fortification** gap, then flags kingdoms with no enemy inside the radius and priority entries beyond it. Distances are straight-line between gate positions: both halves of the ratio carry that bias so it largely cancels, and what does not cancel is terrain, which only ever makes the real gap larger. That is why `--apply` is safe: it can leave a too-far entry in place but will not prune a near one. `--apply` rewrites the config preserving its BOM and line endings, and folds scalar arrays back onto one line so the file stays hand-editable. First run pruned 26 of 80 inert entries. **Pruning re-steepens the priority decay for the survivors**, since the boost spans the list. | `--apply`, `--radius N` |
