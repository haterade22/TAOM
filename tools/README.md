...
| `blender_assemble_lond_cirion_wall.py` | Assembles Lond Cirion wall section 01 (symmetric L: 6 × L3 20 m walls + merlons, 4 in-line `tower_l3_a`, corner `tower_l3_b` at z+5 with adjacent-face doors, full tower interiors) from `Scenes/Gondor/walls/` into the MT-template kit format (base/.lod3/.lod6/bo_ per section, origin pivot; material `.NNN`-dedup for the editor). Piece registration math in the docstring — copy the pattern for further sections. |
