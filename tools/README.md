...
| `blender_prep_witchking_throne.py` | Headless Blender single-prop prep for the Tripo AI throne (2026-07-25): 2.5 m scale, `sm_mordor_mm_throne_001` + `bo_` twin, **xatlas-style chart re-UV** (region-growing + fragment merge; Smart UV Project fragments dense organic meshes — probe data in the docstring), selected-to-active rebake of all maps + fresh AO, Cycles preview render. `--probe-angles`/`--probe-spreads` measure UV candidates without baking. |
